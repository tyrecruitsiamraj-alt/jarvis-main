...
> Path มาตรฐาน (source of truth) คือ `.claude/skills/request-control-tower-advisor/` ซึ่ง track อยู่ใน repository นี้ — copy อื่นเช่น `.agents/skills/` เป็น mirror เฉพาะเครื่อง หากไม่ตรงกันให้ยึด `.claude/skills/` แล้ว sync mirror ใหม่ หากกติกาใน Handbook กับ Skill ไม่ตรงกัน ต้องหยุดและปรับเอกสารทั้งสองชุดให้ตรงกันก่อนแก้ calculation code
...
- Source of truth ของ Skill รวมเป็นชุดเดียวแล้วที่ `.claude/skills/...` (ทั้ง `CLAUDE.md` และ `AGENTS.md` ชี้ที่เดียวกัน) — copy `.agents/skills/...` เป็น mirror เฉพาะเครื่อง ห้ามแก้ mirror โดยไม่แก้ต้นทาง
...
Editing map ฉบับเต็มตาม path มาตรฐาน: `.claude/skills/request-control-tower-advisor/references/09-editing-map.md`
...
> สำหรับงานใน repository นี้ คุณไม่ต้องส่ง Skill ซ้ำทุกครั้ง หาก Skill อยู่ที่ `.claude/skills/request-control-tower-advisor/` หรือถูกลงทะเบียนให้ session เปิดอ่านได้ เพียงระบุว่าเป็นงาน Request Control Tower ระบบจะ route ให้ใช้ Skill นี้อัตโนมัติ
...
```text
อ่าน AGENTS.md, .claude/skills/request-control-tower-advisor/SKILL.md,
