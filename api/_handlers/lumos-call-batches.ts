 ...
 * ⚠️ ตั้งแต่ถอดโหมด assist ทิ้ง (11 ส.ค. 2569) **ไม่มีทางไหนสร้างชุด `pending_approval`
 * ได้อีกแล้ว** — `action:'approve'` จึงเหลือไว้เผื่อชุดเก่าที่ค้างบนฐานเท่านั้น
 * (มีอยู่ 1 ชุด · เจ้าของสั่งปล่อยค้างไว้) ถ้าจะให้ staff อนุมัติเองแก้ที่ canApprove() ที่เดียว
 */
import {
  withRbac,
  sendError,
