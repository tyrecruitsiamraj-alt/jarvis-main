...
async function loadLumosTeam(): Promise<LumosTeamStats> {
  ...
  for (const r of rows) {
    ...
    if (!lane) continue;
    /**
     * 🔴 **"ส่งให้ AI ไปแล้ว" = ทุกสายที่เคยส่ง รวมที่ยกเลิกภายหลัง**
     * (เจ้าของสั่ง 16 ก.ย. 2569: *"ตรงยกเลิกอ่านแล้วงง … ก็ควรเป็น 82 + 20 เท่าไหร่
     * แล้วแจกแจงมามันต้องได้ยอดรวมนั้น"*)
     *
     * ของเดิมหักยกเลิกออกจากยอดบน ⇒ แถวยกเลิกลอยอยู่ข้างล่างแบบบวกกับใครไม่ได้
     * คนอ่านเลยไม่รู้ว่ามันอยู่ในยอดบนหรือไม่ · ตอนนี้ **สี่ช่องล่างบวกกันได้ยอดบนเป๊ะ**:
     * รอโทร + รอผล + รู้ผลแล้ว + ยกเลิก
     *
     * ⚠️ ยกเลิกยังคง **ไม่ถูกนับเป็นผลสำเร็จ** ทุกที่ (Success Rate ยังหารจากสายที่ได้คุย)
     * — กติกาแม่ของโปรเจกต์ที่ห้ามนับของที่ยกเลิกเป็นของที่ทำได้ ยังอยู่ครบ
     */
    lane.total = r.n + r.cancelled;
