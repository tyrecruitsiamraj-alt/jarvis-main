...
export function buildFollowMessage(
  ...
  return renderLines(
    lines,
    {
      ...
      /**
       * 🔴 **ไม่มีหน่วยงาน ต้องมีคำแทน ห้ามปล่อยว่าง** (13 ก.ย. 2569)
       *
       * ขั้น "หน่วยงาน" ในฟอร์มเป็นช่อง **ข้ามได้** ⇒ เคสนี้เกิดจริงแน่นอน
       * ปล่อยว่างแล้วบทที่คนจริงได้ยินคือ *"เตรียมตัวไปทำงาน หน่วยงาน แล้วใช่ไหมคะ"*
       * กับ *"ถึงหน่วยงาน เรียบร้อยแล้วใช่ไหมคะ"* — คำว่า "หน่วยงาน" ค้างเติ่งไม่มีชื่อต่อท้าย
       * (ผู้ทดสอบตาใหม่ทักไว้ 13 ก.ย. · ยืนยันด้วยการรันบทจริงแล้ว)
       *
       * ⚠️ **แก้ที่ค่า ไม่ใช่ที่ template** โดยตั้งใจ — บทถูกแอดมินแก้เองได้จากหน้าตั้งค่า
       * และในฐานมี override ของ `follow_repeat` อยู่จริง · แก้ template อย่างเดียว
       * บทที่ถูก override จะยังพังอยู่ · "ที่นัดไว้" ต่อท้ายคำว่า "หน่วยงาน" ได้พอดี
