...
/**
 * คอลัมน์ "ราชการ / เอกชน" (เจ้าของสั่ง 25 ส.ค. 2569)
 *
 * 🔴 ค่ามาจากตาราง unit_sector คีย์ site_code — ไม่ได้อยู่ใน JobRequest
 *    จึงส่งเข้ามาทาง context · "ยังไม่ระบุ" ต้องถือเป็น**ค่าว่าง** ตกท้ายเสมอ
 */
describe('เรียงตามประเภทหน่วยงาน', () => {
  const j = (id: string, site: string | undefined): JobRequest =>
    job({ id, request_no: id, site_code: site });

  const jobs = [j('A', '01LBD0001'), j('B', '02LML0002'), j('C', '03LBA0003'), j('D', undefined)];
  const ctx = {
    sectors: { '01LBD0001': 'private', '02LML0002': 'government' } as const,
  };

  it('เรียงขึ้น: ราชการ มาก่อน เอกชน (ก-ฮ) · ที่ยังไม่ระบุตกท้าย', () => {
    const got = sortJobsByTableColumn(jobs, { column: 'sector', dir: 'asc' }, new Date(), ctx);
    expect(got.slice(0, 2).map((x) => x.id)).toEqual(['B', 'A']);
    // C (ไม่มีค่า) กับ D (ไม่มี site_code) ต้องอยู่ท้าย
    expect(got.slice(2).map((x) => x.id).sort()).toEqual(['C', 'D']);
  });

  it('🔴 เรียงลง ค่าว่างก็ยังตกท้าย (ไม่ใช่ขึ้นมาบนสุด)', () => {
    const got = sortJobsByTableColumn(jobs, { column: 'sector', dir: 'desc' }, new Date(), ctx);
    expect(got.slice(0, 2).map((x) => x.id)).toEqual(['A', 'B']);
    expect(got.slice(2).map((x) => x.id).sort()).toEqual(['C', 'D']);
  });

  it('ไม่ส่ง context มาเลย = ถือว่ายังไม่ระบุทุกใบ ไม่ระเบิด', () => {
    const got = sortJobsByTableColumn(jobs, { column: 'sector', dir: 'asc' });
    expect(got).toHaveLength(4);
  });

  it('คอลัมน์นี้อยู่ในชุดที่กดเรียงได้ และมีป้ายชื่อ', () => {
