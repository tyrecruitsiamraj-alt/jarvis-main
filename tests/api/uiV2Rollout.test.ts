...
describe('ของที่รอบรื้อทำหายต้องกลับมาครบ', () => {
  ...
  /**
   * 🔴 **ข้อ "หาย-4" ถูกยกเลิกแล้ว 15 ก.ย. 2569** — เจ้าของสั่ง *"หน้าแรกเอาหุ่นยนต์ออก
   * ให้หน่อย"* ⇒ ไม่มีหุ่นให้เคารพ "ลดการเคลื่อนไหว" อีกต่อไป
   *
   * ด่านนี้จึงกลับด้าน: **หุ่นต้องไม่แอบกลับมา** ทั้งสองโฉม
   * (ไฟล์ภาพยังอยู่ใน `public/` โดยตั้งใจ — อยากได้คืนให้ใส่ `<img>` กลับ
   * แล้วแก้เทสต์ข้อนี้พร้อมกัน จะได้รู้ตัวว่ากำลังย้อนคำสั่งเจ้าของ)
   */
  it('🔴 หน้าแรกต้องไม่มีหุ่นยนต์ทั้งสองโฉม (เจ้าของสั่งถอด 15 ก.ย. 2569)', () => {
    for (const f of ['src/components/home/HomeDeckV2.tsx', 'src/components/home/CommandDeck.tsx']) {
      // ⚠️ จับเฉพาะ **การใช้งานจริง** (`'/robot-mascot.webp'`) ไม่ใช่คอมเมนต์ที่เล่าที่มา
      expect(read(f)).not.toMatch(/['"]\/robot-mascot/);
    }
  });
