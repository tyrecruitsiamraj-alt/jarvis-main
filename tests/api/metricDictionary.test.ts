...
/**
 * 🔴 ตำหนิผู้ตรวจรับ 6 ก.ย. 2569 — บอร์ดทีมสรรหา: คนใหม่เอา "ผู้สมัครทั้งหมด" +
 * "ยังไม่มีใครติดต่อ" + "ค้างเกิน 1 วันไม่มีใครแตะ" มาบวกกันแล้วงง (ทั้งที่ข้อสุดท้าย
 * นับซ้อนอยู่ในข้อกลางแล้ว — พิสูจน์จาก OVERVIEW_BUCKETS.untouched ⊆ "not called")
 */
describe('เชิงอรรถกันบวกผิด — ทีมสรรหา', () => {
  const src = fs.readFileSync(PANEL, 'utf8');

  it('มีเชิงอรรถบอกว่า "ค้างเกิน 1 วัน" นับซ้อนอยู่ใน "ยังไม่มีใครติดต่อ" แล้ว', () => {
    expect(src).toContain('นับซ้อนอยู่ใน');
  });
});

/**
 * 🔴 ตำหนิผู้ตรวจรับ 6 ก.ย. 2569 — แถวที่กดแล้วเปิด dialog (onPress) กับแถวที่นำทาง
 * ไปหน้าอื่น (to) หน้าตาเหมือนกันหมด (ลูกศรเดียวกัน) กดแล้วไม่รู้ว่าจะได้ผลแบบไหน
 */
describe('แถวเปิด dialog ต้องมีคำกำกับต่างจากแถวนำทาง', () => {
  const src = fs.readFileSync(PANEL, 'utf8');
