...
describe('เลนคิวโทรบนกล่องทีม', () => {
  it('ส่งจำนวน "รอโทรค้างเกิน 2 วัน" ออกมาให้จอ และเกณฑ์ต้องเป็น 2 วันตัวเดียวกับหน้าอื่น', async () => {
    ...
    expect(lanes.follow).toEqual({
      ...
      cancelled: 2,
      /**
       * 🔴 **ผลละเอียดเป็น null เมื่ออ่านไม่ได้** (16 ก.ย. 2569)
       * เทสต์นี้ mock dbQuery ให้คืนเฉพาะคิวรีเลน ⇒ คิวรีผลละเอียดได้ rows ว่าง
       * ⇒ ต้องเป็น null ไม่ใช่ศูนย์ทุกถัง — จอจะได้ไม่วาด 0 ปลอมว่าไม่มีใครตอบรับเลย
       */
      results: null,
    });
