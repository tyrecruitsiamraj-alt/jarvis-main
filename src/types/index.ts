...
/**
 * รายได้จริงหนึ่งงวดของคนที่ออก — **ยอดแบบใบแจ้งเงินเดือน (eSlip)**
 * เจ้าของเคาะ 27 ส.ค. 2569: *"เอายอด eSlip เพราะต้องการยอดที่เขารับจริง ๆ ของ Site นั้น ๆ"*
 * แหล่ง: `wg2_payment_head` + `wg2_payment_tax` (จ่ายจริง) **แยกตามไซต์อยู่แล้วในฐาน**
 */
export type ResignedIncomeMonth = {
  /** วันเริ่มงวด `YYYY-MM-DD` */
  from: string | null;
  /** วันจบงวด — งวดสุดท้ายของคนที่ออกมักไม่เต็มงวด */
  to: string | null;
  /** เงินได้รวมของงวดนั้น (`seq_paid`) — `null` = อ่านไม่ได้ ห้ามอ่านเป็น 0 บาท */
  pay: number | null;
  /** เงินหักรวมของงวดนั้น (`seq_deduct`) — ภาษี · ประกันสังคม · เงินประกัน · หนี้อื่น */
  deduct: number | null;
  /** **สุทธิที่เขารับจริง** = เงินได้ − เงินหัก (วัดแล้วไม่มีงวดไหนติดลบ) */
  net: number | null;
};
