...
export type ToneClasses = {
  ...
  softHover: string;
  /**
   * **พื้นย้อมสีโทนจาง ๆ ทั้งแถว/ทั้งช่อง** + hover (เจ้าของสั่ง 8 ก.ย. 2569: *"คนไหนไม่ไป
   * ขอสีแดงอ่อน ๆ ในช่องนั้นไปเลย เปิดมารู้เลย"*)
   * ต่างจาก `soft` ที่เป็น "กระดาษเทา + ขอบสี" (บอกความหมายด้วยขอบ) — `wash` ย้อมพื้นเลย
   * ใช้เฉพาะแถวที่ต้องสะดุดตาตั้งแต่ยังไม่ได้อ่าน ไม่ใช่ทาทุกแถวจนจอเป็นสีรุ้ง
   */
  wash: string;
  /**
...
export const TONE: Record<ToneKey, ToneClasses> = {
  neutral: {
    ...
    wash: 'bg-slate-50 hover:bg-slate-100/70 dark:bg-slate-950/30 dark:hover:bg-slate-950/50',
  ...
  info: {
    ...
    softHover: 'hover:bg-slate-100 dark:hover:bg-slate-800',
    wash: 'bg-sky-50 hover:bg-sky-100/70 dark:bg-sky-950/30 dark:hover:bg-sky-950/50',
  ...
  primary: {
    ...
    softHover: 'hover:bg-slate-100 dark:hover:bg-slate-800',
    wash: 'bg-blue-50 hover:bg-blue-100/70 dark:bg-blue-950/30 dark:hover:bg-blue-950/50',
  ...
  success: {
    ...
    softHover: 'hover:bg-slate-100 dark:hover:bg-slate-800',
    wash: 'bg-emerald-50 hover:bg-emerald-100/70 dark:bg-emerald-950/30 dark:hover:bg-emerald-950/50',
  ...
  warn: {
    ...
    softHover: 'hover:bg-slate-100 dark:hover:bg-slate-800',
    wash: 'bg-amber-50 hover:bg-amber-100/70 dark:bg-amber-950/30 dark:hover:bg-amber-950/50',
  ...
  danger: {
    ...
    softHover: 'hover:bg-slate-100 dark:hover:bg-slate-800',
    wash: 'bg-rose-50 hover:bg-rose-100/70 dark:bg-rose-950/30 dark:hover:bg-rose-950/50',
  ...
  violet: {
    ...
    softHover: 'hover:bg-slate-100 dark:hover:bg-slate-800',
    wash: 'bg-violet-50 hover:bg-violet-100/70 dark:bg-violet-950/30 dark:hover:bg-violet-950/50',
  ...
  orange: {
    ...
    softHover: 'hover:bg-slate-100 dark:hover:bg-slate-800',
    wash: 'bg-orange-50 hover:bg-orange-100/70 dark:bg-orange-950/30 dark:hover:bg-orange-950/50',
  ...
  teal: {
    ...
    softHover: 'hover:bg-slate-100 dark:hover:bg-slate-800',
    wash: 'bg-teal-50 hover:bg-teal-100/70 dark:bg-teal-950/30 dark:hover:bg-teal-950/50',
...
/**
 * **จานสีของฉากเปิด (splash)** — เจ้าของสั่ง 4 ก.ย. 2569:
 * *"เปลี่ยนโทนสีเป็น ขาว + Burgundy ... background เป็นโทน navy"*
 *
 * 🔴 อยู่ที่นี่ที่เดียว **ห้ามพิมพ์ hex ในไฟล์จอ** (เทสต์ `typographyRules` คุมไว้)
 * ⚠️ ตั้งใจให้ **คนละจานกับ `FRONT_SCENE`** (ครีม-เขียวป่า) เพราะฉากเปิดทำหน้าที่
 * เป็น "ผ้าคลุมเข้ม" ที่เปิดออกแล้วเผยหน้าสว่าง — ถ้าใช้จานเดียวกันจังหวะเผยจะจืด
 */
export const INTRO_SCENE = {
  /** พื้นฉาก — น้ำเงินกรมท่าเข้ม (ไล่เฉดให้ไม่แบน) */
  navy: '#0f1a33',
  navy2: '#16233f',
  /** ตัวอักษรหลัก — ขาวกระดาษ (ไม่ใช่ขาวจ้า ให้เข้ากับ Kanit) */
  paper: '#f7f6f2',
  /** เบอร์กันดี — คำแรกของโลโก้ กับเส้นเน้น */
  burgundy: '#8c2f39',
  /** เบอร์กันดีสว่างขึ้น — ใช้บนพื้นกรมท่าให้ยังอ่านออก (วัดบนจอแล้ว) */
  burgundyLight: '#b8434f',
} as const;

/**
 * **จานสีหน้าเข้าสู่ระบบ — ขาว + เบอร์กันดี บนพื้นกรมท่า**
 *
 * เจ้าของสั่ง 4 ก.ย. 2569: *"เปลี่ยนโทนสีเป็น ขาว + Burgundy · background เป็นโทน navy ...
 * พอดีของฉันมันมีหลาย BU ถ้าเขียวเยอะไปมันจะดูเป็นการขายพวกงานสวนเกิน"*
 *
 * 🔴 **นี่คือเหตุผลเชิงธุรกิจ ไม่ใช่รสนิยม** — จานเขียว+ภาพป่าเดิม (`FRONT_SCENE`)
 * อ่านเป็น "ธุรกิจดูแลสวน" ซึ่งเป็นแค่ BU เดียว (LM) · หน้าด่านหน้าต้องเป็นกลาง
 * พอที่คนขับรถ (LBD) · ธุรการ/IT (LBA) · สวน (LM) · SO NEXT เห็นแล้วรู้สึกว่าเป็นบ้านตัวเอง
 * ⚠️ `FRONT_SCENE` ยังอยู่ (ยังเป็นจานของฉากภาพ) — ห้ามลบ เผื่อเจ้าของอยากสลับกลับ
 */
export const LOGIN_SCENE = {
  /** ผืนหลัก — ขาว (เจ้าของเคาะ 4 ก.ย. 2569: "ขาว navy burgundy") */
  canvas: '#ffffff',
  /** แสงนวลที่ทาบพื้นขาว — กรมท่า/เบอร์กันดี จาง ๆ ให้ไม่แบน */
  auraNavy: 'rgba(24, 46, 92, .16)',
  auraBurgundy: 'rgba(140, 47, 57, .12)',
  /**
   * กระจกของการ์ด — เจ้าของสั่ง 5 ก.ย. 2569: *"ตรงกล่องทำให้มันดูโปร่งแสงกว่านี้หน่อย"*
   * ⇒ ลดความทึบลง แล้วไปชดเชยการอ่านออกด้วย **การเบลอพื้นหลัง** แทน (ที่ `GLASS_CARD`)
   * ไม่ใช่ด้วยการทาสีขาวทับ — เห็นภาพป่าผ่านการ์ดได้ แต่ตัวหนังสือยังคม
   */
  glass: 'rgba(255, 255, 255, .58)',
  glassStrong: 'rgba(255, 255, 255, .72)',
  /** เส้นขอบบาง */
  line: 'rgba(18, 32, 60, .10)',
  /** ตัวหนังสือหลัก — กรมท่าเข้ม (ไม่ใช้ดำสนิท ให้เข้ากับ Kanit) */
  ink: '#12203c',
  navy: '#182e5c',
  muted: 'rgba(18, 32, 60, .60)',
  /** ปุ่มลงมือ — เบอร์กันดีไล่เฉด */
  burgundy: '#8c2f39',
  burgundy2: '#a4373f',
  /** คำเน้นในหัวเรื่อง — เบอร์กันดีบนพื้นขาว (ไม่ต้องสว่างเหมือนตอนพื้นเข้ม) */
  accent: '#8c2f39',
} as const;
