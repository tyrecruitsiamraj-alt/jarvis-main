 ...
 *
 * โครงใหม่ = **งานจริงเดินเป็นสายพาน 6 ขั้น** ใบขอ → ประกาศรับ → ผู้สมัคร →
 * จับคู่&โทร → ติดตาม → ดูแลหลังเริ่มงาน · ของที่ไม่ใช่ขั้นตอน (คลังคน · WL · Dashboard)
 * แยกเป็นกลุ่ม "คลังข้อมูล" ต่างหาก ไม่ปนกับสายพาน
 *
...
export type ConveyorStep = {
  /** เลขขั้น 1–6 — เลขเดียวกับที่หัวหน้าจอประกาศ ("ขั้นที่ 3 ของสายพาน") */
  step: number;
...
export const CONVEYOR_STEPS: ConveyorStep[] = [
  {
    ...
    key: 'requests',
    label: 'ใบขอ',
    blurb: 'หน่วยงานขอคนมา — ทุกใบบอกตัวเองว่าอยู่ขั้นไหนและค้างมานานเท่าไหร่',
    path: '/jobs/list',
    icon: ClipboardList,
    functionId: 'unit_requests_read',
    // ⚠️ ต้องมาก่อน `/jobs/board` ในการเทียบ — ดู stepForPath (เจาะจงชนะกว้าง)
  ...
  },
  {
    ...
    key: 'postings',
    label: 'ประกาศรับ',
    blurb: 'ปล่อยใบขึ้นหน้าสมัคร · คำขอโพสต์หาคน · ส่งไปดูดประกาศ',
    path: '/jobs/board?view=postings',
    icon: Megaphone,
    functionId: 'recruit_postings',
    match: ['/matching/job-postings', '/recruit/channels'],
  },
  {
    step: 3,
    key: 'applicants',
    label: 'ผู้สมัคร',
    blurb: 'คนที่สมัครเข้ามาแล้ว — คัดกรองและตัดสินใจว่าจะเสนอใบไหน',
