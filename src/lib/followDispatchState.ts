...
export type FollowDispatchMeta = {
  ...
  /**
   * ⚠️ **ยังไม่มีปุ่ม "ส่งใหม่" ด้วยมือบนจอ** (เจ้าของเคาะ 13 ก.ย. 2569: ใช้โทรเองแทน)
   * ธงนี้จึงเป็นแค่ "ส่งซ้ำแล้วมีความหมาย" ไว้ให้ตัวส่งซ้ำอัตโนมัติ/งานอนาคตใช้
   * 🔴 ห้ามเขียน hint ว่า "กดส่งใหม่" — บนจอไม่มีปุ่มนั้น
   */
  retryable: boolean;
};
...
export const FOLLOW_DISPATCH_META: Record<FollowDispatchState, FollowDispatchMeta> = {
  ...
  guarded: {
    ...
    hint: 'ตอนนั้นเปิดบัญชีห้ามโทรไม่ได้ ระบบจึงกันไว้ก่อนเพื่อความปลอดภัย · ตั้งรอบใหม่ที่ปุ่มดินสอ หรือกดปุ่มโทรข้างชื่อเพื่อโทรเอง',
    needsAction: true,
    retryable: true,
  },
  no_phone: {
    label: 'ไม่ได้ส่ง — ไม่มีเบอร์ที่โทรได้',
    hint: 'ต้องเป็นเบอร์มือถือ 10 หลัก · แก้เบอร์ที่ปุ่มดินสอแล้วตั้งรอบใหม่',
    needsAction: true,
    retryable: true,
  },
  off: {
  ...
  push_failed: {
    ...
    hint: 'อยู่ในคิวฝั่งเราแล้วแต่ดันไปหา Lumos ไม่สำเร็จ · ระบบส่งซ้ำให้เองทุก 1 นาที ภายใน 2 ชั่วโมงหลังเวลานัด · ยังค้างอยู่ให้กดปุ่มโทรข้างชื่อเพื่อโทรเอง',
    needsAction: true,
    retryable: true,
  },
};
