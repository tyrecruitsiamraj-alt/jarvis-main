 ...
 * ว่าต่างกันไหม — สองคำนี้ **นับคนละชุดจริง ๆ**:
 *   "ตอบว่าไป" = คำตอบที่ได้จากปากเขาในสาย (ยังไม่ปิดงาน)
 *   "ปิดงาน: ไป" = เจ้าหน้าที่ปิดงานแล้วว่าไปเริ่มงานจริง
 * ⇒ ให้**คำบนจอประกาศตัวเอง**ว่าเป็นคนละแกน ดีกว่าไปหวังให้คนอ่านหมายเหตุตัวเล็ก
 *
 * ⚠️ มีด่านเทสต์กันคำชนกับ `FOLLOW_CALL_CATEGORY_LABEL` อยู่ทั้งสองทิศ
 * (`tests/api/followPlanning.test.ts`) — แก้คำที่นี่ต้องรันเทสต์ชุดนั้นเสมอ
 */
...
export const FOLLOW_ROUND_BUCKET_HINT: Record<FollowRoundBucket, string> = {
  // เดิม "ทั้งหมด / ทุกคนที่อยู่รอบนี้" — สองบรรทัดอ่านเหมือนคนละเรื่อง (ตาใหม่ 12 ก.ย. 2569)
  all: 'นับทุกสถานะรวมกัน',
