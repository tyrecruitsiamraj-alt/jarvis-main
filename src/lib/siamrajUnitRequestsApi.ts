 ...
 *
 * ═══════════════════════════════════════════════════════════════════════
 * 🔴 **ใบขอล่วงหน้าใช้ id เต็มเป็นคีย์** (23 ก.ย. 2569 — เจ้าของแจ้ง
 * *"ใบขอที่เลขเหมือนกันทำไมแก้แล้วมันดันซ้ำกัน แก้ใบไหนก็ควรขึ้นแค่ของใบนั้นสิ"*)
 *
 * ใบขอจริง (`siamraj-sql:`) กับใบขอล่วงหน้า (`siamraj-pre:`) **เลขที่ใบซ้ำกันได้**
 * เพราะสองระบบใช้รูปเดียวกัน (BU+ปีเดือน+running) · วัดจริง 23 ก.ย.: เปิดอยู่
 * 279 ใบจริง + 11 ใบล่วงหน้า ชนกัน 1 เลข (`OPL6909001` — ใบจริงคือ "ลาออก/ยาสกาว่า"
 * ส่วนใบล่วงหน้าคือ "เปิดไซด์/คุณอังคาร" คนละเรื่องกันสิ้นเชิง)
 * ⇒ คีย์ด้วยเลขเปล่า = สองใบใช้แถวเดียวกัน แก้ใบหนึ่งขึ้นอีกใบ
 *
 * ⚠️ **ใบขอจริงยังใช้เลขเปล่าเหมือนเดิม** — ตั้งใจ เพราะของเก่า 1,990+ แถวเขียนไว้
 * ด้วยเลขเปล่า (และบางแถวเป็นเลขล้วนแบบไม่มี prefix ดูย่อหน้าบน) เปลี่ยนคีย์ใบจริง
 * = ของที่คนบันทึกไว้หายทั้งกอง · แยกเฉพาะใบล่วงหน้าพอ เพราะฝั่งที่ชนคือใบล่วงหน้า
 */
...
/** ขึ้นต้น id ของใบขอล่วงหน้า — ต้องตรงกับ `PREQUEST_ID_PREFIX` ฝั่ง API */
export const PREQUEST_ID_PREFIX = 'siamraj-pre:';

/** คีย์นี้เป็นของใบขอล่วงหน้าไหม — ใช้ได้ทั้งฝั่งอ่านและฝั่งเขียน */
export function isPrequestKey(key: string): boolean {
  return (key || '').trim().startsWith(PREQUEST_ID_PREFIX);
}
