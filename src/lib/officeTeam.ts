...
export type LaneCounts = {
   ...
   *
   * `done` ตอบแค่ว่า "รู้ผลแล้ว" ซึ่งเป็นกล่องตัน — รู้แล้วยังไงต่อไม่ได้บอก
   * ตรงกับที่เจ้าของทักไว้ตั้งแต่ต้นว่า *"รับสายแล้วไงอะ"*
   *
   * ⚠️ อ่านจากคำที่เขาพูดจริง ผ่านเครื่องยนต์กลาง `callMicroOutcome.ts`
   * (รหัส `acknowledged` ของ Lumos ปนทั้งไปและไม่ไป ใช้รหัสตรง ๆ ไม่ได้)
   * ⚠️ **คำของแต่ละเลนไม่เหมือนกัน** — เลนติดตามคือ ไป/ไม่ไป · เลนอื่นคือ สนใจ/ไม่สนใจ
   * ⇒ ตัวเลขใช้ชื่อกลาง (`yes`/`no`) ส่วนคำบนจอเลือกตามเลนที่ `TeamBoardPanel`
   */
  results: LaneResults | null;
};

/** ผลจริงแยกถัง — รวมทุกถัง = `done` เสมอ (ห้ามมีสายหายระหว่างทาง) */
export type LaneResults = {
  /** ตอบรับ — เลนติดตาม = ไปทำงาน · เลนอื่น = สนใจงาน */
  yes: number;
  /** ปฏิเสธ */
  no: number;
  /** ยังไม่พร้อม (ยังเตรียมตัวอยู่ / ขอคิดดูก่อน) */
  notYet: number;
  /** คุยแล้วแต่ไม่ได้คำตอบ — ต้องคนไปอ่านเอง */
  unclear: number;
  /** ไม่มีใครรับสาย */
  noPickup: number;
  /** รับแล้วไม่ใช่เจ้าตัว */
  wrongPerson: number;
  /** รับแล้วเงียบ/วางสาย */
  silent: number;
};
