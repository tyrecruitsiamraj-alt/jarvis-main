 ...
 * (อันนี้สายไม่ได้ออก ระบบส่งซ้ำให้เอง · อันนั้นออกไปแล้วรอผลจาก Lumos)
 *
...
/**
 * **คำที่คนรับสายพูดเอง** — `null` = ไม่มี (ไม่มี transcript หรือเขาไม่พูดเลย)
 *
 * 🔴 เจ้าของทัก 10 ก.ย. 2569: *"สายที่ 1 โทรสำเร็จ ... ไม่แสดงข้อความที่ตอบ"* ·
 * *"สายที่ 2 โทรแล้ว ไม่แสดงข้อความที่ตอบ"*
 *
 * ของเดิมช่อง "เขาตอบว่าอะไร" โชว์ `call_summary` ซึ่งเป็น**คำบรรยายของ AI**
 * มุมมองบุคคลที่สาม (*"ผู้รับสายแจ้งว่า…"*) ไม่ใช่คำตอบของเขา · และสายที่จบแบบ
 * `unresponsive` ไม่มี summary เลย ช่องจึงว่างทั้งที่ transcript มีคำพูดอยู่
 *
 * ⚠️ **ห้ามตัด/สรุป/เดาความหมาย** — ต่อคำที่เขาพูดตามลำดับดิบ ๆ ("ฮัลโหล · ใช่ค่ะ ·
 * ไม่ไปแล้ว รถยางแตก") การตัดว่าประโยคไหน "สำคัญ" เป็นการเดาแทนคน
 */
export function roundReplyText(round: FollowPlanningRound): string | null {
  const s = (round.entry.call_reply ?? '').trim();
  return s === '' ? null : s;
}

/**
 * แถวนี้ควรถูก**ระบายสีทั้งแถว**ไหม — `null` = ปล่อยขาว
 *
 * 🔴 เจ้าของสั่ง 7 ก.ย. 2569: *"สีต้องบอกได้ว่า เขียวคือตกลง เหลืองติดต่อไม่ได้
 * แดงคือไม่ไป"* · 8 ก.ย.: *"คนไหนไม่ไปขอสีแดงอ่อน ๆ ในช่องนั้นไปเลย"*
 * รอบนั้นทำแค่**แดง** ⇒ 10 ก.ย. เจ้าของทักว่า *"สายที่ 1 โทรสำเร็จ ไม่แสดงสีเขียว"*
 *
 * กติกา: **มีผลแล้วเท่านั้นที่ได้สี** (เขียว/เหลือง/แดง) · ยังไม่มีผลปล่อยขาว
 * ไม่งั้นทั้งตารางเป็นสีหมดแล้วสีเลิกบอกอะไร
 */
export function callCategoryWashTone(category: FollowCallCategory): ToneKey | null {
  switch (category) {
    case 'agreed':
      return 'success';
    case 'lost':
...
/**
 * 🔴🔴 **ห้ามใช้คำเดียวกับช่องใน Pipeline** — คนละนิยามกัน (แก้ 8 ก.ย. 2569)
 *
 * เจ้าของจับได้เอง: *"คำว่า ไป ด้านบนบอก 3 ด้านล่าง 0 ยังไงเนี่ย แล้วจะเชื่อถือได้หรอ"*
 *
 * เช้าวันเดียวกันผมเพิ่งยืมคำจาก `FOLLOW_ROUND_BUCKET_LABEL` มาใช้ **โดยไม่ตรวจว่านิยามตรงกันไหม**
 * แล้วเขียนเทสต์บังคับให้เหมือนกันด้วย ⇒ จอเดียวมีคำว่า "ไป" สองที่ที่นับคนละเรื่อง:
 *
 * | คำ | หมวดตรงนี้ (`callCategory`) | ช่อง Pipeline (`followRoundBuckets`) |
 * | --- | --- | --- |
 * | ไป | ผลโทรว่ายืนยันจะไป **หรือ** ปิดงานว่าไป | **เฉพาะ** ปิดงานว่าไป (`outcome_code`) |
 * | ไม่ไป | ตอบว่าไม่ไป **หรือ** ปิดงานว่าไม่ไป | เฉพาะปิดงานว่าไม่ไป |
 * | โทรไม่ติด | รวม `reschedule_requested` + `wrong_person` | ไม่รวม (สองอันนั้นอยู่ถัง "โทรติด") |
 * | รอโทร | ส่งแล้วรอผล **+** ยังไม่ถึงเวลา | เฉพาะที่ยังไม่ถูกดึงไปโทร (`delivered` = กำลังโทร) |
 *
 * ⇒ หมวดตรงนี้พูดถึง **"คำตอบที่ได้จากสาย"** จึงใช้คำที่บอกอย่างนั้นตรง ๆ และ
 * **ต้องไม่ซ้ำกับคำของ Pipeline** ซึ่งพูดถึง *สถานะของสาย* กับ *การปิดงาน*
 * มีด่านเทสต์บังคับความต่างที่ `tests/api/followPlanning.test.ts`
 */
