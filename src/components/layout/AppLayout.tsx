...
const AppLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const themeSwitchCompact = (
    /* 🔴 ปุ่มไอคอนบนแถบหัวใช้ Button ของ shadcn ทรง icon — ทั้งแถบจะได้ขนาดเท่ากันหมด
       (4 ก.ย. 2569 · เจ้าของทักเรื่องปุ่มไอคอน) */
    <Button
      type="button"
      variant="ghost"
      size="icon"
    ...
    >
      {theme === 'dark' ? <Sun className="text-amber-500" /> : <Moon className="text-sky-600" />}
    </Button>
  );
            ...
            {showSettings ? (
              <Button
                ...
                className={cn(
                  ...
                  location.pathname.startsWith('/settings') && 'border-blue-300 bg-blue-500/12 text-blue-700',
                )}
              >
