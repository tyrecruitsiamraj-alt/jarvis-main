...
## รอบ 130 — ปฏิทินติดตามแบ่งเป็น 2 หน้า (7 ก.ย. 2569)

**เจ้าของสั่ง:** *"ปฏิทินติดตาม แบ่งเป็น 2 หน้า หน้าแรกเพื่อดูว่าต้องมีกี่คนที่ต้องโทร
ต้องตามผลไรงี้ อีกหน้าเป็นหน้าสรุปเลยว่าทั้งเดือนคนไหนถูกแท็กให้โทรวันไหนบ้างแล้วผลเป็นไง"*

ตาราง 30 คอลัมน์ตอบ "ภาพรวมทั้งเดือน" ได้ แต่ตอบ **"วันนี้ต้องทำอะไร"** ไม่ได้
(ต้องกวาดสายตาหา 30 ช่องต่อคน)

**หน้า 1 "ต้องลงมือ" (ค่าตั้งต้น)** — ตัดคอลัมน์วันทิ้ง เหลือเฉพาะคนที่มีงานค้างจริง
- หัวสรุป 4 ตัว: ต้องลงมือ N คน · เลยเวลานัดยังไม่มีผล N คน · มีนัดวันนี้ N คน ·
  สายที่ต้องตามผล N สาย · **พร้อมป้ายกำกับว่าสองช่องกลางซ้อนกันได้ ห้ามบวก**
  (กติกาป้ายกันบวกเลขนับซ้อน เหมือนหน้าแรก)
- แต่ละคนกางทุกสายพร้อมผล+เหตุผล กดแล้วเปิดป๊อปจัดการสายนั้นได้เลย
- ไม่มีใครค้าง → ข้อความเขียว "ไม่มีใครค้างเลย" (ไม่ใช่ตารางว่าง)
- **ของค้างขึ้นก่อนเสมอ** แล้วเรียงตาม `dueAtMs` ที่ชั้นสร้างแถวคิดไว้แล้ว (ไม่คิดใหม่)

**หน้า 2 "สรุปทั้งเดือน"** — ตารางเดิมทั้งหมด (ชื่อ + คอลัมน์รายสาย + 30 วัน) ·
ปุ่มเดือนก่อน/ถัดไป/วันนี้ และป้าย "กำลังดูรอบที่ N" โผล่เฉพาะหน้านี้

**นิยาม** `buildFollowActionRows()` + `followActionSummary()` ใน `followPlanning.ts`
🔴 ใช้ `followRoundState` ตัวเดียวกับที่เหลือของหน้า — `overdue` = เลยเวลา**และยังไม่มีผล**
ตรงกับ `followScheduleCounts` ที่หน้าแรกใช้ · แยกเขียนเมื่อไหร่เลขสองหน้าจะเถียงกัน
⚠️ คิดจาก `rows` ตรง ๆ **ไม่ผูกกับเดือนที่กำลังดู** — ของค้างข้ามเดือนต้องไม่หาย

**เทสต์** `FollowPlanningCalendar.test.tsx` (+4 · รวมไฟล์นี้เป็น 12): ค่าตั้งต้นเป็นหน้า
"ต้องลงมือ" และยังไม่โชว์ตาราง 30 วัน · หัวสรุปนับคนถูก · **คนที่ปิดงานแล้วต้องไม่โผล่**
(ไม่งั้นลิสต์ไม่มีวันว่าง) · สลับไปหน้าเดือนแล้วตารางกลับมาครบ
⚠️ เทสต์เดิม 8 เคสของไฟล์นี้ต้องเรียก `showMonthView()` ก่อน เพราะค่าตั้งต้นเปลี่ยน

### คำตอบเรื่องเบอร์ฉุกเฉิน (เจ้าของถามซ้ำ — ยืนยันจาก 2 แหล่ง)

ตรวจ **สเปกทางการ** `docs/lumos-api.md:355-428` (ตาราง Reminder Result Object Schema
ครบทุกฟิลด์) และ **ข้อมูลจริงบน production** (7 แถวที่มีผล) — ตรงกันว่า
**ไม่มีฟิลด์ใดบอกว่าโทร `admin_phone` แล้วหรือยัง** · ที่มีคือ `plan_status` `ended_reason`
`call_attempts` `stop_early` ซึ่งทั้งหมดพูดถึง**สายที่โทรหาผู้รับ** ไม่ใช่สายฉุกเฉิน
⇒ จอพูดได้แค่ "แนบไปแล้ว / ยังไม่รู้ว่าโทรหรือยัง" ตามรอบ 129 · ต้องขอ Lumos เพิ่มฟิลด์
