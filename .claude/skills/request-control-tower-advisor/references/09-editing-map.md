...
2. **ใบที่มาก่อนได้ก่อน** — ในบรรดาแถวที่ "ถึงคิว" ของเบอร์เดียวกัน เสิร์ฟแถวแรก
   ตาม `(created_at, id)` แถวเดียว
...
⚠️ เทียบเป็น row comparison `(e.created_at, e.id) < (c.created_at, c.id)` —
...
→ เสิร์ฟจริง 126 แถว = 126 คน** (เฉลี่ยคนละ ~22 ใบขอ) · คิวรี 19 ms ไม่ต้องมี index
