...
### รอบแปดสิบห้า — Admin ดูมุมมองของ role อื่นได้ (2 ก.ย. 2569)

เจ้าของสั่ง: *"ทำให้ Admin ดูมุมมองของ Role อื่นได้ จะได้ตรวจสอบว่าที่แก้ไปได้ตามนั้นไหม"*
+ ทักระหว่างทำ: *"ทำไมลอง Login แล้วพาไปเลือกแผนก … จะเห็นแผนกไหนก็ค่อยไปเลือก
ตอน Admin เลือกมุมมองเอา"*

| ไฟล์ | หน้าที่ |
|---|---|
| `src/contexts/AuthContext.tsx` | `viewAsRole` + `viewAsDepartment` (sessionStorage) · สวมให้ `effectiveUser` **ที่เดียว** ทุกจุดที่อ่าน `user.role` เปลี่ยนตาม · `realRole` = ตัวจริงเสมอ |
| `src/components/layout/ViewAsRoleBanner.tsx` | **ใหม่** — แถบเหลือง + dropdown แผนก + ปุ่มออก |
| `src/App.tsx` | แถบอยู่ **นอก AppLayout/RequireRole** — สวมแล้วโดนเด้งไปหน้าไหนก็ยังออกได้ |
| `src/pages/settings/RolePermissionsTab.tsx` | ปุ่มสลับ opl/staff/supervisor ในแท็บบทบาทและสิทธิ์ |
| `src/components/auth/RequireDepartment.tsx` | ตอนสวมมุมมอง **ไม่บังคับเลือกแผนก** |

🔴 **กติกาที่ห้ามลืม:**
1. **เป็นการจำลองหน้าจอ ไม่ใช่สวมสิทธิ์จริง** — โทเคนยังเป็น admin · กดปุ่มที่ role นั้น
   ไม่ควรกด เซิร์ฟเวอร์ยอมให้ผ่าน ⇒ แถบเตือนเขียนไว้ตรง ๆ ตลอดเวลา
2. **หน้า "เลือกแผนกก่อนใช้งาน" ต้องไม่เด้งตอนสวมมุมมอง** — บัญชี admin ไม่มีแผนก
   สวมเป็น staff แล้วเด้งไปหน้านั้น = ทางตัน (อยู่นอก layout ไม่มีปุ่มออก) และเสี่ยง
   เผลอตั้งแผนกให้บัญชีจริง ⇒ เลือกแผนกจาก dropdown บนแถบเหลืองแทน
3. **dropdown แผนกอยู่บนแถบ ไม่ใช่ในหน้าตั้งค่า** — สวมเป็น role อื่นแล้วหน้าตั้งค่า
   เข้าไม่ได้ วางไว้ที่นั่นคือเลือกทีหลังไม่ได้
4. sessionStorage (ไม่ใช่ localStorage) — ปิดแท็บแล้วหลุดเอง ไม่ค้างข้ามวัน

**ด่านตรวจ:** test 2,583 ผ่าน / 6 skip · tsc 4 = 0 · eslint 0 err / 19 warn · build ผ่าน
**ตรวจบนจอจริงแล้ว** (login กลับเข้าได้ผ่าน dev-role ซึ่งเปิดใน .env.local อยู่แล้ว):
สวม staff → เด้งไป /staff พร้อมแถบเหลือง+ปุ่มออก · ไม่เจอหน้าเลือกแผนก ·
เลือกแผนก LBD จากแถบได้ · กดออก → กลับเป็น admin แถบหาย session ล้าง


### รอบแปดสิบหก — กวาดบั๊ก + Haiku ทดสอบทั้งระบบ (2 ก.ย. 2569)

เจ้าของสั่ง: *"เช็คบั๊กให้หมดเลย UI กดแล้ว UX ไม่ทำงาน หรือกดแล้วเกิดเพี้ยน …
เช็คเสร็จแล้วแก้แล้วให้เอาโมเดลที่อ่อนที่สุดมาทดลองใช้งานทั้งระบบ"*

**กวาดเอง:** 18 route + 14 แท็บตั้งค่า + 5 แท็บกล่องงาน + ป๊อปไล่งาน — เปิดได้หมด
· เจอบั๊กจริง 1: **แท็บตั้งค่าไม่ตาม URL** (แก้แล้ว รอบก่อนหน้า `7512535`)

**Haiku สวมบทพนักงานใหม่ (5 ปลอดภัย: ห้ามกดปุ่มเขียน):** เฉลี่ย 4.6/10
· กล่องงานดีสุด 7/10 · จับคู่งานกับแดชบอร์ดต่ำสุด 3/10

**ผลที่ทานกับจอจริงแล้ว:**
* ✅ แก้ทันที: หน้าดูแลหลังเริ่มงาน — ข้อความเหลือง "ยังไม่ได้ตั้งสาย" ขึ้นกับรอบ**อนาคต**ด้วย
  ขัดกับ legend และขัดกับ "เลยรอบที่ควรโทร 0 คน" บนหัว ⇒ เหลืองเฉพาะเลยกำหนด
  รอบอนาคตเป็นสีจาง "ยังไม่ถึงกำหนด"
* ❌ ไม่ใช่ data bug: แดชบอร์ด "หาย 350 อัตรา" (7,661−5,324−1,987) — คือ **เหลือหา**
  ตามสมการหลัก แค่จอไม่พิมพ์บรรทัดนั้น (การสื่อสาร รอเจ้าของเคาะ)
* 📌 จุดงงที่เหลือ = หนี้ UX เดิมที่จดไว้แล้ว (audit 29 ส.ค.): ช่องเลขไม่บวกกันหน้า
  จับคู่งาน · ปุ่ม "ส่งประกาศทีเดียว" ไม่บอกผล · ปุ่มการ์ดใบขอหลายคำ (ส่งคนแทน/ดำเนินการ)
  · "การโทรครั้งที่ 1/2/3" ไม่มีคำอธิบาย — **รอเจ้าของจัดลำดับ**


### รอบแปดสิบเจ็ด — เลือกช่องทางแบบ หลัก → รอง · 1:1 (2 ก.ย. 2569)

เจ้าของทัก: *"ตอน Gen link มันดูงง มันจะเลือกช่องทางหลัก ช่องทางรองยังไง"*
แล้วเคาะ: *"กดช่องทางหลัก แล้วพาไปเลือกช่องทางรองต่อ · เลือกได้หลายอันไม่เอา เอาแบบ 1:1"*

| ไฟล์ | หน้าที่ |
|---|---|
| `src/components/shared/ChannelPicker.tsx` | **เขียนใหม่** — drill-down หลัก→รอง · เลือกได้ 1 ช่อง (ถอด prop `multiple` ทิ้งทั้งตัว) |
| `src/components/jobs/GenApplyLinkDialog.tsx` | `picked` เป็น object เดี่ยว · หัวหุบโชว์ชื่อช่องที่เลือกแทน "เลือกไว้ N ช่อง" |
| `src/components/recruit-rm/AddApplicantDialog.tsx` | `channel` เป็น object เดี่ยว (เดิมก็ใช้ `channel[0]` อยู่แล้ว — ตรงความจริงขึ้น) |

**เดินทางใหม่:** หลัก 43 ตัว (ตัวที่มีลูกติดป้าย "มีช่องรอง N ›") → กดแล้วกางลูกของตัวนั้น
(ค้นในพ่อ + ปุ่มโหลดเพิ่มทีละ 50 — Facebook Group ตัวเดียวมีลูก 2,776) → กดลูก = จบ
· พ่อไม่มีลูก กดแล้วเลือกเลย · ในหน้าลูกมีปุ่ม **"ใช้ตัวหลักโดยไม่ระบุช่องรอง"** เสมอ
· ค้นหน้าแรก = ค้นข้ามทุกชั้นเหมือนเดิม · **เลือกแล้วซ่อนลิสต์ทันที** เหลือ chip เดียว + กด X เปลี่ยน

⚠️ เส้นข้อมูลใช้ของเดิมทั้งหมด (`fetchRecruitChannelRoots` คืน childCount อยู่แล้ว ·
`fetchRecruitChannelChildren` ค้น+แบ่งหน้าได้อยู่แล้ว) — ไม่แตะ API
⚠️ ผลข้างเคียงที่ตั้งใจ: สร้างลิงก์ได้ทีละช่อง — อยากส่งหลายช่องต้องสร้างหลายใบ
(จอบอกไว้แล้ว: "1 ลิงก์ต่อ 1 ช่องทาง — อยากส่งช่องอื่นด้วย ให้สร้างลิงก์ใหม่อีกใบ")

**ด่านตรวจ:** test 2,583 ผ่าน / 6 skip · tsc 4 = 0 · eslint 0 err / 18 warn · build ผ่าน
**ตรวจบนจอจริงแล้ว (ไม่ได้กดสร้างจริง):** กด Facebook Group (มีช่องรอง 2,776) → เข้าหน้า
ลูกพร้อมปุ่มกลับ+ปุ่มใช้ตัวหลัก · ค้น "ชลบุรี" เจอ 50 · กดเลือก → เหลือ chip เดียว
หัวหุบขึ้น "— Facebook Group · กลุ่มคนทำงาน…" · กด X กลับมาหน้าเลือกได้


### รอบแปดสิบแปด — แก้จุดงง 5 ข้อจากผล Haiku (เจ้าของเคาะ "ok ลุย" · 2 ก.ย. 2569)

| ข้อ | ไฟล์ | ทำอะไร |
|---|---|---|
| 3 ป๊อปยืนยันส่งชุด | `JobBoardView.tsx` | ปุ่ม "ส่งประกาศทีเดียว" กดแล้ว**ยังไม่ยิง** — เปิด AlertDialog บอกกี่ใบ ใครเห็น เกิน 300 ทำไง + ปุ่ม "ยังก่อน" · เลขในป๊อปใช้ `releasableJobs` ชุดเดียวกับปุ่มเป๊ะ |
| 1 จับคู่งาน 2 กลุ่ม | `aiCallFlowCells.ts` (+`group`) · `AiCallFlowPanel.tsx` | 8 ช่องแบนจัดเป็นสองกลุ่มติดหัว: **"สายอยู่ตรงไหน"** (ทั้งหมด·กำลังโทร·รอ AI โทรใหม่) กับ **"คุยแล้วผลเป็นยังไง"** (5 ช่องผล) + บรรทัดกันคนบวกเลขเอง · เทสต์คุมสังกัดกลุ่ม |
| 2 แดชบอร์ด | `DashboardShell.tsx` · `DashboardFilterBar.tsx` | สมการ เข้ามา−ปิด−ยกเลิก = **ยังเปิดหาอยู่ N** ขึ้นเสมอ (เดิมขึ้นเฉพาะตอนเลือกช่วงวัน — โหมด default คือโหมดที่คนเจอ "หาย 350") + อธิบายว่า "คงเหลือ" ฐานคนละช่วง · หัวแผงตัวกรองบอกขอบเขต: ก้อนไหนเปลี่ยนเลขบนหน้า ก้อนไหนกรองแค่ CSV |
| 5 คำเดียวทั้งระบบ | `followRoundVisual.ts` · `FollowPlanningCalendar.tsx` · `CommandDeck.tsx` | "การโทรครั้งที่ N" → **"สายที่ N"** (คำเดียวกับ dropdown ตอนตั้งรอบ — คำที่คนเลือกเองชนะ) · เชิงอรรถหน้าแรกเลิกใช้ประโยคปฏิเสธซ้อน |
| 4 ภาษาสถานะ | `UnitRequestReplacementBadge/Toggle.tsx` · `jobUrgency.ts` · `JobListPage` · `UnitRequestFilterFields` · `JobDetailPage` | ป้าย/ตัวเลือก "ส่งคนแทน" → **"ต้องส่งคนแทน / ไม่ต้องส่งคนแทน"** (ภาษาสถานะ ไม่ใช่กริยาคำสั่ง — Haiku อ่าน pill เป็นปุ่มแล้วไม่กล้ากด) · ตัวกรอง "ดำเนินการ" → "ยังดำเนินการอยู่" / "กำลังดำเนินการ" |

⚠️ ค่าที่เก็บในฐาน (`send_replacement` · status `active`/`in_progress`) **ไม่เปลี่ยน** — เปลี่ยนแต่คำบนจอ

**ด่านตรวจ:** test 2,584 ผ่าน / 6 skip · tsc 4 = 0 · eslint 0 err / 18 warn · build ผ่าน
**นัดวัดผล:** แก้ครบแล้วส่ง Haiku ชุดใหม่ทดลองซ้ำโจทย์เดิมเป๊ะ — เป้า ≥7/10 (จาก 4.6)


### รอบแปดสิบเก้า — Haiku วัดผลรอบสอง + เก็บตก 3 จุด (2 ก.ย. 2569)

**ผลรอบสอง (โจทย์เดิมเป๊ะ): เฉลี่ย 4.6 → 6.6** · แดชบอร์ด 3→9 · หน้าแรก 4→8 ·
ใบขอ 5→7 · aftercare 6→7 · ติดตาม 5→6 · จับคู่ 3→4 · **กล่องงาน 7→5 (ตก)**

**เก็บตกที่แก้ตามผลรอบสอง:**

| จุด | ไฟล์ | ทำอะไร |
|---|---|---|
| จับคู่: ยังบวก "กำลังโทร+รอใหม่" เทียบ "ทั้งหมด" | `AiCallFlowPanel.tsx` | บรรทัดกันบวกเขียนใหม่ให้ครอบ**ทุก**ช่อง (ตัวอย่างเดิมพูดแต่ฝั่งผล เลยเข้าใจว่าฝั่ง "สายอยู่ตรงไหน" ต้องบวกลงตัว) + นิยาม "ทั้งหมด = นับหัวละครั้ง" |
| กล่องงาน: 156−127 = "29 ใบนี้คือใบไหน" | `JobBoardView.tsx` | ส่วนต่างพิมพ์**ใต้ปุ่ม**เลย: "ไม่รวม N ใบที่มีคนเริ่มงานแล้ว" — เดิมอยู่แค่ใน hover ซึ่งจอสัมผัส/คนกวาดตาไม่มีวันเจอ |
| ติดตาม: "สายที่ห่างกันกี่วัน" + "สำเร็จ 0 แต่โทรครบ 1" | `FollowCallRoundsPanel.tsx` · `FollowCompletedPanel.tsx` | hint ใหม่: สายที่ = ลำดับที่คนเลือกตอนเพิ่ม ไม่มีระยะห่างตายตัว · กล่องโทรครบเติม "โทรครบ ≠ สำเร็จ — สำเร็จ = ปิดงานว่าไปทำงานแล้ว" |

📌 **บทเรียน tooltip:** คำอธิบายที่อยู่ใน `title=` อย่างเดียว = ไม่มีอยู่จริงสำหรับคนส่วนใหญ่
ข้อมูลที่ตัดสินความเชื่อใจ (เลขไม่ตรงกันเพราะอะไร) ต้องพิมพ์บนจอ

**ด่านตรวจ:** test 2,584 ผ่าน / 6 skip · build ผ่าน · ตรวจบนจอครบสามจุด


### รอบเก้าสิบ — ชื่อหน่วยงาน: เอาชื่อจุดทำงานชื่อเดียว (เจ้าของจับได้เอง 3 ก.ย. 2569)

**เคสที่จุดชนวน:** ใบขอ `OPL6908110` ไซต์ `69LBDL0044` จอขึ้น
*"บริษัท สมิติเวช ศรีราชา จำกัด (สำนักงานใหญ่)"* แต่คนต้องไปทำงานที่ **สมิติเวช ชลบุรี**
(รพ.สาขาชลบุรีจดทะเบียนใต้บริษัทศรีราชา) ⇒ ชื่อนิติบุคคลตอบว่า *ใครจ่ายเงิน* ไม่ได้ตอบว่า *ไปที่ไหน*

ERP มีสองช่องแยกกันอยู่แล้ว — เราหยิบมาโชว์ผิดช่อง:
* `ms_site.site_name` = จุดทำงาน + ตำแหน่ง/จำนวน (`"สมิติเวช ชลบุรี - พขร. (Valet Parking) 4 คน"`)
* `st_site_contract_p1.customer_name` = นิติบุคคลคู่สัญญา

🔴 **สองข้อที่ห้ามทำ (วัดจริงก่อนลงมือ):**
1. **ห้ามสลับค่า `unit_name` เฉย ๆ** — สำรวจ 414 สัญญาที่มีใบขอปี 69 พบชื่อจุดทำงาน
   **240 จาก 414 เป็นชื่อย่ออังกฤษ** (`krungsri` `SCB` `TDEM` `MEGA`) เอาขึ้นเดี่ยว ๆ คนนอกอ่านไม่รู้เรื่อง
2. **`unit_name` เป็นกุญแจแมตช์** — `jobPenalty.countPenaltyDays()` แมตช์กับ `client_name` ของ WL
   ที่บันทึกไว้แล้วในอดีต (`AssignDialog` ก็หยิบค่านี้ไปเก็บ) เปลี่ยนค่า = วันขาดคนย้อนหลังเพี้ยนเงียบ ๆ

⇒ วิธีที่เลือก: **เพิ่มช่องใหม่ ไม่แตะค่าเดิม**

| ไฟล์ | ทำอะไร |
|---|---|
| `api/_lib/siamrajUnitName.ts` (ใหม่) | `sitePlaceName()` ตัดหางตำแหน่ง/จำนวนคน (คั่นด้วย `" - "` มีเว้นวรรคสองข้าง — `Asian-HD` จึงไม่ถูกหั่น) · `workSiteNameOf()` คืน `null` เมื่อชื่อซ้ำ/ไม่มีไซต์ |
| `siamrajSqlServerRequests.ts` · `siamrajSqlServerClosed.ts` · `siamrajSqlServerUnits.ts` | เพิ่ม `work_site_name` / `workSiteName` — **`unit_name` เดิมไม่ขยับ** |
| `src/lib/unitDisplay.ts` (ใหม่) | `unitLabel` = ชื่อจุดทำงาน (ไม่มีก็ถอยไปชื่อคู่สัญญา) · `unitOneLine` = ตัวเดียวกัน ไว้ให้จุดที่ต่อสตริงเองอ่านง่าย |
| `unitRequestDisplay.jobBoardCardTitle()` | หัวการ์ดใช้ `unitLabel` — **จุดคอขวดของทั้งบอร์ด + ตัวเลือกหน่วยงานงานติดตาม** |
| `JobListPage` · `JobBoardView` · `JobDetailPage` · `JobDashboard` · `PreCheckPage` · `MatchingPage` · `EditPublicJobFieldsDialog` · `UnitRequestInfoFields` | ทุกจอขึ้น**ชื่อจุดทำงานชื่อเดียว** · ช่องค้นหาใบขอค้นชื่อจุดทำงานได้ด้วย |

🔴 **รอบแรกทำเป็นสองบรรทัด (จุดทำงาน + "คู่สัญญา …") เจ้าของสั่งเอาออกทันที:**
*"เอาแค่ สมิติเวช ชลบุรี มา ขึ้นคู่สัญญาไม่ต้องเดี๋ยวงง"* ⇒ **ห้ามเอาชื่อนิติบุคคล
กลับมาต่อท้ายอีก** (มีเทสต์ `ห้ามมีชื่อคู่สัญญาปนมาด้วย` คุมไว้) · ชื่อคู่สัญญายังอยู่ใน ERP
ตามเดิม ใครต้องใช้ไปดูที่ ERP · ผลข้างเคียงที่รู้อยู่: 240 จาก 414 ไซต์ตั้งชื่อจุดทำงาน
เป็นชื่อย่ออังกฤษ (`krungsri` `SCB` `MEGA`) จอจะขึ้นชื่อย่อนั้น — เจ้าของรับทราบแล้วเลือกแบบนี้

⚠️ **กระทบของจริง:** ตัวเลือกหน่วยงานของงานติดตามเก็บ `follow_entries.unit_name` ที่ AI พูดออกเสียง
ตอนโทร (*"ไปทำงาน หน่วยงาน…"*) ⇒ ตั้งแต่นี้ AI จะพูด **"สมิติเวช ชลบุรี"** แทน
**"บริษัท สมิติเวช ศรีราชา จำกัด (สำนักงานใหญ่)"** — ถูกกว่าเดิม (รอบที่ตั้งไว้ก่อนหน้านี้ยังพูดค่าเก่า)

**ด่านตรวจ:** เทสต์ใหม่ `tests/api/siamrajUnitName.test.ts` 12 ข้อ (มีเคสจริง 69LBDL0044) ·
รวม 2,605 ผ่าน · tsc 4 = 0 · eslint 0 err · build ผ่าน · ตรวจบนจอ: รายการใบขอ (ขึ้น
"สมิติเวช ชลบุรี" ไม่มีคำว่าคู่สัญญา/ศรีราชาแล้ว) · การ์ดกล่องงาน · หน้ารายละเอียด (แถว "ชื่อหน่วยงาน")


### รอบเก้าสิบเอ็ด — แผง "Rate ผลการโทร Lumos" บนแดชบอร์ด (3 ก.ย. 2569)

เจ้าของสั่ง: *"ต้องการดู Rate ว่าที่เราส่งไปให้ Lumos ผลเป็นยังไงบ้าง ส่งไปเท่าไหร่
ปฏิเสธเท่าไหร่ สำเร็จเท่าไหร่ ปริมาณการโทรโตขึ้นหรือลดลง แล้วผลสำเร็จดีขึ้นหรือลดลง"*
⇒ แผงพับได้ต่อท้ายคอลัมน์ขวาของหน้าแดชบอร์ด (ทรงเดียวกับแผงอื่นในหน้า · เปิดค้างเป็นค่าเริ่มต้น)

| ไฟล์ | หน้าที่ |
|---|---|
| `api/_handlers/lumos-call-funnel.ts` | เพิ่ม `?series=day&days=N` (ควบ 1-120) — ยอดรายวันนับตามวันที่ส่ง โซนไทย · นิยามผลจาก `_lib/lumosQueueDefs` (`queueOutcome`/`queueCancelled`) ไม่เขียนใหม่ |
| `src/lib/lumosCallRate.ts` | **ใหม่ · pure** — สรุปช่วง + เทียบ "N วันล่าสุด vs N วันก่อนหน้า" (เทสต์ 9 ที่ `tests/api/lumosCallRate.test.ts`) |
| `src/lib/callFunnelApi.ts` | `fetchCallRateSeries()` — โหลดพลาดคืน **null ไม่ใช่ []** (จอต้องแยก "อ่านไม่ได้" จาก "ไม่มีสาย") |
| `src/components/dashboard/LumosCallRatePanel.tsx` | **ใหม่** — การ์ด 6 ถัง + 2 บรรทัดแนวโน้ม + แท่งรายวันสองช่วง (ช่วงก่อนจาง มีเส้นประคั่น) · สลับต้นทาง/ช่วง 7·14·30 วัน |
| `DashboardShell.tsx` · `SupervisorDashboard.tsx` | prop `extraPanels` ต่อท้ายคอลัมน์ขวา — แผงจัดการข้อมูลเอง ไม่โดน filter/ช่วงวันของหน้า |

**นิยามที่เคาะ (เขียนบนจอทั้งหมด ไม่ซ่อน tooltip):**
* **ส่งไป = queued − cancelled** (สายยกเลิกไม่เคยถูกโทร โชว์แยก) — กติกาแม่ห้ามนับยกเลิก
* **สำเร็จ = confirmed เท่านั้น** — acknowledged (แค่รับสาย) ไม่นับเป็นสำเร็จ · "โทรติด" เป็นคนละชั้น (connected รวมปฏิเสธ)
* **ปฏิเสธ = declined** เขียนกำกับสองบริบท "ไม่สนใจ (งานเสนอ) / ไม่ไปแล้ว (งานติดตาม)"
* % ทุกตัวฐาน = สายที่มีผลจริงหักยกเลิก (หลัก `callFunnelMath`) · ไม่มีฐาน = null → จอเขียน "เทียบไม่ได้" **ห้ามโชว์ 0% หรือเดาทิศ**
* ซีรีส์ผูกผลกับ**วันที่ส่ง** (cohort) ไม่ใช่วันที่ผลกลับ — จอมีบรรทัดกำกับ

**ด่านตรวจ:** เทสต์ใหม่ 9 · เต็มชุดผ่าน · tsc 2 config = 0 · eslint 0 err / 18 warn เดิม · build ผ่าน
**ตรวจบนจอจริงทั้งสองธีม + ทานเลขกับฐานตรง 3 ชุด:** 7 วัน (ส่ง 37 · สำเร็จ 29% · ช่วงก่อนว่าง →
"คิด % ไม่ได้"/"เทียบไม่ได้") · 14 วัน (ลดลง −7% · สำเร็จดีขึ้น +24 จุด: 5%→29%) ·
กรอง Job Offer (ช่วงก่อน 24 — ตรง SQL เป๊ะ)

⚠️ ระหว่างรอบนี้มี session ขนานทำรอบเก้าสิบ (ชื่อหน่วยงาน) แล้ว commit `1bf64a8`
**กวาดไฟล์ของรอบนี้ติดไปด้วยทั้งชุด** (เวอร์ชันสุดท้ายที่ตรวจแล้วพอดี — ทานแล้วไม่มีอะไรหาย)
พร้อมไฟล์ชั่วคราว `scripts/__tmp_mssql.mjs` ของสายนั้น (ไม่มี secret ดิบ — อ่าน .env ตอนรัน)
บทเรียนซ้ำของ [jarvis-parallel-session-merges]: **ก่อน commit ต้อง `git status` ดูว่ามีไฟล์แปลกปนไหม**


### รอบเก้าสิบสอง — "ติดตรงไหน" บนแผง Rate โทร: ระบบห้ามเงียบ (3 ก.ย. 2569)

เจ้าของสั่งต่อจากรอบก่อน: *"ฉันไม่ต้องการให้ระบบเงียบ และต้องการรู้ว่าส่งไปเท่าไหร่
โทรแล้วเป็นไง รับแล้วสำเร็จไหม หรือติดตรงไหน"*

🔴 **ไล่ฐานตอนนั้นแล้วเจอของจริง — 16 สายไม่มีผลกลับ และสองขั้นคนละเรื่อง:**
* **13 สาย `status='pending'` ไม่เคยถึงมือ Lumos เลย** (สร้าง 2 ก.ย. 16:16–16:42 · ค้าง 22 ชม.
  · `next_attempt_at` เลยเวลามาแล้วทั้งหมด) — **ฝั่งเราค้าง ตัวส่งไม่ได้หยิบไป**
* **3 สาย `delivered` ตั้งแต่ 2 ก.ย. 10:07 เงียบ 28 ชม.** — ฝั่ง Lumos ไม่ส่งผลกลับ
* ของเดิมทั้ง 16 กองอยู่ในกล่องเทา "รอผลกลับ" กล่องเดียว ⇒ จอไม่ได้โกหก แต่**เงียบ**
  ซึ่งคนอ่านว่า "ปกติ" (บทเรียนเดียวกับ `_lib/lumosQueueDefs` รอบก่อน ๆ)

| ไฟล์ | หน้าที่ |
|---|---|
| `api/_handlers/lumos-call-funnel.ts` | `loadStuck()` — แยก `notDelivered` (`queuePending`) กับ `deliveredSilent` (`queueWaiting`) + อายุตัวที่ค้างนานสุด · คืนพร้อม `?series=day` |
| `src/lib/lumosCallRate.ts` | `stuckLevel()` + เกณฑ์ `STUCK_WARN_HOURS=2` / `STUCK_ALERT_HOURS=14` + `ageText()` (เทสต์เพิ่ม 4 → รวม 13) |
| `src/components/dashboard/LumosCallRatePanel.tsx` | `StuckStrip` บนสุดของแผง + งานค้างโผล่บนหัวแผงตอนหุบ (`🚩 ค้างไม่มีผลกลับ N สาย`) |

**กติกาที่ติดมากับก้อนนี้:**
1. 🔴 **แถบนี้พูดทุกสภาพ ห้ามหาย** — ไม่มีค้าง = เขียนว่า "ไม่มีสายค้าง" (เขียว) ·
   อ่านไม่ได้ = "อ่านสถานะสายค้างไม่ได้ **ไม่ได้แปลว่าไม่มีสายค้าง**" (ห้ามเงียบ ห้ามแปลเป็น 0)
2. 🔴 **งานค้างไม่ผูกกับช่วงเวลาที่เลือกบนจอ** — สายที่ค้างมาสามอาทิตย์ยังเป็นงานค้างวันนี้
   ถ้ากรองด้วยช่วง 7 วัน ของเก่าจะหายจากจอเงียบ ๆ
3. **เกณฑ์ 14 ชม. ไม่ใช่ 4** เพราะห้ามโทร 20:00–08:00 (`shiftOutOfQuietHours`) —
   สายที่ตั้งไว้ตอนเย็นค้างข้ามคืนเป็นเรื่องปกติ จอเขียนเหตุนี้กำกับไว้ด้วย
4. แยกป้ายว่า**ใครต้องแก้**: "ฝั่งเราต้องดู" vs "ฝั่ง Lumos ต้องดู" — คนละคนแก้ ห้ามยุบเป็นคำเดียว

**ด่านตรวจ:** test 2,609 ผ่าน / 6 skip · tsc 2 config = 0 · eslint 0 err / 18 warn เดิม · build ผ่าน
**ตรวจบนจอจริงสองธีม:** แถบแดงขึ้น "ยังไม่มีผลกลับ 16 สาย · ค้างข้ามคืนแล้ว" แยก 13 / 3
พร้อมอายุ 22 ชม. / 28 ชม. — **ทานกับฐานตรงเป๊ะทุกตัว** · หัวแผงตอนหุบขึ้นธงค้าง 16 สาย

⚠️ **ยังไม่ได้แก้ต้นเหตุ** ของ 13 สายที่ไม่ถูกส่งออก (เป็นงานคนละก้อน — ต้องดูตัวส่ง/worker)
จอตอนนี้แค่ทำให้เห็น ไม่ได้ทำให้หาย


### รอบเก้าสิบสอง — ปุ่ม "ลบทิ้ง" หน้าติดตาม (admin เท่านั้น) + ล้างข้อมูลทดลอง (3 ก.ย. 2569)

เจ้าของสั่ง: *"หน้าการติดตาม ทำให้ฉันลบได้หน่อย เฉพาะฉันนะเพราะตอนนี้ทดสอบอยู่
และล้างคิวออกให้หมด เหลือไว้ 1 ให้ฉันทดสอบลบเอง"*

| ไฟล์ | ทำอะไร |
|---|---|
| `api/_handlers/follow.ts` | `DELETE /api/follow?id=…&**purge=1**` → `purgeFollow()` ลบจริง · เช็ค `req.user.role !== 'admin'` ⇒ 403 (เส้น `follow` เปิดถึง staff จึงต้องกันในฟังก์ชันเอง) · ลบแถว `lumos_dispatch_queue` ที่ `person_ref = 'follow-<id>'` **แบบตรงตัวเป๊ะ ห้าม LIKE** · จด audit `follow.purge` เก็บค่าเดิมใน `before` ก่อนลบ |
| `src/lib/followApi.ts` | `purgeFollowEntry()` คืนจำนวนสายที่ยกออกจากคิว |
| `FollowRoundsDialog.tsx` | ปุ่ม "ลบทิ้ง" อยู่**ใต้เส้นประ แยกแถวจากปุ่มทำงานปกติ** กันกดพลาด · กดแล้วถามซ้ำ "ลบแล้วกู้ไม่ได้ ลบเลยไหม" → `ลบเลย` / `ไม่` · โชว์ทุกสถานะ (แถวขยะส่วนใหญ่คือแถวที่ยกเลิก/ปิดแล้วซึ่งไม่มีปุ่มอื่นเหลือ) · `onPurge={null}` = ไม่มีสิทธิ์ ไม่ขึ้นปุ่ม |
| `FollowPage.tsx` | `canPurge = user?.role === 'admin'` (ใช้ role ที่มองอยู่ ⇒ Admin สลับมุมมองเป็น staff แล้วปุ่มหายจริง) · `doPurge()` ปิดป๊อปหลังลบเสมอ (รอบที่ลบไปแล้วไม่มีอยู่ในข้อมูล) |

**ล้างข้อมูลทดลอง (แตะฐานจริง):** สำรองครบ 40 แถว + 40 แถวคิวเป็น JSON ไว้ที่
`…/scratchpad/follow-backup-2569-09-03.json` (176 KB) → ยิงเส้นลบจริง 1 แถวเพื่อตรวจว่าเส้นทำงาน
(ยิงซ้ำได้ 404 ถูกต้อง) → ลบที่เหลือ 38 แถวด้วย **id เป๊ะ ๆ ทีละตัวใน array** → เหลือ **1 แถว**
(กล้วยตานี รอบ 2 · 2 ก.ย. 17:14) ให้เจ้าของกดลบเอง · คิว Lumos จาก 40 เหลือ 1
⚠️ ในนั้นมี **15 แถวสถานะ pending** = สายที่ยังจ่อโทรหาเบอร์จริง ล้างทิ้งไปด้วยแล้ว

**ด่านตรวจ:** test 2,609 ผ่าน · tsc 4 = 0 · eslint 0 err · build ผ่าน · ตรวจบนจอ: ปุ่มขึ้นใต้เส้นประ ·
กดแล้วถามซ้ำ · กด "ไม่" แล้วถอยได้ · เส้นลบจริงคืน `queueRowsDeleted`


### รอบเก้าสิบสาม — หน้าติดตาม: ผลการโทรไม่ขึ้น + แถบส่งต่อหาย (เจ้าของแจ้ง 3 ก.ย. 2569)

เจ้าของแจ้งสองข้อ พร้อมย้ำ *"หน้าการติดตามค่อนข้างสำคัญนะ ให้ความสำคัญหน่อย"*
สืบแล้ว **ไม่ใช่สองบั๊ก แต่เป็นสี่** (สองข้อหลังเจอตอนทานเลขกับฐานจริง)

| # | อาการ | ต้นเหตุจริง | แก้ที่ |
|---|---|---|---|
| 1 | ผลการโทรทั้งสองสายไม่ขึ้นบนแผง | 7 กล่องไม่มีแกนไหนบอกคำตอบของ AI เลย — `โทรติด` กลืน *ยืนยันว่าไป* กับ *ไม่ไปแล้ว* ไว้ก้อนเดียว · `ไป/ไม่ไป` คือ **ผลปิดงานที่คนกรอก** ยังไม่มีใครกด จึงเป็น 0 ทั้งที่ AI คุยจบ 13 สาย | `followCallResults.ts` (ใหม่) + `FollowCallRoundsPanel` — บรรทัด "AI ได้คำตอบแล้ว N สาย — ยืนยันว่าไป x · ไม่ไปแล้ว y…" ใต้กล่อง + ผลเด่น 2 อันดับบนแท็บทุกสาย · **ไม่แตะ 7 กล่องเดิม** |
| 2 | แถบ "ย้ายไปดูแลหลังเริ่มงาน" ไม่ขึ้น | `selectCompletedFollowPeople` บังคับ **ทุกรอบ**ต้องเดินจบ (`every`) · ของจริง Lumos ตอบสายที่ 2 แล้วปิดคนนั้น ส่วน **สายที่ 1 ค้าง `pending` ตลอดกาล** ⇒ ไม่มีใครเข้ากองเลย | `followCompletion.ts` — เปลี่ยนเป็น "มีสายใดได้คำตอบ + ไม่มีอะไรจ่อข้างหน้า = จบ" · สายที่ไม่เคยยิงถือว่า**ตกไป** · `retry_scheduled` ยังกันไว้ (AI นัดโทรซ้ำ = ยังตามอยู่) |
| 3 | (เจอเพิ่ม) คนที่ AI ได้คำตอบว่า **ไม่ไป** ตกหายเงียบ | `reasonOf` เช็คแต่ `outcome_code` ที่คนกรอก ไม่ได้ดูผลจาก AI | เพิ่มเหตุ `ai_going` / `ai_not_going` + `reasonBlocksAftercare()` ⇒ คนไม่ไปขึ้นกองแต่ติดป้าย **"ไม่ต้องส่งต่อ"** ไม่มีปุ่มย้าย · หัวกล่องเลิกเหมารวม เขียน "โทรได้คำตอบแล้ว N คน — ส่งไปดูแลหลังเริ่มงานได้ M คน" |
| 4 | (เจอเพิ่ม) แถบส่งต่อหายเมื่อสลับแท็บ/เลือกวัน | แถบกิน `groups` ที่**ผ่านตัวกรองแท็บ/วันที่/ช่วงเวลา**มาแล้ว | `FollowPage` ส่ง `allGroups` (ชุดเต็ม) — แถบนี้คือ**คิวงานของทั้งระบบ** ไม่ใช่มุมมองของตัวกรอง · มีด่านคุมใน `aftercare.test.ts` |
| 5 | (เจอเพิ่ม · **จอโกหก**) แถบขึ้น *"รอโทร 12 คน — ยังไม่ถึงเวลาที่ตั้งไว้"* ตอนบ่าย ทั้งที่นัด 08:20 | `roundSignal` ไม่รู้เวลา ดูแต่จำนวน | `overdueWaitingCount()` + `roundSignal(counts, overdue)` ⇒ ขึ้นเหลืองว่า **"เลยเวลานัดแล้ว 12 สาย ยังไม่ถูกส่งออกไปโทร — เช็คคิว Lumos"** |

⚠️ **ของจริงที่โผล่จากข้อ 5:** สายที่ 1 ของ 12 คน **ไม่เคยถูกยิงออกจากคิว** (นัด 08:20
ค้าง `pending` ถึงบ่าย) — เดิมจอกลบเรื่องนี้ไว้ใต้คำว่า "ยังไม่ถึงเวลา"

📌 **"สำเร็จ" ยังเป็น 0 ตามนิยามเดิม** — แท็บนั้นนับ *เจ้าหน้าที่กดปิดงานว่าไปทำงานแล้ว*
ไม่ใช่ผลจาก AI · ตอนนี้กดปิดงานได้จากแถบส่งต่อแล้ว เลขจะขึ้นเมื่อมีคนกด
(ถ้าจะให้ AI ปิดงานเองเมื่อได้คำตอบว่าไป = เปลี่ยนนิยาม ต้องให้เจ้าของเคาะก่อน)

**ด่านตรวจ:** เทสต์ใหม่ `followCallResults.test.ts` 13 ข้อ + `followCompletion.test.ts` 17 ข้อ
(มีเคสจริง 3 ก.ย.) · รวม 2,626 ผ่าน · tsc 4 = 0 · eslint 0 err · build ผ่าน ·
ตรวจบนจอ: สายที่ 1 → "AI ได้คำตอบแล้ว 1 สาย — ยืนยันว่าไป 1" · สายที่ 2 → "13 สาย —
ยืนยันว่าไป 4 · ไม่ไปแล้ว 6 · รับสายแล้ว 2 · ไม่ตอบ 1" · แถบ "โทรได้คำตอบแล้ว 6 คน —
ส่งไปดูแลหลังเริ่มงานได้ 2 คน" อยู่ครบแม้สลับแท็บ · แถบเตือน "เลยเวลานัดแล้ว 12 สาย"


### รอบเก้าสิบสี่ — ดันคะแนนความพร้อมทุกหน้า (ยกเว้น WL) ให้ถึง 8 · ยกที่ 1 (3 ก.ย. 2569)

เจ้าของสั่ง: *"นอกจาก WL ที่เหลือทำให้ได้อย่างน้อย 8 จะดีมากถ้าได้ 10"*
ยกแรกเก็บของที่ **แก้ด้วยโค้ดฝั่งเราได้เอง** ก่อน (ที่เหลือติดคนอื่น — จดไว้ท้ายตาราง)

| หน้า | เดิม | ทำอะไร |
|---|---|---|
| **จับคู่งาน** | 6 | 🔴 **เลขทุกช่องกดดูรายชื่อได้แล้ว** — เส้นใหม่ `GET /api/lumos/call-funnel?people=<ช่อง>` + ป๊อปรายชื่อ (ชื่อ · เบอร์ · ผล · สายที่เท่าไหร่) · เหตุ: พนักงานใหม่ให้ 4/10 เพราะ *"เห็นแต่ตัวเลขรวม ไม่เห็นรายชื่อจริง"* — เลขที่พิสูจน์ไม่ได้ คนไม่เชื่อ |
| **กล่องงาน** | 7 | "ขั้นนี้ต้องทำอะไร" ย้ายจาก `title=` (hover) มา**พิมพ์บนจอ** · ไม่ได้เลือกขั้น = อธิบายภาพรวมสี่ขั้นเป็นประโยคเดียว |
| **ดูแลหลังเริ่มงาน** | 5 | ปุ่มเดียวจบ: **"ย้าย + ปิดงานติดตาม"** — เดิมย้ายชื่ออย่างเดียว งานติดตามค้างในกอง "กำลังตาม" ทำให้แท็บสำเร็จเป็น 0 ตลอด และคนเดิมโผล่สองหน้า |
| **ตั้งค่า** | 7.5 | ตารางผู้ใช้ 10 คอลัมน์ล้นขอบขวา ⇒ จอแคบซ่อน Username/Email (อีเมลไปอยู่ใต้ชื่อ ข้อมูลไม่หาย) ช่องที่ต้องกรอกจริงไม่หลุดจอแล้ว |

**ของใหม่ที่ต้องรู้:**
* `api/_lib/lumosQueueDefs.ts` เพิ่ม `queuePayloadName()` / `queuePayloadPhone()` — **นิยามเดียว**
  ของการอ่านชื่อ/เบอร์จาก payload คิว (ต้นทางเก็บคนละคีย์: ติดตามใช้ `recipient_*` บอร์ดใช้ `candidate_*`)
* `CELL_WHERE` ใน `lumos-call-funnel.ts` = เงื่อนไข SQL ของแต่ละช่อง · 🔴 **key ต้องตรงกับ
  `aiCallFlowCells()` เป๊ะ** มีเทสต์คุมที่ `tests/api/callFunnelPeople.test.ts`
* รายชื่อ **ห้าม dump payload ทั้งก้อน** (ในนั้นมีบทพูดและเบอร์ฉุกเฉิน) · เพดาน 500 แถว
* โหลดรายชื่อพลาด = โยน error ให้จอบอก **ห้ามคืนลิสต์ว่าง** (ว่างอ่านเหมือน "ไม่มีใคร")

**ยังไม่ถึง 8 เพราะติดเรื่องนอกโค้ด — ต้องให้เจ้าของตัดสิน/คุยกับคนอื่น:**
1. **คลังผู้สมัคร (3)** — ในฐานนี้มีคนเดียว ของจริงอยู่บอร์ดนอก ⇒ ต้องเดินแผนยุบ iRecruit
2. **Lumos (5.5)** — ข้ามสายแรก + ไม่ส่งผลโทรเบอร์ฉุกเฉินกลับ ⇒ ฝั่งเขาต้องแก้
3. **WL (2.5)** — เจ้าของสั่งข้ามไปก่อน

**ด่านตรวจ:** test 2,633 ผ่าน · tsc 4 = 0 · eslint 0 err · build ผ่าน · ตรวจบนจอ:
กดกล่อง "ทั้งหมด" หน้าจับคู่งาน → ป๊อปขึ้นรายชื่อ 54 คนพร้อมเบอร์และผล ·
กดขั้น 2 บนกล่องงาน → ขึ้น "ต้องทำ: ใส่สถานที่ปฏิบัติงานที่ผู้สมัครจะเห็น"


### รอบเก้าสิบห้า — โหมดฝัง iframe ของหน้าสาธารณะ (เจ้าของถาม 3 ก.ย. 2569)

คำถาม: *"หน้าสาธารณะทำเป็น iframe ได้ไหม"*
คำตอบ: **ฝังได้อยู่แล้ว** — ระบบไม่ได้ตั้ง `X-Frame-Options` / `frame-ancestors` กันไว้เลย
แต่ฝังดิบ ๆ จะได้หัวเว็บ+ท้ายเว็บของเราซ้อนเข้าไปในหน้าเขา จึงทำ **โหมดฝัง** ให้

| ไฟล์ | ทำอะไร |
|---|---|
| `src/lib/embedMode.ts` (ใหม่) | `isEmbedMode()` อ่าน `?embed=1` · `isFramed()` · `startEmbedHeightReporter()` ส่งความสูงให้หน้าแม่ผ่าน `postMessage` (ชื่อข้อความ `so-recruit:height`) |
| `PublicApplyLayout.tsx` | โหมดฝัง = คืนแค่ `{children}` บนพื้นโปร่ง — **ตัดแค่เปลือก** เนื้อหา/ตัวกรอง/ปุ่มสมัครครบเหมือนเดิม |
| `docs/user-guide/embed-apply-page.md` (ใหม่) | โค้ดคัดลอกไปวางได้เลย + วิธีให้กรอบสูงตามเนื้อหา + ฝังเฉพาะตำแหน่ง/ประกาศใบเดียว |

🔴 **ข้อความที่ส่งหาหน้าแม่มีแค่ความสูง** — ห้ามยัดอะไรเพิ่ม (หน้าแม่เป็นเว็บของใครก็ไม่รู้
ต้องส่ง `'*'`) · ไม่ได้อยู่ในกรอบ = ไม่ส่งเลย

⚠️ **ช่องที่ยังเปิดอยู่ (รอเจ้าของสั่ง):** ไม่มีการกันฝังทั้งเว็บ ⇒ **หน้าที่ต้องล็อกอิน
ก็ถูกเอาไปใส่ iframe ได้** (clickjacking) · ล็อกให้ฝังได้เฉพาะ `/apply` ต้องแก้ `vercel.json`
ซึ่งเป็นไฟล์ routing ของ production — ยังไม่แตะ

**ด่านตรวจ:** เทสต์ใหม่ `tests/api/embedMode.test.ts` 8 ข้อ · รวม 2,641 ผ่าน · tsc 4 = 0 ·
eslint 0 err · build ผ่าน · **ตรวจบนจอด้วย iframe จริง**: สร้างกรอบชี้ `/apply?embed=1`
เนื้อในโหลดครบ ไม่มีหัว/ท้าย และหน้าแม่ได้ข้อความความสูง 646px แล้วยืดกรอบตามได้จริง


### รอบเก้าสิบหก — แท็กจำนวนคลิกหน้าสาธารณะ (เจ้าของถาม 3 ก.ย. 2569)

**ของที่มีอยู่ก่อนแล้ว (คนมักลืม):** `recruit_posting_links.hit_count` นับ **คลิกเปิดลิงก์ช่องทาง**
มาตลอด — วัดจริง 3 ก.ย.: **24 ลิงก์ · 103 คลิก · มีคลิกจริง 11 ลิงก์** และโชว์เป็นชิป
"ช่องทาง · คลิก N" บนการ์ดกล่องงานอยู่แล้ว

**ที่ยังไม่มีและเพิ่งทำ:** เข้ามาแล้ว **กดสมัครใบไหน · ส่งใบสมัครจริงกี่ใบ · มาจากช่องทางไหน**

| ไฟล์ | ทำอะไร |
|---|---|
| `migrations/115_public_page_clicks.sql` | ตารางยอดคลิก **รายวัน** (day · action · job_ref · posting_id · link_code · embedded · hits) + unique index ให้ upsert บวกยอดเข้าแถวเดิม |
| `api/_lib/publicClicks.ts` | `recordPublicClick()` upsert · `listPublicClicksByJob(days)` สรุปรายใบ |
| `api/_handlers/public/click.ts` | `POST /api/public/click` ไม่ต้องล็อกอิน · **คืน 204 เสมอ** · rate limit 120/10 นาที |
| `api/_handlers/public-click-stats.ts` | `GET /api/public-click-stats?days=30` (เจ้าหน้าที่) |
| `src/lib/publicClickApi.ts` | ยิงแล้วลืม + `keepalive` (ผู้ใช้กดแล้วหน้าเปลี่ยนทันทีก็ยังส่งจบ) · อ่านรหัสช่องทางจาก `/apply/p/<code>` หรือ `?ref=` |
| `JobBoardView` · `PublicApplyDialog` | จุดนับ: กดปุ่มสมัคร (`open_apply`) · ส่งใบสมัครสำเร็จ (`submit`) · ชิป "กดสมัคร N · ส่งจริง M" บนการ์ด |

🔴 **กติกาที่ห้ามละเมิด (มีเทสต์คุมทุกข้อ):**
1. **ไม่เก็บว่าใครกด** — ไม่มี IP / user-agent / คุกกี้ · ยอดรวมรายวันพอตอบคำถามธุรกิจแล้ว
2. **นับพลาดห้ามทำให้หน้าสมัครงานล้ม** — เส้นคืน 204 เสมอ · ฝั่งจอยิงแล้วลืม
3. **ห้ามนับการกดของเจ้าหน้าที่** — `if (!isStaff)` เท่านั้น (ไฟล์การ์ดใช้ร่วมสองหน้า
   ถ้านับปนกัน เลขจะโป่งด้วยการกดของทีมเราเองจนอ่านไม่ออกว่าประกาศไหนดังจริง)
4. **ห้ามใส่ CHECK บน `action`** — ของเดิมโดนมาสองรอบ (source · result_scope)
5. ชิปขึ้น**เฉพาะใบที่มีคนกดจริง** — ชิป 0 ที่ขึ้นทุกใบไม่ใช่สัญญาณ มันคือพื้นหลัง

**ด่านตรวจ:** เทสต์ใหม่ 9 ข้อ · รวม 2,650 ผ่าน · tsc 4 = 0 · eslint 0 err · build ผ่าน ·
**ตรวจครบวงบนของจริง**: ยิงเส้นซ้ำสองครั้ง ⇒ แถวเดียว hits=2 (upsert ทำงาน) · ยิงข้อมูลเสีย
⇒ 204 ไม่มีแถวขยะ · กดปุ่มสมัครจริงบน `/apply` ⇒ ได้แถว `siamraj-sql:OPL6808001` ·
เปิดกล่องงาน ⇒ ชิป **"กดสมัคร 1"** ขึ้นบนการ์ดใบนั้น · ลบแถวทดสอบด้วย job_ref เป๊ะแล้ว


### รอบเก้าสิบเจ็ด — คืนกติกา shadcn + จัดหน้าหลักให้เข้าชุด (เจ้าของทัก 3 ก.ย. 2569)

เจ้าของทัก: *"หน้า UI ฉันให้ใช้ Shadcn เพื่อคุม Framework ห้ามสร้าง component เอง
ลืมไหม หน้า UI หน้าหลักมันดูสะเปะสะปะ"*

**ผมผิดจริง — ของใหม่ที่ทำวันนี้มี `<button>` ดิบ 3 จุด** (ปุ่มลบทิ้ง/ลบเลย/ไม่ ในป๊อปสาย ·
ปุ่มย้ายไปดูแลหลังเริ่มงาน) แก้เป็น `Button` ของ shadcn แล้วทั้งหมด

**ทำไมหน้าหลักดูสะเปะสะปะ (วัดจากโค้ดจริง):** แต่ละแผงปั้นเปลือกของตัวเอง
`LumosCallHealthPanel` = `rounded-2xl border border-white/70 bg-white/70 p-3.5` ·
`FollowTodayPanel` = `rounded-2xl border p-4 md:p-5` · `TeamBoardPanel` = `jarvis-deck` ·
`HomeKpiRow`/`HomeDigestPanels` = **ไม่มีเปลือกเลย ลอยบนพื้นเปล่า** ⇒ มุม/ขอบ/ระยะใน/เงา
ไม่ตรงกัน + แต่ละแผงใส่ `mb-` ของตัวเอง (mb-3 · mb-5 · mb-6) ระยะห่างจึงไม่เท่ากันทั้งหน้า

| ไฟล์ | ทำอะไร |
|---|---|
| `src/components/home/HomeSection.tsx` (ใหม่) | **เปลือกเดียวของทุกแผง — ประกอบจาก `Card`/`CardHeader`/`CardContent` ของ shadcn** (ไม่ใช่ primitive ใหม่) · หัวข้อ/คำอธิบาย/ช่องปุ่มมุมขวา ใช้ระดับตัวอักษรชุดเดียว |
| `HomePage.tsx` | ก้อน "ตัวเลขวันนี้" (หัวข้อ + ตัวกรอง BU + การ์ด KPI) เข้า `HomeSection` · **จังหวะแนวตั้งชุดเดียว** `space-y-5` ที่กล่องนอก เลิกใส่ `mb-` ทีละแผง · ปุ่มในป๊อป → `Button` (เลิก `jarvis-btn-*`) |
| `FollowTodayPanel` · `LumosCallHealthPanel` | ปุ่มรีเฟรช/ดูรายชื่อ → `Button` variant ghost/secondary |
| `FollowRoundsDialog` · `FollowCompletedPanel` | ปุ่มที่เพิ่มวันนี้ → `Button` (destructive/outline) |

**วัดผลบนจอ:** ความสูงหน้าหลัก **6,449px → 2,941px** (ครึ่งเดียว) · การ์ดบนหน้าเหลือ
มุม/ขอบแบบเดียว · เหลือสามก้อนเรียงระยะเท่ากัน

🔴 **ที่ยังไม่แก้ — ต้องให้เจ้าของเคาะก่อน (ของเดิม แก้ทีเดียวหน้าตาเปลี่ยนทั้งระบบ):**
1. คลาส `jarvis-btn-*` (ปุ่มที่ปั้นเองใน `index.css`) ยังเหลือ **61 จุดใน 19 ไฟล์**
   หนักสุด `MatchingPage.tsx` 17 จุด · `RmWorkspace` 6 · `RmSearchBar` 5
2. **การ์ดตัวเลขที่กดได้** (`HomeKpiRow` · `HomeDigestPanels` · ช่องบนแผง AI โทร)
   ยังเป็น `<button>` ธรรมชาติหุ้มการ์ด — ไม่ใช่ปุ่มที่ปั้นเอง แต่ก็ยังไม่ได้ผ่าน `Card` ของ shadcn
3. **deck สองก้อนบนสุดยังเป็นธีมมืด** ส่วนก้อนล่างสว่าง (deck เจ้าของเคาะเองไว้แล้ว)
   ถ้าจะให้ทั้งหน้าเป็นภาษาเดียวต้องเลือกว่ายึดมืดหรือสว่าง

**ด่านตรวจ:** เทสต์ใหม่ `tests/api/homeShadcnShell.test.ts` 5 ข้อ (กันไม่ให้ `jarvis-btn-*`
กลับมาบนหน้าหลัก · กันไม่ให้ปั้นเปลือกซ้อน Card · กัน `mb-` ทีละแผง) ·
รวม 2,655 ผ่าน · tsc 4 = 0 · eslint 0 err · build ผ่าน


### รอบเก้าสิบแปด — Kanit ทั้งระบบ + Success Rate (เจ้าของสั่ง 4 ก.ย. 2569)

**คำสั่งที่ต้องจำถาวร:** *"ทั้งระบบต้องเป็น font Kanit ดูเรื่องความเป็นระเบียบ ขนาด
ช่องไฟ ชิดซ้าย ชิดขวา ตรงกลาง อย่าหลุด Framework จำไว้เลยนะ"*

**ของเดิมที่ผิดกติกา (วัดจริง):** โหลดสามฟอนต์ — Kanit (ตัวหลัก) · **JetBrains Mono**
(คลาส `font-mono` ใช้อยู่ **73 จุด**) · **Instrument Sans** (หน้าเข้าสู่ระบบ กำหนด
`fontFamily` ในไฟล์เอง) ⇒ จอมีสามหน้าตาปนกัน

| ไฟล์ | ทำอะไร |
|---|---|
| `src/index.css` | `@import` เหลือ Kanit ตัวเดียว · `--font-mono` ชี้ Kanit |
| `tailwind.config.ts` | `mono: ['Kanit', …]` ⇒ `font-mono` 73 จุดเดิมยังใช้ได้แต่ได้ Kanit หมด **ไม่ต้องไล่แก้ทีละจุด** |
| `LoginPage.tsx` | เลิก Instrument Sans → Kanit (⚠️ ทับคำสั่งเดิม 26 ส.ค. ที่ให้ยก mockup มาเป๊ะ — คำสั่งล่าสุดชนะ) |

🔴 **ความเป็นระเบียบของตัวเลขมาจาก `tabular-nums` ไม่ใช่จากการสลับไปฟอนต์ mono**
(ระบบใช้ `tabular-nums` อยู่ 173 จุดแล้ว) · การจัดชิด: ไทยชิดซ้าย · ตัวเลขชิดขวา

**Success Rate (เจ้าของสั่งรอบเดียวกัน):** *"เพิ่มค่า Success Rate ... เพื่อจะได้รู้ว่า
คนรับสายมี % สำเร็จเท่าไหร่"*

🔴 **คนละตัวกับ "สำเร็จ" ที่มีอยู่แล้ว — ฐานคนละอัน ห้ามสลับกันใช้**
* `confirmedPct` (ช่อง **สำเร็จ**) = สำเร็จ ÷ **สายที่มีผลทั้งหมด** (รวมที่ยกหูไม่ได้)
  ⇒ ตอบว่า *"ส่งไปแล้วได้ผลดีกี่ %"*
* `successRatePct` (ช่อง **Success Rate**) = สำเร็จ ÷ **โทรติด** ⇒ ตอบว่า
  *"พอได้คุยกับคนแล้ว ปิดได้กี่ %"* · **สูงกว่าเสมอ** · ไม่มีใครรับสาย = `null` ห้ามโชว์ 0%
* บนจอ **ต้องเขียนฐานกำกับทั้งสองช่อง** — วัดจริง 4 ก.ย.: สำเร็จ 36% · Success Rate 38%
  (5 จาก 13 คนที่รับสาย) ถ้าไม่เขียนฐาน คนอ่านสลับกันแน่นอน

**ด่านตรวจ:** เทสต์ใหม่ `tests/api/typographyRules.test.ts` 5 ข้อ (กันฟอนต์อื่นกลับมา ·
กันไฟล์จอกำหนด `fontFamily` เอง · บังคับเขียนฐานของสอง %) + เทสต์เลข Success Rate ·
รวม 2,660 ผ่าน · tsc 4 = 0 · eslint 0 err · build ผ่าน ·
**ตรวจบนจอ**: ทุก element บนหน้าหลัก **456 ชิ้นเป็น Kanit ล้วน** ฟอนต์ที่โหลดเหลือตัวเดียว ·
แดชบอร์ดขึ้นช่อง "Success Rate 5 (38%) · % จากคนที่รับสาย 13 สาย"


### รอบเก้าสิบเก้า — Success Rate ขึ้นหน้าหลักตรง Lumos (เจ้าของสั่ง 4 ก.ย. 2569)

| ไฟล์ | ทำอะไร |
|---|---|
| `TeamBoardPanel.tsx` | คอลัมน์ทีม Lumos เพิ่มบรรทัด **"SUCCESS RATE · 7 วันล่าสุด"** ตัวเลขใหญ่ + ฐานกำกับ *"จากคนที่รับสาย N สาย"* · ไม่มีใครรับสาย = **ขีด** ห้ามโชว์ 0% |
| `HomePage.tsx` | โหลด `fetchCallRateSeries(60)` แล้ว `compareCallRate(series, 7, วันนี้)` — **ทางเดียวกับแดชบอร์ดเป๊ะ** |

🔴 **ทำไมต้องใช้ทางเดียวกับแดชบอร์ด** — ถ้าหน้าหลักคำนวณเองคนละสูตร (เช่นใช้ funnel
ยอดสะสม แทนหน้าต่าง 7 วัน) สองหน้าจะโชว์ % ไม่ตรงกัน แล้วไม่มีใครเชื่อสักหน้า
(กติกา "หนึ่งเมตริกหนึ่งนิยาม" ที่โดนมาแล้วกับหน้าแรก)

**ด่านตรวจ:** ตรวจบนจอ — หน้าหลักขึ้น **38% · จากคนที่รับสาย 13 สาย** ตรงกับแดชบอร์ด
ที่ขึ้น *"Success Rate 38% (จากคนที่รับสาย)"* เป๊ะ · test 2,660 ผ่าน · tsc 4 = 0 ·
eslint 0 err · build ผ่าน


### รอบร้อย — ล้างของที่หลุด Framework ทั้งระบบ 3 ยก (เจ้าของสั่ง 4 ก.ย. 2569)

เจ้าของถาม *"มั่นใจไหมว่าทั้งระบบไม่หลุด Framework"* → ผมวัดจริงแล้วตอบว่า **ยังไม่ทั้งระบบ**
เจ้าของสั่ง *"เริ่มทั้งหมด ห้ามหลุด Framework"* + *"ฉันไม่รีบ แต่ขอความถูกต้อง เป๊ะ"*

**ยกที่ 1 — ปุ่ม (85 จุด)**
* `jarvis-btn-*` **54 จุด / 19 ไฟล์** → `<Button>` ของ shadcn
  (primary→default · secondary→secondary · ghost→ghost · danger→destructive)
* เจอคลาสปั้นเองอีกชุดระหว่างทาง: `jarvis-pill-btn` **31 จุด / 26 ไฟล์** → `<Button>`
* `<a>` / `<Link>` ที่ทำหน้าที่ปุ่ม → `<Button asChild>` · ปุ่มสลับสถานะ → สลับ `variant`
* **ถอด `.jarvis-btn*` และ `.jarvis-pill-btn` ออกจาก `index.css` ทั้งหมด** ไม่ให้หยิบกลับมาใช้ได้อีก

**ยกที่ 2 — กล่อง (4 ไฟล์ เหลือ 0)**
| ไฟล์ | เดิม | ใหม่ | ได้อะไรเพิ่ม |
|---|---|---|---|
| `FollowEditDialog` | `fixed inset-0` + `role="dialog"` | `Dialog` | picker หน่วยงาน **ฝังเป็นแผงในฟอร์ม** (`BoardUnitPickerBody`) แทน Dialog ซ้อน Dialog |
| `AppNavDrawer` | `<aside role="dialog">` + translate เอง | `Sheet side="left"` | focus trap · ปิดด้วย Esc · ล็อกสกรอลล์พื้นหลัง (เดิมไม่มีสักอย่าง) |
| `FollowTodayPanel` | ปั้นเอง | `Dialog` | ปุ่มปิดมากับ DialogContent |
| `FollowPage` (เตือนลงซ้ำ) | `role="alertdialog"` | `AlertDialog` | บังคับเลือกทาง กดข้างนอกไม่ปิด |

**ยกที่ 3 — มุม/ระยะ/สี**
* `rounded-[1.5rem]` **52 จุด → `rounded-3xl`** (24px เท่ากันเป๊ะ หน้าตาไม่เปลี่ยน) ·
  `StatCard` ตัดมุมซ้ำที่ `glass-card` ตั้งให้อยู่แล้ว ⇒ มุมสุ่มในไฟล์จอ **เหลือ 0**
* ระยะสุ่ม 4 จุดในหน้าเข้าสู่ระบบ (`mb-[7px]` `gap-[22px]` `p-[25px]`) → สเกล Tailwind ⇒ **0**
* สี hex ดิบ → **ตัวแปรธีม + สีของ Tailwind**: `--jarvis-gold` `--jarvis-hero(-strong/-dim/-hover)`
  `--jarvis-splash` `--jarvis-night(-hover)` ประกาศที่ `tailwind.config.ts` เป็น
  `ink · gold · hero · splash · night` ⇒ ไฟล์จอเรียก `bg-ink` `text-gold` `bg-hero`
  · ทองในธีมมืดสลับที่ตัวแปร ไฟล์จอไม่ต้องมี `dark:` อีก
  · **ยกเว้นสีโลโก้ Microsoft 4 สี** (สีแบรนด์คนอื่น ห้ามเปลี่ยน)

🔴 **ด่านกันย้อนกลับ** — `tests/api/typographyRules.test.ts` เพิ่ม 5 ข้อ ไล่สแกนไฟล์จอทุกไฟล์
(ไม่รวม `src/components/ui/**` ที่เป็นโค้ดของ shadcn เอง) ว่าห้ามมี: คลาสปุ่มปั้นเอง ·
`role="dialog"` · `rounded-[` · ระยะ `-[` · hex ดิบ · และ CSS ต้องไม่มีคลาสปุ่มเหลือ
⚠️ ตัวสแกนต้อง**ตัดคอมเมนต์ทุกแบบออกก่อน** ไม่งั้นจับคอมเมนต์ที่อธิบายว่า "เลิกใช้แล้ว" ว่าผิด

**ด่านตรวจ:** test 2,665 ผ่าน · tsc 4 = 0 · eslint 0 err · build ผ่าน ·
ตรวจบนจอ 7 หน้า (หน้าหลัก · ใบขอ · กล่องงาน · ติดตาม · แดชบอร์ด · WL · จับคู่งาน):
ทุกตัวอักษรเป็น Kanit · ไม่เหลือคลาสปุ่มเก่าสักจุด · เมนูซ้ายกด Esc ปิดได้จริง ·
การ์ดบนแต่ละหน้าเหลือมุมชุดเดียว (ใบขอ 20 การ์ด = 22.5px ทั้งหมด)


### รอบร้อยเอ็ด — ปุ่มไอคอน: ขนาดไอคอนไม่เท่ากันทั้งระบบ (เจ้าของจับได้ 4 ก.ย. 2569)

เจ้าของบอกว่าเจอจุดที่ไม่ถูกเรื่องปุ่มไอคอน แล้วให้ผมหาเอง — **หาเจอและวัดได้**

**อาการที่วัดจริงบนหน้าติดตาม:** ปุ่มที่ข้อความ **11px เท่ากัน** แต่ไอคอนออกมา
**3 ขนาดในแถวเดียวกัน** — `แก้ไข` 14px · `เลือกวัน` 16px · `ลบทิ้ง` 18px

**ต้นเหตุ:** `buttonVariants` base มี `[&_svg]:size-4` ตัวเดียวทุกขนาด และ selector ลูก
(`.[&_svg]\:size-4 svg`) **ชนะคลาสบนตัวไอคอนเอง** ⇒ ที่โค้ดเขียน `h-3 w-3` **ไม่มีผลเลย**
ปุ่มที่แปลงเป็น shadcn แล้วจึงโดนบังคับเป็น 1rem ส่วนปุ่มที่ยังเป็น `<button>` ดิบยังได้ขนาดเดิม
⇒ ยิ่งแปลงยิ่งปนกัน (⚠️ root font-size ของระบบนี้ = **18px** ไม่ใช่ 16 · 1rem = 18px)

| แก้ที่ | ทำอะไร |
|---|---|
| `ui/button.tsx` | **ขนาดไอคอนผูกกับขนาดปุ่ม**: `sm → size-3.5` · `default/icon → size-4` · `lg → size-5` · ถอด `size-4` ออกจาก base (กัน twMerge งงว่าใครชนะ) · เพิ่ม variant **`hero` / `heroSolid`** สำหรับแถบพื้นเข้ม |
| ทั่วระบบ (60 ปุ่ม / 33 ไฟล์) | ลบคลาสขนาดไอคอนที่ตายแล้ว (`h-3 w-3` · `w-4 h-4`) ออกจากไอคอนใน `<Button>` — โค้ดจะได้ไม่โกหก |
| `FollowRoundsDialog` · `FollowCompleteControls` | ปุ่มที่เหลือในแถวเดียวกัน (แก้ไข · บันทึกว่าเสร็จสิ้น · ยกเลิก · ย้อนสถานะ) → `Button` ⇒ ทั้งแถวขนาดเดียวกัน |
| `PageHeroStrip` | ถอด `heroButton` / `heroButtonSolid` (คลาสปุ่มที่ประกาศเอง 16 จุด/5 ไฟล์) → `variant="hero"` |
| `AppLayout` · `NotificationPanel` | ปุ่มไอคอนบนแถบหัว (ธีม · เมนู · ตั้งค่า · รหัสผ่าน · ออกจากระบบ · กระดิ่ง) → `Button size="icon"` |

**ผลบนจอ (กล่องงาน):** ปุ่มที่มีไอคอน 47 ปุ่ม — **ไม่เหลือปุ่มดิบสักปุ่ม** · ขนาดไอคอนเหลือ
2 ค่าตามขนาดปุ่ม (16px สำหรับ `sm` · 18px สำหรับ `icon`/`default`) ไม่ใช่ 3-4 ค่าปนกันแบบเดิม

🔴 **ด่านใหม่ 3 ข้อ:** ไอคอนใน `<Button>` ห้ามมีคลาสขนาดของตัวเอง (ดูเฉพาะ**ลูก** —
`h-7 w-7` บนตัวปุ่มเองยังทำได้ เพราะนั่นคือย่อขนาดปุ่ม) · ทุก `size` ต้องประกาศขนาดไอคอน ·
`PageHeroStrip` ห้ามมี `export const heroButton` อีก

**ล้างข้อมูลหน้าติดตาม (เจ้าของสั่งรอบเดียวกัน):** สำรอง 27 รายการ + 27 แถวคิวเป็น JSON
(`follow-backup-2569-09-04.json`) แล้วลบด้วย id เป๊ะ ๆ เหลือ 0 · ⚠️ ในคิวมี **12 สายสถานะ
pending** ที่ยังจ่อโทรหาเบอร์จริง ยกออกไปพร้อมกันแล้ว

**ด่านตรวจ:** test 2,668 ผ่าน · tsc 4 = 0 · eslint 0 err · build ผ่าน


### รอบร้อยสอง — หน้าหลักเข้ามาตรฐาน shadcn + กฎใหม่ "ห้ามเขียน CSS เอง" (4 ก.ย. 2569)

เจ้าของสั่ง: *"ปรับหน้า UI/UX ให้มาใช้ Standard โดยใช้ Shadcn คุม เริ่มจากหน้าหลักก่อน"*
และระหว่างทางสั่งเพิ่ม: **"css ที่สร้างเอง ต่อไปห้ามสร้างเอง จำไว้เลยนะ"**

🔴 **กฎใหม่ถาวร (จดใน CLAUDE.md + memory แล้ว):** ห้ามเพิ่มคลาสใหม่ใน `src/index.css`
หรือไฟล์ CSS ใด ๆ · อยากได้หน้าตาใหม่ให้ประกอบจาก component ของ shadcn + utility ของ
Tailwind · ต้องมี variant ใหม่จริงให้เพิ่มที่ `src/components/ui/<component>.tsx` ที่เดียว
(คลาสเดิมที่มีอยู่แล้วอย่าง `jarvis-deck` `glass-card` ยังใช้ต่อได้ รอเจ้าของสั่งค่อยล้าง)

| ไฟล์ | เดิม | ใหม่ |
|---|---|---|
| `HomeBuFilter` | `<button>` + `style={{ boxShadow: 'inset 0 0 0 1px …' }}` + สี hex จาก `HUD_HEX` | **`ToggleGroup type="single"`** + `Badge` บอกจำนวนใบขอ ⇒ ได้คีย์บอร์ด (ลูกศรเลื่อนในกลุ่ม) และ `data-state` มาให้เอง |
| `HomeKpiRow` | ประกาศชุดคลาสการ์ดเอง (กรอบ/พื้น/เงา/hover + คู่ `dark:` ทุกตัว) | **`Card` + `CardContent`** ห่อด้วยปุ่มให้กดทั้งใบ · เหลือแต่ utility ขนาดตัวอักษร |
| `CommandDeck` | `<section className="jarvis-deck">` · ชิปสถานะวาดกรอบเอง | **`Card`** + **`Badge variant="outline"`** (สกินพื้นเข้มยังเป็นคลาสเดิมที่เจ้าของเคาะ) |
| `TeamBoardPanel` | `<section className="jarvis-deck">` | **`Card`** |

**ตรวจบนจอ:** หน้าหลักเหลือ 3 ก้อนเป็น `Card` ทั้งหมด · การ์ด KPI 9 ใบใช้เปลือกเดียวกันหมด
(มุม 22.5px เท่ากันทุกใบ) · ตัวกรอง BU เป็น ToggleGroup จริง (`data-state=on/off`) และ
**ยิง `?bu=LBD` ออกไปจริง** ตอนกด (ทานกับ network แล้ว — เลข "ใบขอที่ยังเปิดรับ" ไม่เปลี่ยน
เพราะการ์ดนั้นเป็นยอดสะสมทั้งระบบโดยตั้งใจ ไม่ผูกกับ BU)

**ด่านตรวจ:** test 2,671 ผ่าน · tsc 4 = 0 · eslint 0 err · build ผ่าน


### รอบร้อยสาม — ภาษาการ์ดกลาง `FeatureCard` (จากสเปก Premium Features · 4 ก.ย. 2569)

เจ้าของส่งสเปกหน้า "Premium Features Section" (ธีมดำ · Tailwind CDN · custom CSS ·
ฟอนต์ Inter · ฉาก 3D isometric) มาถามว่าเอามาปรับใช้ทั้งระบบได้ไหม
→ ผมชี้ให้เห็นว่า **4 อย่างในสเปกนั้นชนกฎที่เจ้าของสั่งไว้เอง** แล้วเจ้าของเลือก
**"เอาเฉพาะภาษาการ์ด"**

| สเปกสั่ง | ชนกฎข้อไหน | ทำแทนด้วยอะไร |
|---|---|---|
| ฟอนต์ Inter | "ทั้งระบบต้องเป็น Kanit" | Kanit (Inter ไม่มีตัวไทย) |
| `<style>` custom CSS | "ห้ามเขียน CSS เอง" | utility + `tailwindcss-animate` |
| Tailwind CDN + hex ดิบ | โปรเจกต์ build เอง · ห้าม hex ดิบ | token ของธีม (`TONE`) |
| `rounded-[2.5rem]` · ระยะสุ่ม | เทสต์ `typographyRules` ฟ้องทันที | สเกลของ Tailwind |

**ของใหม่:** `src/components/shared/FeatureCard.tsx` — ประกอบจาก `Card`/`CardContent`
ของ shadcn · กล่องไอคอน 56px มุมมนสีประจำเรื่องจาก `TONE[tone].soft/.value` ·
ไหลเข้าทีละใบด้วย `animate-in fade-in slide-in-from-right-4` + `delay-*` (ไม่ใช้ framer-motion) ·
ไอคอนโตขึ้นตอนชี้การ์ด · ส่ง `onClick` = ห่อปุ่มให้กดทั้งใบและโฟกัสได้

**ใช้ที่แรก:** `RoleHubPage` (`/opl` `/staff` `/supervisor` `/admin`) — เดิมใช้คลาส
`jarvis-menu-card` ที่ปั้นเอง + framer-motion ⇒ เปลี่ยนเป็น `FeatureCard` ทั้งกริด
พร้อมกันนี้ **ยกชื่อ/ไอคอนมาจาก `soRecruitNav`** เพราะหน้านี้เคยตั้งชื่อเอง
("Follow" · "หน่วยงาน") ไม่ตรงกับเมนูหลัก ("ติดตาม" · "ใบขอ") — กติกาเดียวกับเมนูล่าง

⚠️ **ที่ไม่เอามาจากสเปก:** ธีมดำล้วน (`#0B0A1A`) และฉาก 3D มือถือ/ลูกบาศก์ลอย —
หน้าทำงานของระบบนี้มีตัวเลขหนาแน่น พื้นดำ + เงาเรืองทุกกล่องอ่านนาน ๆ แล้วล้า
(สเปกนั้นออกแบบมาสำหรับหน้าขายของที่มีข้อมูลไม่กี่ตัว)

**ด่านตรวจ:** test 2,671 ผ่าน · tsc 4 = 0 · eslint 0 err · build ผ่าน

🔴 **เจ้าของตีตกทันทีที่เห็นจอ: *"ย้อนคืน ๆ ไม่สวย"* (4 ก.ย. 2569)**
⇒ `FeatureCard` **ถูกลบทิ้ง** · `RoleHubPage` คืนหน้าตาเดิม (`jarvis-menu-card` +
framer-motion) · **เก็บไว้อย่างเดียวคือชื่อ/ไอคอนที่ยกมาจาก `soRecruitNav`**
เพราะอันนั้นเป็นการแก้บั๊กชื่อไม่ตรงเมนู ไม่ใช่เรื่องหน้าตา
📌 **บทเรียน:** ภาษาการ์ดของสเปกภายนอก (การ์ดกว้างเต็มแถว ไอคอนซ้าย ข้อความขวา)
ไม่เข้ากับหน้าเมนูที่เป็น **กริดการ์ดสี่เหลี่ยม** ของระบบนี้ — ครั้งหน้าถ้าจะลอง
ภาษาการ์ดใหม่ ให้ทำหน้าตัวอย่างให้ดูก่อน อย่าไปแทนของที่ใช้อยู่


### รอบร้อยสี่ — หน้าเข้าสู่ระบบ: ซ้ายอธิบายระบบ · ขวาล็อกอิน (เจ้าของสั่ง 4 ก.ย. 2569)

คำสั่งชุดนี้มา 3 ท่อนติดกัน แล้วท่อนหลังทับท่อนหน้า:
1. *"ทำกล่องให้มันเท่ากัน"* → `items-stretch` + `h-full` (วัดก่อนแก้ 342 vs 391 · หลังแก้ 391 เท่ากัน)
2. *"Logo So ทำพื้นหลังเป็นโปร่งใสเลย"* → `BrandMark` ถอด `bg-card border` ออกทั้ง `img` และทรง hero
   ⚠️ **แต่พื้นขาวไม่หาย** เพราะ **ไฟล์โลโก้เองมีพื้นทึบฝังมา** (ทานด้วย canvas:
   มุมภาพ = `rgba(240,243,250,255)` alpha เต็ม) — CSS แก้ไม่ได้ ต้องอัปโหลดไฟล์โปร่งใหม่
3. *"เอา `<div class=...>` (การ์ดผู้สมัคร) ออกได้นะ แล้วแบ่งครึ่งซ้ายขวา ขวาเป็น Login
   ซ้ายเป็นแบบอธิบายระบบ อธิบายแบบด้วยลูกเล่นที่เท่ ๆ"* ⇒ **ทับคำสั่ง 2 ก.ย.**
   ที่เคยสั่งว่าการ์ด "ดูประกาศงาน" ต้องมีอยู่

**โครงใหม่:** `grid lg:grid-cols-2` — ซ้าย `SystemIntro` · ขวาการ์ดล็อกอิน
🔴 **ทางเข้าของคนหางานห้ามหาย** — ย้ายเป็นลิงก์ใต้ฟอร์ม ("ไม่ได้เป็นพนักงาน? มาสมัครงานได้เลย")

**ลูกเล่นที่ใส่ (ไม่มี CSS ใหม่สักบรรทัด ตามกฎ 4 ก.ย.):**
* คำหัวเรื่องสลับวน 3 คำทุก 2.2 วิ (จาง+เลื่อนขึ้น) ด้วย state + `transition`
* สายพาน 4 ขั้น **ไฮไลต์ไล่ลงมา**ทุก 1.6 วิ + ไล่โผล่ทีละอันด้วย `animate-in` + `delay-*`
* จุดสถานะเต้นด้วย `animate-pulse`
* เคารพ `prefers-reduced-motion` — เปิดไว้ = อยู่นิ่ง (กติกาเดิมของโปรเจกต์)

⚠️ **ฝั่งซ้ายต้องมีพื้นกระจกบาง ๆ** — ตัวหนังสือเขียวเข้มบนภาพป่าอ่านไม่ออก (วัดบนจอ)
ใช้พื้นจางกว่าการ์ดล็อกอินเพื่อไม่ให้กลายเป็น "สองการ์ด" อีก

**ด่านตรวจ:** test 2,671 ผ่าน · tsc 4 = 0 · eslint 0 err · build ผ่าน ·
ตรวจบนจอ 1440px: สองคอลัมน์ 467/467 · คำสลับจริง (จับได้ 3 ค่าไม่ซ้ำ) · ไฮไลต์เดินจริง
(ขั้น 0→1→3) · มือถือ 375px: ซ้อนกันเป็นคอลัมน์เดียว ไม่มีสกรอลล์แนวนอน
📌 ตอนวัดแอนิเมชัน **แถบเบราว์เซอร์ที่ซ่อนอยู่จะหน่วง timer** — ต้อง `tabs_select`
ให้เห็นจอก่อน ไม่งั้นวัดได้ว่า "ไม่ขยับ" ทั้งที่โค้ดถูก

## รอบ 105 — หน้าเข้าสู่ระบบเปลี่ยนจานสี + หุ่นยนต์ผู้ช่วยไปอยู่หน้าหลัก (5 ก.ย. 2569)

เจ้าของสั่ง: *"หน้า Login เอาแบบเดิม มีตัวหนังสือก่อน ตามด้วยหน้า Login แต่หน้า Login
กลับไปพื้นหลังเดิม ที่เปลี่ยนโทนเป็น ขาว+navy+burgundy เล็กน้อย ส่วนหุ่นยนต์เอาไปใส่ใน
หน้าหลัก แล้วเอาพื้นหลังหุ่นออก จะได้ดูไม่เอามาวางเฉย ๆ ดูมีชีวิตขึ้นหน่อย"*

| ไฟล์ | ทำอะไร |
| --- | --- |
| `src/lib/designTokens.ts` | `LOGIN_SCENE` เป็นจานขาว·กรมท่า·เบอร์กันดี (`canvas` `glass` `line` `ink` `navy` `muted` `burgundy` `burgundy2` `accent` `auraNavy` `auraBurgundy`) |
| `src/pages/LoginPage.tsx` | ฉาก **ภาพป่าเดิม** (`<PhotoScene />`) ไม่ถูกแตะ · เปลี่ยนแค่สีที่ทาบ: ตัวหนังสือกรมท่า ปุ่มเบอร์กันดี การ์ดกระจกขาว · ตัวหนังสือที่วางบนภาพมีรัศมีขาวหนุน (`TEXT_ON_PHOTO`) ไม่งั้นจมเงาต้นไม้ · มือถือสลับให้ฟอร์มมาก่อนด้วย `order` |
| `src/components/shared/PhotoScene.tsx` | คืนสภาพเดิม (ถอด prop `tone` ที่เคยทาบผ้ากรมท่าทับภาพ — ทึบจนอ่านยาก) |
| `src/components/home/CommandDeck.tsx` | เพิ่ม `RobotMascot` เป็น **คอลัมน์ที่สามของ hero** (ขึ้นเฉพาะจอ `xl`) · ลอยขึ้นลงเบา ๆ ด้วย framer-motion · เครื่องที่ตั้งลดการเคลื่อนไหวใช้ภาพนิ่ง |
| `public/robot-mascot.webp` `public/robot-mascot.png` | หุ่นยนต์ **ลบพื้นหลังออกจากตัวไฟล์แล้ว** (WebP เคลื่อนไหวมีช่องโปร่งใส 0.66 MB + ภาพนิ่ง) · `public/login-robot.mp4` เดิม (2.9 MB พื้นขาว) ถูกลบทิ้ง |

**วิธีลบพื้นหลัง (ถ้าต้องทำซ้ำ):** ตัวหุ่นเป็นสีขาวบนพื้นขาว ⇒ คีย์ด้วยสีอย่างเดียว
**รั่วเข้าตัวหุ่น** ต้องใช้สองเงื่อนไขคู่กัน — *ขาวจัด* **และ** *ไม่มีการไล่สีรอบตัว*
(พื้นหลังเรียบสนิท ส่วนตัวหุ่นมีแสงเงา) แล้วเติมสีจากขอบภาพ + เก็บเฉพาะก้อนใหญ่สุด
สคริปต์อยู่ที่ scratchpad ของรอบนั้น (`key3.py`) ใช้ ffmpeg จาก `npx ffmpeg-static`

🔴 ป้ายอวดยอดผู้ใช้/รูปหน้าคนในสเปกที่เจ้าของส่งมา **ไม่เอาขึ้น** — เป็นข้อมูลปลอม

## รอบ 106 — แผงซ้ายหน้าเข้าสู่ระบบ พูดตรงกับเมนูจริง (5 ก.ย. 2569)

เจ้าของทวง: *"ฝั่งซ้ายไม่ได้คิดหรือต่อยอดอะไรเลยหรอ"*

| ไฟล์ | ทำอะไร |
| --- | --- |
| `src/pages/LoginPage.tsx` | `SystemIntro` ดึง **ชื่อ · คำอธิบาย · ไอคอน** ของสี่ขั้นจาก `CONVEYOR_STEPS` (เมนูจริงหลังล็อกอิน) แทนคำโฆษณาที่เขียนมือ · ลูกเล่นเป็นรางสายพานที่ไฟไล่ลงมา (เส้นเติมตามความสูง + ชิปไอคอนติดสว่าง) · หัวเรื่องไม่มีกรอบ ใช้ผ้าไล่เฉดขาวที่ขอบบนแทน |
| `tests/api/loginIntroPanel.test.ts` | ด่านใหม่ — ห้ามกลับไปเขียนลิสต์ขั้นด้วยมือ (`INTRO_STEPS`) และห้ามยิง `/api/public/jobs` จากหน้าที่ยังไม่ล็อกอิน |

**ทำไมถึงดึงจากเมนู:** แก้ที่เดียวได้ทั้งสองที่ และหน้า Login กลายเป็นที่สอนเมนู —
ไอคอนที่เห็นตอนรอเข้าระบบคือไอคอนที่ต้องกดจริง · ของเดิมเพี้ยนไปแล้ว (หน้า Login เขียน
...
## รอบ 108 — รื้อหน้าตา เฟส 1+2: ของกลางโฉมใหม่ + หน้าแรกหลังสวิตช์ (5 ก.ย. 2569)

เจ้าของสั่ง *"เริ่มตามแผนเลย"* หลังเคาะแผน `docs/plan-ui-overhaul-2569-09-05.md`

| ไฟล์ | ทำอะไร |
| --- | --- |
| `src/lib/uiV2.ts` (ใหม่) | สวิตช์โฉมใหม่ — **ค่าตั้งต้นปิด** · เปิด `?ui=v2` ปิด `?ui=v1` · จำใน localStorage ของคนที่กดเท่านั้น |
| `src/components/shared/ui-v2/Sheet2.tsx` (ใหม่) | ของกลางเฟส 1: `Sheet2` (ผืนเดียว) · `SheetHead2` · `Rule2` (เส้นคั่น) · `StatRow2`/`Stat2` (แถวตัวเลขทรงเดียว) |
| `src/components/home/HomeDeckV2.tsx` (ใหม่) | หน้าแรกโฉมใหม่ — รับ props ชุดเดียวกับ `CommandDeck` และแสดงข้อมูลครบทุกชิ้น |
| `src/components/home/TeamBoardPanel.tsx` | เพิ่ม prop `skin` (`deck` = เดิม ค่าตั้งต้น · `plain` = ผืนขาวโฉมใหม่) — ข้อมูลข้างในไม่ถูกแตะ |
| `src/pages/HomePage.tsx` | สลับ deck ตามสวิตช์ · ป้อน props ชุดเดียวกันทั้งสองโฉม |
| `src/lib/brandingStorage.ts` | `DEFAULT_BRANDING` เปลี่ยนเป็นจานใหม่ (เบอร์กันดี `353 50% 37%` · หมึกกรมท่า `220 54% 15%`) |
| `tests/api/homeDeckV2Parity.test.ts` (ใหม่) | ด่าน "ข้อมูลต้องครบ" — 26 เคส ไล่ทีละชิ้นว่าโฉมใหม่ไม่ทำอะไรหาย |

**ตรวจบนจอจริงแบบวัดได้:** ดึง `innerText` ของหน้าแรกทั้งสองโฉมมาเทียบเป็น multiset
⇒ **ตัวเลขที่โฉมเดิมมี โฉมใหม่มีครบทุกตัว** (`numMissing: []`) · ต่างกันเฉพาะถ้อยคำ/รูปแบบ
ที่ตั้งใจเปลี่ยน (ป้าย "SO RECRUIT"/"สถานะสด" → "งานถัดไปของคุณ"/"อัปเดตล่าสุด HH:MM")

🔴 **ค้นพบระหว่างทาง — จานสีจริงถูกคุมโดย "ตั้งค่า → แบรนด์" ไม่ใช่แค่ `index.css`**
`src/lib/brandingStorage.ts` เขียน inline style ทับตัวแปรธีมบน `<html>`
ค่าที่บันทึกไว้ใน production ตอนนี้คือ **แดง `0 72% 50%` + หมึกเทา `0 0% 12%`**
⇒ ปุ่ม/วงแหวนโฟกัส/พื้น hover ทั้งระบบยังเป็นแดง ไม่ใช่เบอร์กันดี
**ไม่แตะข้อมูลของ production เอง** — รอเจ้าของสั่งเปลี่ยนที่หน้าตั้งค่า

## รอบ 109 — รื้อหน้าตา เฟส 3-8 (ทั้งหมดอยู่หลังสวิตช์ `?ui=v2`) · 5 ก.ย. 2569

| ไฟล์ | ทำอะไร |
| --- | --- |
| `src/components/shared/ui-v2/surface.ts` (ใหม่) | ชุดคลาสพื้นผิว — แปลระหว่าง "แผงพื้นเข้มเดิม" กับ "ผืนขาวโฉมใหม่" ที่เดียว |
| `src/components/shared/PageHeroStrip.tsx` | โฉมใหม่ = แถบขาวคั่นเส้น (ของเดิม = ผืนกรมท่าทึบ) |
| `src/components/ui/button.tsx` | โฉมใหม่แปลง `hero`/`heroSolid` → `outline`/`default` อัตโนมัติ (แถบหัวเป็นพื้นขาวแล้ว) |
| `src/components/shared/StatCard.tsx` | โฉมใหม่ = การ์ดขาว เลขใหญ่ ไม่มีพื้นพาสเทล 6 สี (ใช้ 5 หน้า) |
| `src/components/matching/AiCallFlowPanel.tsx` | 16 ช่องผลโทรอ่านออกบนพื้นขาว (สีความหมายเดิม) |
| `src/components/follow/CallFunnelPanel.tsx` | funnel + ชิป "ทำก่อน→หลัง" ใช้ชุดสีพื้นผิว |
| `src/components/dashboard/analytics/DashboardHeroStrip.tsx` | แถบหัวแดชบอร์ดเป็นผืนขาว + เลขความเร่งด่วนมีชุดสีพื้นขาว |
| `src/components/jobs/RecruitBoardTools.tsx` | ปุ่มบนแถบหัวสลับทรงตามโฉม |
| `src/index.css` | **ลบ CSS ฉากห้องทำงาน 3D ที่ตายแล้ว ~490 บรรทัด** |
| `tests/api/uiV2Rollout.test.ts` (ใหม่) | ด่าน 13 เคส — ทุกไฟล์ที่เปลี่ยนหน้าตาต้องอ่านสวิตช์ก่อน · ของเดิมห้ามหาย · CSS ตายห้ามค้าง |

⏸️ เฟส 7 (หน้าสาธารณะ) **ตั้งใจไม่ทำ** — ผู้สมัครภายนอกไม่มีทางใส่ `?ui=v2`
ทำหลังสวิตช์ก็ไม่มีใครเห็น · ทำแบบไม่มีสวิตช์ = แตะ production ตรง ๆ ⇒ รอเจ้าของเคาะ

## รอบ 110 — ออกแบบใหม่จริงรอบแรก: หน้า "คิวงานของฉัน" (5 ก.ย. 2569)

เจ้าของตีกลับรอบรื้อก่อนหน้าว่า *"ไม่ได้มีอะไรใหม่เลย ก็ที่ฉันเคยทำไว้ทั้งนั้น"* — ถูกต้อง
รอบนั้นคือ **ทาสีใหม่ + ถอดของประดับ** โครงหน้าเดิมทุกหน้า · รอบนี้เจ้าของเลือกทิศทาง
**"หน้าเดียวจบงาน"** (จาก Choice 4 ทาง)

| ไฟล์ | ทำอะไร |
| --- | --- |
| `src/pages/work/WorkQueuePage.tsx` (ใหม่) | คิวงานซ้าย + ลิ้นชักงานขวาในหน้าเดียว · แถวเป็นเส้นบางไม่มีกล่อง เลข 01–04 นำแถว · ปุ่มจองตัวทำงานจริงในหน้านี้ |
| `src/App.tsx` | route `/work` (ไม่เปิดสวิตช์ = `<Navigate to="/">`) |
| `src/components/layout/AppNavDrawer.tsx` | เมนู "คิวงานของฉัน" โผล่เฉพาะคนที่เปิด `?ui=v2` — โครงเมนู Burger เดิมไม่เปลี่ยน |
| `src/lib/brandingStorage.ts` | 🔴 เลิกทับ `--accent` ด้วยสีแบรนด์ — `--accent` คือ **พื้น hover** ของ shadcn ไม่ใช่สีแบรนด์ (ของเดิมทำให้ชี้เมนู/แถวที่เลือก กลายเป็นบล็อกสีทั้งก้อน) |
| `tests/api/workQueuePage.test.ts` (ใหม่) | 8 เคส: v2-only · เมนูตามสวิตช์ · **ค่าที่ป้อน buildNextTasks ต้องตรงกับหน้าแรกทุกช่อง** · ห้ามยิง API ใหม่ · ปุ่มจองใช้เส้นเดิม |

**สองชนิดของแถวในคิวเดียวกัน** (นี่คือของใหม่จริง): `bucket` = กองงานทั้งถัง (เปิดหน้าเต็ม)
· `person` = รายคนที่ตัดสินใจได้ทันที (จองตัว/โทรเอง) — คนกดจบงานได้โดยไม่ต้องเปลี่ยนหน้า

⚠️ ตัวเลขทุกช่องมาจาก `flow-summary` + `office-floor` ที่หน้าแรกโหลดอยู่แล้ว **ไม่มีเส้นใหม่**

## รอบ 112 — Wave 1 ของแผนดันคะแนนใช้งาน: คำศัพท์ + ป้ายว่าง + ปุ่มบอกผล (5 ก.ย. 2569)

เจ้าของเคาะตารางคำใน `docs/plan-quality-100-2569-09-05.md` (Wave 1) — งานนี้เปลี่ยน
**ข้อความบนจอเท่านั้น ไม่แตะนิยาม/ตรรกะของตัวเลข**

| ไฟล์ | ทำอะไร |
| --- | --- |
| `src/lib/nextTask.ts` | badge/reason ถัง SLA "ยอดสะสม" → "ค้างสะสมทั้งระบบ" · title ถัง `claimed-idle` "เก็บไปโทรเองแล้วเงียบ" → "รับไปโทรเองแล้วยังไม่บันทึกผล" |
| `src/pages/work/WorkQueuePage.tsx` | ป้ายสถิติ "รวมของค้างทุกถัง" → "งานค้างรวมทั้งระบบ" |
| `src/lib/followRoundVisual.ts` | `roundTabLabel()` — **แหล่งเดียว** ของป้ายรอบโทร "สายที่ N" → "รอบโทรที่ N" |
| `src/components/follow/FollowCallRoundsPanel.tsx` · `FollowPlanningCalendar.tsx` · `RoundScriptNote.tsx` · `src/pages/follow/FollowPage.tsx` | เดิมมีบางจุด hardcode คำว่า "สายที่ N" ซ้ำแทนที่จะเรียก `roundTabLabel()` — รีแฟกเตอร์ให้เรียกฟังก์ชันเดียวกันหมด เพื่อไม่ให้จอไหนตกหล่นตอนเปลี่ยนคำอีก |
| `src/components/matching/AiCallFlowPanel.tsx` | หัวกลุ่มช่อง "สายอยู่ตรงไหน" → "ตอนนี้สายไปถึงขั้นไหน" · "คุยแล้วผลเป็นยังไง" → "ผลจากคนที่คุยแล้ว" · คำใบ้ใต้ช่อง "กดดูรายชื่อ" → "กดดูรายชื่อ (เปิดในหน้านี้)" |
| `src/lib/aiCallFlowCells.ts` | อัปเดตคอมเมนต์ JSDoc ให้ตรงคำใหม่ (ชื่อ `group` ในโค้ดยังคงเดิม) |
| `src/components/dashboard/analytics/DashboardShell.tsx` | หัว `Request Control Tower` → หัวหลักไทย "ศูนย์ควบคุมใบขอ" + คง EN เดิมเป็นบรรทัดรองตัวเล็กจาง |
| `src/lib/unitRequestDisplay.ts` | `unitRequestCardSubtitle()` / `jobBoardCardSubtitle()` — ค่า `request_action_name` จริงจาก ERP เติมคำนำหน้า "สาเหตุที่ขอ: " (ค่าจริงไม่แปลง) · ตอนถอยไปใช้ `JOB_TYPE_LABELS` (เป็นประเภทงาน ไม่ใช่สาเหตุ) **ไม่ติด** คำนำหน้านี้ |
| `src/components/recruit-rm/RmWorkspace.tsx` | ปุ่ม "ส่ง AI โทร" (เอกพจน์ ไม่มีตัวนับต่อท้าย ในแถวรายคนของกอง "เลือกวิธีโทร") → "ส่ง AI โทร (ยิงสายจริง)" |
| `src/pages/aftercare/AftercarePage.tsx` | หน้าว่าง (0 คน) ใส่ข้อความ 3 ชั้นแทนความว่าง: หัว "ยังไม่มีใครเข้าช่วงดูแลหลังเริ่มงาน" · อธิบายที่มา · ปุ่ม "เพิ่มคนจากบอร์ด ERP" เด่นขึ้น + ลิงก์ตัวหนังสือใหม่ "ไปดูคนที่กำลังติดตาม →" ไป `/follow` |
| `src/components/jobs/JobBoardView.tsx` | มือถือ: แถวเงินเดือน/วันที่/สัญชาติเจ้านาย และแถวสวัสดิการ พับเหลือแถวเดียวเลื่อนได้ (`flex-nowrap overflow-x-auto` ที่ไม่มี `sm:`) แทนพับหลายบรรทัด — การ์ดสั้นลง ปุ่ม "สมัครงาน" เห็นเร็วขึ้นโดยไม่ตัดข้อมูล |
| `src/lib/followRoundVisual.test.ts` | แก้ค่าคาดหวังของ `roundTabLabel()` ตามคำใหม่ พร้อมคอมเมนต์อ้างเจ้าของเคาะ 5 ก.ย. 2569 |

⚠️ **ปุ่มที่ข้อความชนกับตัวนับ** ("ส่ง AI โทร (ติ๊ก N)" ที่ `LumosPanels.tsx` ·
"ส่ง AI โทร (N)" ที่ `RmSearchBar.tsx` และ `JobRecallSuggestions.tsx`) **ไม่แตะ** —
ใส่ "(ยิงสายจริง)" ซ้อนวงเล็บกับตัวนับจะอ่านไม่รู้เรื่อง ตามกติกาที่เจ้าของวางไว้
(คงของเดิม + จดไว้ในรายงาน ไม่ใช่เดาทางแก้เอง)

⚠️ **จุดที่เจอ `request_action_name` ลอย ๆ แต่ไม่แตะ** เพราะไม่ใช่ "การ์ดใบขอ/การ์ดหน้าสาธารณะ"
ตามสเปก: `src/pages/jobs/JobDashboard.tsx` (ชิปรายการล่าสุดของหน้าแดชบอร์ดหน่วยงาน)
และ `src/pages/jobs/JobListPage.tsx` (คอลัมน์ตาราง + การ์ดในหน้ารายการ) — รอเจ้าของสั่งถ้าต้องการให้ตรงกัน

## รอบ 113 — Wave 2 ของแผนดันคะแนนใช้งาน: มือถือ + หน้าที่คะแนนต่ำสุด (5 ก.ย. 2569)

ต่อจากรอบ 112 · แผนเต็มอยู่ที่ `docs/plan-quality-100-2569-09-05.md` (Wave 2)
งานนี้ **แตะ layout + คำกำกับเท่านั้น — ไม่มีบรรทัดไหนแตะตรรกะ/นิยามตัวเลข**
กติกาที่ยึด: ข้อมูลห้ามหายแม้แต่ตัวเดียว (ยุบ/พับ/เรียงใหม่ได้ แต่ทุกเลขต้องยังเข้าถึงได้)

| ไฟล์ | ทำอะไร |
| --- | --- |
| `src/components/follow/FollowCallRoundsPanel.tsx` | **2.1** การ์ด "รอบโทรที่ 1/2/3" จอ < sm เรียงตั้ง 1 คอลัมน์ (ป้าย+เลขบรรทัดเดียว การ์ดเตี้ย) · ช่องถัง 7 ช่อง จอ < sm เป็น 2 คอลัมน์ (เดิม 4 ช่อง/แถว ป้ายถูกตัดเป็น "ทั้งห…") · คำอธิบายยาว "รอบโทรที่ 1/2/3 = …" พับหลังไอคอน (?) ด้วย `Popover` ของ shadcn เฉพาะมือถือ (เดสก์ท็อปโชว์เต็มเหมือนเดิม) — ข้อความอยู่ที่ค่าคงที่ `ROUND_HELP_TEXT` **ที่เดียว** ใช้สองจอ · แถวปุ่มมุมขวาบนเลิก `shrink-0` (เดิมบน 375px ปุ่มตัวท้ายถูกตัดออกนอกกรอบ กดไม่ได้เลย) |
| `src/components/follow/FollowPlanningCalendar.tsx` | **2.1** แถบคำอธิบายสัญลักษณ์ (legend จุดสี) `text-[10px]` → `text-xs` · มือถือเรียง 2 คอลัมน์ (`grid` → `sm:flex`) จุดสี `shrink-0` ไม่ให้บี้ตอนป้ายขึ้นสองบรรทัด |
| `src/components/matching/AiCallFlowPanel.tsx` | **2.2** `WHERE_GRID` จาก `grid-cols-3` คงที่ → `grid-cols-2 sm:grid-cols-3` (จอ 375px เดิมป้ายถูกตัดเป็น "กำลังโทร / ถึ…") · จำนวนช่อง/ตัวเลข/ปุ่มกดดูรายชื่อ เท่าเดิมทุกช่อง |
| `src/pages/matching/MatchingPage.tsx` | **2.2** `CANDIDATE_ACTION_BUTTON_CLASS` `min-h-8` → `min-h-9 sm:min-h-8` · ชิปตัวกรอง (ทั้งหมด/งานด่วน/มีคนแนะนำ/ไม่มีคนแนะนำ) เพิ่ม `min-h-9 sm:min-h-0` — วัดจริงบนจอ 375px เดิม 32–34px แตะยาก |
| `src/components/matching/SelectionRecallButton.tsx` | **2.2** ปุ่ม "หาคนจากกองไม่สนใจ" (32px) เพิ่ม `min-h-9 sm:min-h-0` |
| `src/components/dashboard/analytics/DashboardShell.tsx` | **2.3** แผงตัวกรองบนมือถือ (`useIsMobile()` = < 768px) พับด้วย `Collapsible` ของ shadcn **ค่าตั้งต้นปิด** — เดสก์ท็อปวางตรง ๆ เหมือนเดิม (แผงเดียวกัน ตัวแปร `filterBar` ไม่โคลนโค้ด) · ตอนโหลดครั้งแรกใส่ `Skeleton` ของ shadcn ทรงเดียวกับของจริง (ตัวกรอง + hero + KPI 4 ใบ + กราฟ) แทนจอเปล่า **ไม่มีตัวเลขปลอมสักตัว** — บรรทัด "กำลังโหลดข้อมูล…" ยังอยู่ |
| `src/pages/work/WorkQueuePage.tsx` | **2.4** ช่อง "งานค้างรวมทั้งระบบ" เติม `hint` = `นับจากคิวงาน — คนละเลขกับ "เหลือหา" ของศูนย์ควบคุมใบขอ` (ผู้ทดสอบตาใหม่เอาสองเลขนี้มาเทียบกันแล้วงง เพราะคนละนิยาม) — เติมคำกำกับอย่างเดียว ไม่แตะตัวเลข |

**หลักฐานว่าข้อมูลไม่หาย** (นับจาก `main.innerText` บนจอ 375px ก่อน–หลัง):
`/follow` 2873 ตัวอักษร / 91 ตัวเลข → 2700 / 88 — ส่วนต่างคือ **คำอธิบายยาวที่ย้ายเข้า Popover
ตามสเปก** (ในนั้นมี "1/2/3" = 3 ตัวเลข) กดไอคอน (?) แล้วอ่านได้ครบทุกคำ ·
`/matching/match` 21712 / 1141 → **21712 / 1141 เท่าเดิมเป๊ะ** · ไม่มีหน้าไหนล้นจอแนวนอน
(`scrollWidth = clientWidth = 375`)

⚠️ **ปุ่มไอคอน h-7/h-8 บนหน้า `/matching/match` ไม่มีอยู่จริง** — สเปกอ้างปุ่ม "⋯"
แต่วัดจริงบนจอแล้วไม่พบปุ่มไอคอนล้วนบนหน้านี้เลย (grep `h-7`/`h-8`/`size="icon"` ในไฟล์หน้า
และ component ที่มันเรียก = ไม่เจอ) · ของที่เล็กจริงคือ **ปุ่มมีข้อความ** สามชุดข้างบน จึงแก้ชุดนั้นแทน

## รอบ 114 — Wave 3.1 ของแผนดันคะแนนใช้งาน: ความเร็วครั้งแรก (5 ก.ย. 2569)

ต่อจากรอบ 113 · แผนเต็มอยู่ที่ `docs/plan-quality-100-2569-09-05.md` (Wave 3.1)
🔴 งานนี้ **ไม่แตะนิยาม/ตรรกะของตัวเลขแม้แต่ตัวเดียว** — แตะเฉพาะ "ไปเอาข้อมูลมาเมื่อไหร่"
กติกาสามข้อของสำเนาใบขอ (หัวไฟล์ `api/_lib/unitRequestCache.ts`) ยังครบทุกข้อ

| ไฟล์ | ทำอะไร |
| --- | --- |
| `api/_lib/unitRequestCache.ts` | **ข้อ 4 ใหม่ — ตอบของเก่าก่อนแล้วเติมของใหม่เบื้องหลัง (stale-while-revalidate)** · สำเนาเกิน `TTL_MS` (90 วิ) แต่ยังไม่เกิน `STALE_MAX_MS` (**10 นาที**) ⇒ ตอบสำเนาเดิมทันที + ยิงโหลดใหม่เบื้องหลัง · เกินเพดาน ⇒ กลับไปรอโหลดจริงเหมือนเดิม · เพิ่ม `source: 'stale-revalidating'` แยกจาก `'cache'` · **`fetchedAt` ยังเป็นเวลาที่ถามมาได้จริง** header `x-data-age-seconds` จึงไม่โกหกอายุ · เพิ่ม **รวมคิวโหลด (`startLoad`)** หนึ่งคีย์ต่อหนึ่งการถามระบบงานหลัก มีเพดานเวลา `REFRESH_DEADLINE_MS` (2 นาที) กันธงค้าง · `fresh: true` ไม่เกาะคิวใคร ต้องได้ของสดของตัวเอง · เพิ่ม `unitRequestRefreshCount()` / `settleUnitRequestRefreshes()` สำหรับเทสต์ |
| `api/_handlers/siamraj-unit-requests.ts` | แยกก้อนอ่านลิสต์ใบขอ (สำเนาชั้นนอก + ของแนบ 5 อย่าง) ออกมาเป็น `readUnitRequestListThroughCache()` **ที่เดียว** — handler กับตัวอุ่นสำเนาต้องเดินเส้นเดียวกันเป๊ะ ไม่งั้นอุ่นแล้วไม่ตรงคีย์ · เพิ่ม `warmUnitRequestListCache()` = อุ่น `limit 500` scope `{mode:'all'}` แบบ fire-and-forget (ย้ายโค้ดเฉย ๆ ไม่แตะรูปข้อมูล) |
| `server/local-api.ts` | เรียก `warmUnitRequestListCache()` **หลัง `listen()`** — เซิร์ฟเวอร์ที่อยู่ยาวเท่านั้น (Vercel serverless ไม่ได้ประโยชน์ อาศัย stale-while-revalidate แทน) · ไม่บล็อกการรับ request |
| `src/lib/siamrajUnitRequestsApi.ts` · `src/lib/boardDataState.ts` | เติม `'stale-revalidating'` เข้า union ของ `source` (ป้ายบนจอยังบอกอายุจริงตามเดิม — ตัวนี้ไม่ใช่ "ระบบงานหลักล่ม" จึงไม่ขึ้นข้อความเตือนแบบ `stale-after-error`) |
| `tests/api/unitRequestCache.test.ts` | เทสต์ใหม่ 8 เคส: ตอบ stale ทันที · อายุที่บอกต้องเป็นของเดิม · คนถัดไปได้ของใหม่ · in-flight ตัวเดียว · โหลดเบื้องหลังล้มแล้วสำเนาเดิมยังอยู่ · `fresh` ข้ามทางลัด · หลายคนขอพร้อมกันตอนสำเนาเย็น = ถามรอบเดียว · โหลดล้มแล้วทุกคนที่รอเห็นว่าพัง · error แบบ synchronous ไม่ทำให้ธงค้าง · **แก้ 2 เคสเดิม** ที่ยิง `now: 200_000` แล้วคาด `live` → เปลี่ยนเป็น `700_000` (เกินเพดาน) พร้อมคอมเมนต์บอกเหตุผล |

**ตัวเลขที่วัดได้จริงบนเครื่อง** (`curl` ผ่าน vite proxy · ล็อกอิน admin):

| เส้น | ก่อน (สำเนาเย็น) | หลัง (บูตใหม่ + อุ่นแล้ว) | หลัง (สำเนาหมดอายุ) |
| --- | --- | --- | --- |
| `/api/matching/flow-summary` | **4.6 วิ** | **0.23 วิ** | — |
| `/api/siamraj/unit-requests?limit=500` | **4.4 วิ** | **0.025 วิ** | **0.026 วิ** (เดิม 4.4 วิ) |
...
## รอบ 118 — ปิดตำหนิรอบสองจาก QA ภายนอก: ทำให้เห็น ไม่ใช่เขียนนิยามใหม่ (6 ก.ย. 2569)

ต่อจาก `docs/plan-quality-100-2569-09-05.md` (รอบสอง) · ธีมร่วม: คำอธิบายที่ถูกต้องมีอยู่แล้ว
แต่เล็ก/ไกลจากตัวเลข คนเลยไม่เห็น — ทุกข้อเป็นป้าย/ตำแหน่ง/spacing ล้วน ๆ
🔴 **ไม่แตะวิธีคำนวณตัวเลขแม้แต่บรรทัดเดียว** เหมือนรอบ 117

### ข้อ 1 — ความสัมพันธ์ "นับซ้อน" ต้องเห็นด้วยตา ไม่ใช่เชิงอรรถล่างสุด

ต่อยอดเชิงอรรถของรอบ 117 (ยังคงไว้เหมือนเดิม) — เพิ่ม prop `childOf` ให้ component `Row`
ใน `src/components/home/TeamBoardPanel.tsx`: เยื้องป้ายเข้า (`pl-3`) + ขึ้นต้นด้วย
`└ ในนั้น ` ก่อนป้ายจากพจนานุกรมเดิม (`recruit.untouched` label ไม่ถูกแก้ที่พจนานุกรม
กลาง `metricDictionary.ts` — ต่อคำแค่ตอน render ที่นี่ที่เดียว) ใช้กับแถว
`recruit.untouched` เท่านั้น (สับเซตของ `recruit.apps_uncontacted` ที่พิสูจน์ไว้แล้วรอบ 117)

### ข้อ 2 — คำเตือน "นับซ้ำ ห้ามบวก" ต้องติดหัวกลุ่มตัวเลข

`src/components/matching/AiCallFlowPanel.tsx` (component `CellRow`) — เพิ่มบรรทัดจิ๋ว
"นับซ้ำกันได้ ห้ามบวกรวม" ใต้หัว "ตอนนี้สายไปถึงขั้นไหน" ทันที (ทั้งฝั่ง AI และฝั่งคนเก็บไปโทรเอง
เพราะใช้ template เดียวกัน) · ประโยคเต็มท้ายแผงเดิมยังคงไว้ไม่แตะ

### ข้อ 3 — ปฏิทินติดตามมือถือ: touch target เล็ก + แยกช่องมีนัด/ว่างไม่ออก

`src/components/follow/FollowPlanningCalendar.tsx`:
- ปุ่มหัววัน (`th > button`) และปุ่มช่องที่มีรอบโทร (`td > button`) เติม
  `min-h-10 sm:min-h-0` (มือถือ ≥40px ตามที่เจ้าของสั่ง จริง ๆ วัดได้ 45px เพราะรวม padding ·
  เดสก์ท็อป `sm:min-h-0` คืนความสูงเดิมตามเนื้อหา วัดแล้ว 38px เท่าก่อนแก้)
- เพิ่มฟังก์ชัน `mobileSoftBg(tone)` — ดึงเฉพาะ token `bg-*`/`dark:bg-*` จาก `TONE[tone].soft`
  (ไม่แตะ token กลาง แค่กรองมาใช้บางส่วน) แล้ว prefix `max-sm:` ใส่ `<td>` ของช่องที่มีรอบโทร
  ให้พื้นจาง ๆ แยกจากช่องว่าง (ยังใช้ `bg-secondary/25`) เฉพาะจอเล็ก — เดสก์ท็อปไม่มี class
  `max-sm:*` ทำงาน จึงหน้าตาเดิมเป๊ะ (ยืนยันด้วย `getBoundingClientRect` จริง)

### ข้อ 4 — Dashboard: "คงเหลือ = ใบเปิดทั้งระบบ" ต้องติดการ์ด ไม่ใช่จมย่อหน้าบน

`src/components/dashboard/analytics/DashboardShell.tsx` — บล็อก "สรุปอัตราในช่วงที่เลือก"
เดิมมีย่อหน้ายาวอธิบายไว้ด้านบนกริด 4 การ์ด (เข้ามา/ปิดแล้ว/ยกเลิก/คงเหลือ) คนอ่านต้องกวาดตา
ขึ้นไปหา — ห่อการ์ดแต่ละใบด้วย `<div className="flex flex-col gap-1">` แล้วต่อท้ายเฉพาะการ์ด
`id === 'remaining'` ด้วยบรรทัดรอง "ทั้งระบบ ไม่ใช่เฉพาะช่วงที่เลือก" **เฉพาะตอน
`dateRange == null`** (กรณีเดียวที่คงเหลือคนละที่มากับสามตัวแรก — ตอนเลือกช่วงวันแล้ว
สมการเข้ามา−ปิด−ยกเลิก=คงเหลือ ลงตัวอยู่แล้วตามย่อหน้าอีกสาขาที่มีอยู่ก่อน ไม่ต้องเตือนซ้ำ)
ย่อหน้ายาวเดิมด้านบนยังคงไว้ไม่ตัด

### ข้อที่ข้ามตามคำสั่งเจ้าของ — ใบขอค้าง 858 วันไม่บอกเหตุผล

ไม่แก้ — เหตุผลที่ระบบตอบไม่ได้ว่า "ทำไมใบเก่ายังเปิด" คือข้อมูลนั้นไม่มีในฐาน (ต้องถามหน่วยงาน)
เขียนเหตุผลใส่จอเองไม่ได้ (จะกลายเป็นแต่งข้อมูล) — เจ้าของสั่งให้ข้ามข้อนี้ไว้ก่อน

### ทดสอบ

`npx tsc` ทั้ง 4 config (`tsconfig.json` / `tsconfig.app.json` / `tsconfig.node.json` /
`tsconfig.api.json`) ผ่านหมด · `npx vitest run` ครบ 250 ไฟล์ (2768 ผ่าน, 6 skip ของเดิม
ไม่มีเทสต์พังจากงานนี้) · `npx eslint .` 0 error (มีแต่ 18 warning เดิมก่อนแก้) ·
`npm run build` ผ่าน · ดูจอจริง `?ui=v2` ทั้ง desktop 1400x900 + มือถือ 375x812 ผ่าน
`javascript_tool` ยิง dev-role ผ่าน `getBoundingClientRect`/`get_page_text` ยืนยันครบ 4 ข้อ
(หน้าแรก/Matching/Follow/Dashboard) — สลับ "ทั้งหมด" ↔ "เดือนนี้" บน Dashboard ยืนยันเพิ่มว่า
บรรทัดข้อ 4 หายไปถูกต้องตอนเลือกช่วงวัน (ไม่ใช่ค้างโชว์ผิดเงื่อนไข)

## รอบ 119 — ฉากเปิด "SO RECRUIT" เด้งทุกครั้งที่เข้าหน้า Login (6 ก.ย. 2569)

เจ้าของสั่ง: *"ตอนเข้า Web ไม่เห็นคำว่า So Recruit เด้งก่อนแล้วค่อยไปหน้า Login"* — ของเดิม
(ตั้งแต่ 4 ก.ย. 2569) จำการดูไว้ใน `sessionStorage` คีย์ `jarvis.brandIntroSeen` เลยเห็นแค่
รอบแรกของการเปิดเบราว์เซอร์ ไม่ตรงกับที่เจ้าของต้องการ

`src/components/auth/BrandIntro.tsx`:
- ถอดกลไก `sessionStorage` ออกทั้งชุด — ลบค่าคงที่ `SEEN_KEY` และฟังก์ชัน `alreadySeen()` /
  `markSeen()` · ฉากเล่นทุกครั้งที่ component mount แล้ว (ไม่มีอะไรจำสถานะ "เคยดู" อีก)
- คงของเดิมไว้ทั้งหมด: ข้ามได้ทุกทาง (คลิก/คีย์/แตะ/สกรอลล์) · `pointer-events-none` ตลอด
  (ไม่กินคลิกฟอร์มข้างหลัง) · เครื่องที่ตั้ง "ลดการเคลื่อนไหว" ไม่โชว์ · จังหวะเวลาเดิม
  (พุ่งครบ ~0.62 วิ · จางหมด ~0.95 วิ)
- `?intro=1` เดิมมีไว้บังคับเล่นซ้ำ (ตอนยังจำ session) — ตอนนี้เล่นทุกครั้งอยู่แล้วไม่ต้อง
  บังคับเรื่องความถี่ จึงเปลี่ยนความหมายเหลือ **บังคับเล่นแม้เครื่องตั้งลดการเคลื่อนไหวไว้**
  (แก้ที่ `introParam()`/`forcedByUrl()` — ใช้แค่ bypass เงื่อนไข `matchMedia` เดียว)
  `?intro=hold` ความหมายเดิมไม่เปลี่ยน (ค้างฉากไว้ให้ดูสี/จัดวาง)
- คอมเมนต์หัวไฟล์อัปเดตเหตุผล: ความกังวลเรื่องน่ารำคาญจากรอบ 4 ก.ย. 2569 ตกไป เพราะพนักงาน
  ที่ล็อกอินค้างไว้แล้วไม่ผ่านหน้า Login นี้อยู่แล้ว (เห็นฉากเฉพาะตอนต้องกรอกฟอร์มจริง ๆ)

`src/components/auth/BrandIntro.test.tsx`:
- เคส "🔴 โชว์ครั้งเดียวต่อการเปิดเบราว์เซอร์ — รอบสองไม่ขึ้น" เปลี่ยนเป็น
  "🔴 เล่นทุกครั้งที่ mount — รอบสองก็ต้องขึ้นอีก" (mount สองรอบ ต้องเห็น
  `[data-brand-intro]` ทั้งสองรอบ)
- เคส `?intro=1` เดิมทดสอบว่าบังคับเล่นซ้ำได้แม้เคยดูแล้ว (ผูกกับ `SEEN_KEY`) — เปลี่ยนเป็น
  ทดสอบความหมายใหม่: mock `matchMedia` ให้ `prefers-reduced-motion: reduce` เป็นจริง แล้วยืนยันว่า
  `?intro=1` ยังบังคับให้ฉากขึ้นอยู่ดี
- เคสอื่น (ตัวอักษรครบ / pointer-events-none / กดคีย์จางทันที / ถอดตัวเองเมื่อจางจบ /
  ลดการเคลื่อนไหวไม่โชว์) คงไว้ไม่แตะ

### ทดสอบ

`npx tsc -p tsconfig.app.json --noEmit` + `npx tsc --noEmit` ผ่าน · `npx vitest run
...
## รอบ 121 — ทางเข้าหน้า "คิวงานของฉัน" (`/work`) ให้คนหาเจอ (7 ก.ย. 2569)

โจทย์มาจาก `docs/audit-v1-v2-functions-2569-09-07.md` §5 **งง-6** + §6 **ข้อเสนอที่ 7**:
หน้า `/work` มีมาตั้งแต่รอบ 110 แต่ **ไม่มีทางเข้าจากหน้าไหนเลยนอกจากเมนู burger**
⇒ คนที่ไม่เปิดเมนูไม่มีวันรู้ว่ามีหน้านี้ · และคนที่บังเอิญเจอก็ไม่รู้ว่าต่างจากคิวบนหน้าแรกยังไง

🔴 **v1 ไม่เห็นอะไรเปลี่ยนเลย** — ยืนยันด้วย `git diff --name-only`: ไม่มีไฟล์เส้นทาง v1
(`CommandDeck.tsx`) ถูกแตะ · ทางเข้าทั้งสองจุดอยู่หลังสวิตช์ `?ui=v2` ทั้งคู่

### สถานะก่อนเริ่ม — จุดที่ 1 ทำไว้แล้วตั้งแต่รอบ 110

`src/components/layout/AppNavDrawer.tsx:161-173` — รายการ "คิวงานของฉัน" กลุ่ม "งานของฉัน"
ถัดจาก "หน้าหลัก" ห่อด้วย `{uiV2 ? … : null}` **มีอยู่แล้ว ไม่ต้องแก้** (โครงเมนูเดิมไม่ถูกรื้อ
ตามที่เจ้าของสั่ง) ⇒ รอบนี้เหลือทำจุดที่ 2 อย่างเดียว + เพิ่มด่านคุมทั้งสองจุด

### ไฟล์ที่แก้

| ไฟล์ | แก้อะไร |
| --- | --- |
| `src/components/home/HomeDeckV2.tsx` | เพิ่มแถบปิดท้ายผืน: `<Rule2 />` + ประโยคอธิบาย + ปุ่ม `outlineStrong` ลิงก์ `/work` (ไอคอน `ListChecks` ตัวเดียวกับในเมนู ⇒ อ่านออกว่าเป็นปลายทางเดียวกัน) |
| `tests/api/workQueuePage.test.ts` | describe ใหม่ "ทางเข้าหน้า /work ต้องมีสองจุด และเห็นเฉพาะ v2" (+4 เคส · 8 → 12) |

### เหตุผลของตำแหน่งกับน้ำหนักสายตา

- **อยู่ท้ายผืน นอกเงื่อนไข `head` / `rest`** ⇒ วันที่คิวว่างเปล่าก็ยังหาทางเข้าเจอ
  (ถ้าไปแปะไว้ใต้ลิสต์ "คิวของคุณวันนี้" มันจะหายไปพร้อมลิสต์ตอน `rest.length === 0`)
- **ปุ่มรอง ไม่ใช่พระเอก** — `variant="outlineStrong" size="sm"` ไม่ใช่ `default`
  เพราะปุ่มเบอร์กันดีของหน้านี้ถูกจองไว้แล้วโดยปุ่ม "ไปทำงาน" ของงานถัดไป
  (กติกา "ปุ่ม primary ใบเดียวต่อหน้า") · มีเทสต์คุมว่านับ `<Button asChild>` ที่ไม่ระบุ
  `variant` ได้ **1 ตัวเท่านั้น** ⇒ เผลอเพิ่มปุ่มเบอร์กันดีใบที่สองเมื่อไหร่ เทสต์แดง
- **มีประโยคบอกว่ามันคืออะไร** (นี่คือครึ่งหลังของ งง-6 ที่ขาดไป ไม่ใช่แค่ลิงก์เปล่า):
  *"คิวงานของฉัน — คิวชุดเดียวกับข้างบน แต่รวมไว้หน้าเดียว กดแถวไหนแล้วทำต่อได้เลย
  ในหน้านั้น ไม่ต้องเปิดหลายหน้า"*

### ⚠️ ไม่ต้องเช็กธง `uiV2` ซ้ำในไฟล์ `HomeDeckV2.tsx`

ธงอยู่ที่ **หน้า** ไม่ใช่ในแผง — `src/pages/HomePage.tsx:357` เลือก `<HomeDeckV2>` เมื่อ
`uiV2` เท่านั้น ปิดสวิตช์ได้ `<CommandDeck>` ของเดิมซึ่งไม่มีลิงก์นี้ (แพตเทิร์นเดียวกับ
ทุกอย่างในไฟล์นี้ตั้งแต่รอบ 109) ⇒ เทสต์คุมที่ตัวแยกกิ่ง `/uiV2 \? \(\s*<HomeDeckV2/`
และคุมว่า `CommandDeck.tsx` **ห้ามมีสตริง `/work`** แทน

### เทสต์ที่เพิ่ม — `tests/api/workQueuePage.test.ts` (+4 เคส · 8 → 12)

- จุดที่ 1 · เมนู burger มีคำว่า "คิวงานของฉัน" + `go('/work')`
- จุดที่ 2 · `HomeDeckV2` มี `to="/work"` + ประโยคอธิบาย (regex คุมทั้งวลี ไม่ใช่แค่ลิงก์)
- 🔴 ทางเข้าอยู่หลังสวิตช์ — `HomePage` แยกกิ่ง `uiV2 ? (<HomeDeckV2` · `CommandDeck`
  ต้องไม่มี `/work`
- 🔴 ปุ่มเบอร์กันดีของหน้าแรกยังมีใบเดียว — ห้ามมี `variant="default"` และนับปุ่ม
  `<Button asChild>` ที่ไม่ระบุ variant ได้ 1

### ทดสอบ

`npx tsc --noEmit` ผ่าน · `npx vitest run` **250 ไฟล์ · 2784 ผ่าน** (6 skip ของเดิม ·
ก่อนแก้ 2780 ⇒ +4 คือด่านใหม่ของรอบนี้ ไม่มีเทสต์เดิมพัง) · `npx eslint` บนไฟล์ที่แก้
...
## รอบ 123 — กล่องทีมหน้าแรกโชว์ผลโทรตรง ๆ ไม่ต้องกดเข้าไปดู (7 ก.ย. 2569)

**ที่มา:** เจ้าของสั่งคำต่อคำ — *"หน้าหลัก ตามกล่องทีม บอกด้วยว่าโทรไปแล้วเท่าไหร่
สนใจลงงานอะไรยังไง ไม่เอาแค่คำว่า ผลการโทร แบบนั้นก็ต้องกดเข้าไปเพื่อดูอีก"*

### อาการเดิม

คอลัมน์ทีม Lumos บนบอร์ดทีมจบที่ปุ่มบรรทัดเดียว
`ผลโทรวันนี้ N สาย — เปิดดูรายชื่อ →` ⇒ อยากรู้ว่า **ผลเป็นอะไรบ้าง / ใครสนใจ /
สนใจลงงานไหน** ต้องเปิดป๊อป "ผลจากการโทร" อีกชั้นเสมอ

### ไฟล์ที่แก้

| ไฟล์ | แก้อะไร |
| --- | --- |
| `src/lib/homeCallDigest.ts` | **ใหม่ · pure** — `buildCallDigest()` ประกอบสรุปผลโทรจาก `FlowSummary` ที่หน้าแรกโหลดอยู่แล้ว (**ไม่ยิงเส้นใหม่ ไม่มีนิยามเลขใหม่**) · ย้าย `FOLLOW_UP_TONE` + `CALL_BOX_META` (ป้าย/โทน/ลำดับ 4 กล่อง) มาไว้ **ที่เดียว** · `interestedJobLine()` = บรรทัด "สนใจลงงานอะไร" |
| `src/components/home/TeamBoardPanel.tsx` | prop ใหม่ `callDigest` + `onOpenPerson` · `CallDigestBlock` วาดสรุปในคอลัมน์ Lumos: ผลกลับวันนี้/เดือนนี้ · ชิป 4 กล่องพร้อมยอด · ชื่อคนที่สนใจ + ตำแหน่ง·หน่วยงาน (กดชื่อ = เปิดรายละเอียดคน) · ปุ่มเปิดป๊อปเดิม |
| `src/pages/HomePage.tsx` | `buildCallDigest(flow)` แล้วส่งให้บอร์ด **เฉพาะตอน `uiV2`** · ป๊อป "ผลจากการโทร" เลิกพิมพ์ป้ายเอง (อ่าน `CALL_BOX_META`) และ **เลิกใช้ `items.length` เป็นยอด** → `callBoxCount()` + บรรทัด "แสดง N รายแรกจาก M" เมื่อลิสต์ถูกตัด · เหลือไว้ในไฟล์จอแค่ `CALL_BOX_ICON` |

### กติกาที่ยึด

* 🔴 **หนึ่งเมตริกหนึ่งนิยาม** — ทุกยอดอ่านผ่าน `callBoxCount()` / `callResultsThisMonth()`
  ของ `flowSummaryApi` เท่านั้น · บอร์ดทีมกับป๊อปใช้ **ตัวสร้างและป้ายชุดเดียวกัน**
  ⇒ สองที่บนจอเดียวกันโกหกกันเองไม่ได้
* 🔴 **ของใหม่อยู่หลังสวิตช์** — `uiV2` ปิด ⇒ `callDigest = null` ⇒ คอลัมน์ Lumos
  เป็นปุ่มเดิมเป๊ะ (v1 ไม่ขยับ) · **ป๊อปเดิมไม่ถูกถอด** ปุ่มท้ายบล็อกใหม่เปิดป๊อปเดิมได้
* **ไม่แตะ SQL** — `job_position` / `job_unit` ที่ใช้เขียนบรรทัด "สนใจลงงานอะไร"
  มีอยู่ใน `call_boxes` อยู่แล้ว (`enrich()` ของ `matching-flow-summary`)
  ใบที่หลุดขอบเขต ⇒ ถอยไปบอกเลขที่ใบขอ **ห้ามเดา/ห้ามแต่งชื่องาน**

### เลขบนจอเปลี่ยนจากอะไรเป็นอะไร

| ที่ | ก่อน | หลัง |
| --- | --- | --- |
| คอลัมน์ Lumos (v2) | `ผลโทรวันนี้ N สาย — เปิดดูรายชื่อ →` บรรทัดเดียว | ผลกลับวันนี้ · ผลกลับเดือนนี้ · ชิป 4 กล่อง (สนใจงาน/รอ AI โทรซ้ำ/ต้องเร่งจัดการ/ไม่สนใจงาน) · ชื่อคนสนใจ 4 คนแรก + งานที่สนใจ · "…และอีก N ราย" |
| ป๊อป "ผลจากการโทร" (ทั้ง v1/v2) | `สนใจงาน (50)` = `.length` ตันที่ 50 | `สนใจงาน (137)` จาก `call_box_counts` + `แสดง 50 รายแรกจาก 137` |
| "…และอีก N ราย" | — | อิงยอดจริง (137−4 = 133) ไม่ใช่ 50−4 |

### เทสต์ที่เพิ่ม

- `tests/api/homeCallDigest.test.ts` (19 เคส) — ยอดมาจาก `call_box_counts` ไม่ใช่ `.length` ·
  `interestedMore` อิงยอดจริง · API เก่าไม่มีตัวนับ ⇒ ถอยไปใช้ลิสต์ · `interestedJobLine`
  ไม่เดา · ด่านสแกนไฟล์: หน้าแรกไม่พิมพ์ป้ายเอง · ป๊อปไม่ใช้ `items.length` ·
  `callDigest` ส่งเฉพาะตอน `uiV2` · v1 ยังมีปุ่มเดิม · ป๊อปเดิมยังเปิดได้
- `src/components/home/TeamBoardPanel.test.tsx` (9 เคส · **render จริงด้วย jsdom**) —
...
## รอบ 126 — ปฏิทินติดตามยุบคนละคนเป็นแถวเดียว (7 ก.ย. 2569)

**อาการที่เจ้าของทัก:** *"โชว์ไม่ครบคนป้ะ"* — รอบโทรที่ 1 เดือน ก.ย. ในฐานมี **9 คน**
แต่ปฏิทินโชว์ **7 แถว**

**ต้นเหตุ:** `src/lib/followGrouping.ts` จัดกลุ่มด้วยคีย์ `เบอร์(9 ตัวท้าย)|เรื่อง`
**ไม่มีชื่อคนอยู่ในคีย์** ⇒ คนละคนที่ใช้เบอร์เดียวกัน + เรื่องเดียวกัน ถูกยุบเป็นแถวเดียว
ชื่อบนแถว = ชื่อของรอบที่สร้างล่าสุด (`name: latest.recipient_name`) คนอื่นหายเข้าป้าย "+N"

ของจริงที่วัดจากฐาน (7 ก.ย. 2569):

| เบอร์ | คนจริง | ปฏิทินโชว์ |
| --- | --- | --- |
| +66922511703 | เอาหูมาถูหมี (15:26) · จับหูมาถูหมี (15:30) · ยายสี บ่มักบักมี่ (15:35) | แถวเดียว "ยายสี บ่มักบักมี่" เวลา **15:26** (ของเอาหูมาถูหมี) |
| +66898143230 | นนท์ ธนนท์ (15:23) · "นิพนธ์ กอแก้ว" (15:30) | แถวเดียว "นิพนธ์ กอแก้ว" เวลา **15:23** (ของนนท์) |

🔴 **อันตรายกว่า "คนหาย" คือเวลานัดของคนหนึ่งไปแปะใต้ชื่ออีกคน** — ข้อมูลจริงจะโทรผิดคน

**แก้ (เจ้าของเคาะทางนี้):** เพิ่มชื่อผู้รับเข้าคีย์ → `เบอร์|เรื่อง|ชื่อ`
เทียบชื่อผ่าน `nameKey()` (ตัดช่องว่างหัวท้าย · ยุบช่องว่างซ้ำ · ตัวพิมพ์อังกฤษไม่สนใจ)
กระทบทั้ง **ปฏิทินติดตาม + ลิสต์หน้า Follow** (ใช้ `groupFollowEntries` ตัวเดียวกัน)

**ทางที่พิจารณาแล้วไม่เลือก:** จัดกลุ่มด้วย `group_id` — ข้อมูลจริง**ทุกแถว `group_id` เป็น null**
(โหมด "ระบุเวลาเอง" ไม่ได้เซ็ตให้) จัดกลุ่มด้วยตัวนี้จะยุบ 13 ชื่อเป็นก้อนเดียว แย่กว่าเดิม

**ผลข้างเคียงที่เจ้าของยอมรับแล้ว:** คนเดิมที่ **แก้สะกดชื่อ** จะกลายเป็นสองการ์ด
(เดิมสะกดล่าสุดชนะ) — แลกกับความเสี่ยงโทรผิดคน

**เทสต์:** `tests/api/followGrouping.test.ts` — แก้เคส "ชื่อ/หน่วยงานใช้ของรอบล่าสุด"
ให้ไม่พึ่งการยุบข้ามชื่อ · เพิ่ม 2 เคส: **ด่านกันบั๊กกลับมา** (เบอร์เดียวกันคนละชื่อ
ต้องเป็นคนละการ์ด และเวลานัดต้องอยู่กับเจ้าของตัวจริง) · ชื่อเดียวกันแต่เว้นวรรคเกิน/
ตัวพิมพ์ต่าง ต้องไม่แตกการ์ด

## รอบ 127 — ปฏิทินติดตามโชว์ผล "ทุกสาย" + สรุปจาก AI (7 ก.ย. 2569)

**เจ้าของสั่ง 2 ข้อต่อกัน:**
1. *"เวลาได้ผลจาก Lumos ถ้าตกลงไปให้ขึ้นสีเขียว และบอกว่าเขาตอบว่าอะไร ...
   และเอาสรุปผลโดย AI มาด้วย"*
2. *"ตอนนี้ถ้าเพิ่มไว้ 2 สาย ช่วยเอาผลมาทั้ง 2 สาย ตอนนี้ต้องรอสายที่ 2 ถึงจะรายงานผลมา"*

**ของเดิมมีอะไรอยู่แล้ว (ไม่ต้องแก้):** เขียว+เครื่องหมายถูกตอน `confirmed`/`acknowledged`
ทำงานถูกอยู่แล้วผ่าน `roundTone` → `CALL_OUTCOME_TONE` และคำบนจอใช้
`followCallOutcomeText` (confirmed = "ยืนยันว่าไป" · declined = "ยกเลิก — ไม่ไปแล้ว")

**สิ่งที่ขาดจริง 2 อย่าง:**
- `call_summary` (มาจาก `result->>'summary'` ของ Lumos) **มาถึงหน้าจอแล้วแต่ใช้แค่ในป๊อป**
  ⇒ ต้องกดเข้าไปทีละคนถึงจะรู้ว่าคุยว่าอะไร
- คอลัมน์เดิมชื่อ **"ผลสายแรก"** โชว์สายเดียว ⇒ ผลของสายที่ 2 ไม่มีที่ยืนในตาราง

**แก้:**
- `src/lib/followPlanning.ts` — เพิ่ม `roundAiSummary()` (trim แล้วว่าง = `null`
  ห้ามขึ้นกล่องเปล่าให้เข้าใจผิดว่า AI เงียบ) · **ลบ `firstCallOfRow()`** ที่ตายแล้ว
- `FollowPlanningCalendar.tsx` — ยุบคอลัมน์ "ผลสายแรก" เป็น
  **"ผลการโทรแต่ละสาย · สรุปจาก AI"** วนทุกสายของคนนั้น: `สาย N · เวลา` + ชิปผล
  (เขียว+✓ เมื่อจบดี · จาง+ขีดฆ่าเมื่อยกเลิก) + สรุปของ AI ใต้บรรทัด (`line-clamp-2`
  คำเต็มที่ tooltip) · มีผลแล้วแต่ AI ไม่เขียนสรุป → เขียน "(ไม่มีสรุปจาก AI)" ตรง ๆ
- prop `firstCalls` → **`allCalls`** (`Map<groupKey, รอบทั้งหมด>`)
- `FollowPage.tsx` — ส่ง `allCalls` จาก `planningRowsAllRounds`
  🔴 **ต้องเป็นชุดไม่กรองรอบ** ไม่งั้นสลับแท็บ "ครั้งที่ 2" แล้วสายที่ 1 หายจากคอลัมน์
- `cellTitle` ของช่องวัน ใส่สรุป AI ต่อท้ายแต่ละสาย + ขึ้นบรรทัดใหม่ให้อ่านออก

**เทสต์** `tests/api/followPlanning.test.ts` (+4): confirmed = เขียว + คำว่า "ยืนยันว่าไป" ·
declined = แดง (ห้ามเขียวเพราะแค่ "มีผลแล้ว") · `roundAiSummary` ว่าง/ช่องว่างล้วน = null ·
**สายที่ 1 ได้ผลแล้วต้องอ่านได้ทันทีโดยไม่ต้องรอสายที่ 2** และห้ามแต่งสรุปให้สายที่ยังไม่มีผล

⚠️ **หมายเหตุสำคัญ:** ตอนทำรอบนี้ **ยังไม่มีผลจริงจาก Lumos สักรายการ**
(ทุกแถวคิว `status=pending` · ช่อง reminder ไม่เคยถูกดึงเลย — ดูรอบ 125/บันทึกวันเดียวกัน)
⇒ คอลัมน์นี้ยังพิสูจน์บนจอจริงด้วยข้อมูลจริงไม่ได้ ทดสอบด้วยเทสต์เท่านั้น

## รอบ 128 — แยกคอลัมน์ "สายที่ 1 / สายที่ 2" พร้อมคำตอบ + เหตุผล (7 ก.ย. 2569)

**เจ้าของทักต่อจากรอบ 127:** *"ฉันหมายถึงแยกว่าสายแรก สาย 2 พอได้ผลก็แยกรอบ
เอามารวมกันแบบนี้งงตาย เช่น ตอนแรกบอก รอผลโทร ถ้ารอบแรกบอกไป ก็เปลี่ยนเป็น ตกลง
และคำตอบเขาคือ ไป ถ้าเขาไม่ไปก็บอก ไม่ไป เหตุผลที่เขาตอบ ไรงี้ ทั้ง 2 รอบ"*

รอบ 127 ทำเป็น **ช่องเดียวกองทุกสาย** (`<ul>` ในคอลัมน์เดียว) ⇒ อ่านไม่ออกว่าอันไหนของสายไหน

**แก้:** `FollowPlanningCalendar.tsx` — **หนึ่งสาย = หนึ่งคอลัมน์**
- `roundSlots` = สายที่มีจริงในข้อมูล (`followRoundSlot` ของทุกคน) · ไม่มีเลยถอยเป็น `[1]`
  🔴 นับจาก `allCalls` (ไม่กรองรอบ) ไม่ใช่ `rows` ที่กรองแล้ว — ไม่งั้นเลือกแท็บ
  "ครั้งที่ 2" ปุ๊บ คอลัมน์ "สายที่ 1" หายทั้งตาราง
- แต่ละช่อง: เวลา + ชิปผล (เขียว+✓ เมื่อตกลงไป · จาง+ขีดฆ่าเมื่อยกเลิก) +
  **เหตุผลที่เขาตอบ** จาก `roundAiSummary` ใต้บรรทัด (`line-clamp-3` · คำเต็มที่ tooltip)
- ไม่ได้ตั้งสายนั้น → เขียน "ไม่ได้ตั้งสายนี้" (คนละเรื่องกับ "ตั้งแล้วรอผล")
- มีผลแล้วแต่ AI ไม่เขียนสรุป → "(ไม่มีสรุปจาก AI)"

**เทสต์ใหม่** `src/components/follow/FollowPlanningCalendar.test.tsx` (render จริง 5 เคส):
2 สาย ⇒ มี columnheader "สายที่ 1"+"สายที่ 2" แยกกัน · **สายที่ 1 ได้ผลแล้วเห็นคำตอบ+เหตุผล
ทันทีทั้งที่สายที่ 2 ยังรอผล** และผลของสายที่ 1 ต้องไม่ไปโผล่ในช่องสายที่ 2 ·
ไม่ไป ⇒ ต้องมีเหตุผล · ไม่มีสรุป ⇒ บอกตรง ๆ · ตั้งสายเดียว ⇒ ไม่มีคอลัมน์สายที่ 2

### ยืนยันจากข้อมูลจริงว่า Lumos ส่งอะไรมาบ้าง (สืบวันเดียวกัน)

`result` ที่ Lumos POST กลับมามีคีย์: `tone, title, status, message, outcome, plan_id,
step_id, summary, event_id, language, step_type, stop_early, transcript, next_action,
plan_status, ended_reason, scheduled_at, call_attempts, recording_url, step_position,
recipient_name, recipient_phone, client_contact_id`

⇒ **`summary` มีจริงและเป็นภาษาไทยอ่านรู้เรื่อง** เช่น
*"ผู้รับสายแจ้งว่าท้องเสียและไม่ไปทำงานแล้ว"* · *"ผู้รับสายยืนยันว่าได้เดินทางไปถึง
หน่วยงาน SCB และทำงานแล้ว"* — คอลัมน์ใหม่จึงมีของจริงให้โชว์
📌 ยังมี **`transcript`** (บทสนทนาเต็ม role agent/candidate) และ **`recording_url`**
ที่ยังไม่ได้เอาขึ้นจอเลย — ของดีที่รอเจ้าของสั่ง

## รอบ 129 — สถานะเบอร์ฉุกเฉินขึ้นตาราง (7 ก.ย. 2569)

**เจ้าของสั่ง:** *"ต้องมีบอกด้วยว่าโทรหาเบอร์ฉุกเฉินยัง"*

**ตรวจก่อนทำ — ข้อมูลนี้ Lumos ยังไม่ส่งกลับมา** · คีย์ทั้งหมดใน `result` ที่ได้จริง
(ยืนยันจากฐาน production 7 ก.ย. 2569 · 7 แถวที่มีผล):
...
⇒ **ไม่มีช่องไหนบอกว่าโทร `admin_phone` แล้วหรือยัง** (ป๊อป `FollowRoundsDialog` สรุปแบบนี้
ไว้แล้วตั้งแต่ 2 ก.ย. — ตรวจซ้ำวันนี้ยังเหมือนเดิม)

**ทำได้แค่นี้ และทำแล้ว:** `roundEmergencyPhone()` ใน `followPlanning.ts` +
โชว์ใต้ผลแต่ละสายในตาราง
- มีเบอร์ + ได้ผลแล้ว → `ฉุกเฉิน <เบอร์> · ยังไม่รู้ว่าโทรหรือยัง`
- มีเบอร์ + ยังไม่ได้ผล → `ฉุกเฉิน <เบอร์> · แนบไปกับสายนี้`
- **ไม่มีเบอร์ → ชิปสีเตือน "ไม่ได้แนบเบอร์ฉุกเฉิน"** (ติดต่อผู้รับไม่ได้แล้ว AI ไม่มีใคร
  ให้โทรต่อ — เป็นความเสี่ยงจริง ไม่ใช่ช่องว่างเฉย ๆ)

**เทสต์** `FollowPlanningCalendar.test.tsx` (+3): บอกเบอร์+ยังไม่รู้ว่าโทรหรือยัง ·
ยังไม่ได้ผลบอก "แนบไปกับสายนี้" · ไม่มีเบอร์ต้องขึ้นเตือน · **ห้ามมีคำว่า "โทรแล้ว"**

🔴 **ของที่ต้องขอจาก Lumos ถึงจะตอบคำถามนี้ได้จริง:** ฟิลด์บอกว่าโทร `admin_phone`
แล้วหรือยัง (เช่น `admin_call_status` / `escalated_at`) — จนกว่าจะมี จอห้ามเขียนว่าโทรแล้ว

## รอบ 131 — ย้อน "ปฏิทิน 2 หน้า" ออก (7 ก.ย. 2569)

**เจ้าของสั่ง:** *"dd4eaf3 ย้อนกลับ ไม่เอาและไม่สวย"* ⇒ `git revert dd4eaf3` (= `b8eb0ec`)

🔴 **บันทึกไว้กันทำซ้ำ:** แนวทาง "แบ่งการ์ดปฏิทินติดตามเป็นแท็บ ต้องลงมือ / สรุปทั้งเดือน"
**เจ้าของไม่เอา** — เหตุผลที่ให้คือ *ไม่สวย* (ไม่ใช่เรื่องตรรกะหรือข้อมูลผิด)
ถ้าจะเสนอเรื่อง "หน้าสำหรับดูว่าวันนี้ต้องโทรกี่คน" อีก ต้องเสนอเป็น**หน้าตาแบบอื่น**
ห้ามกลับไปใช้แท็บ + ลิสต์แบบเดิม

**สิ่งที่ถูกถอดออกไปพร้อมกัน** (ถ้าต้องการกลับมาใช้ ดู `dd4eaf3` ใน git):
`buildFollowActionRows()` · `followActionSummary()` · เทสต์ 4 เคสของสองหน้า ·
`showMonthView()` ในไฟล์เทสต์

**สิ่งที่ยังอยู่ ไม่โดนย้อน** (รอบ 127-129 คนละ commit): คอลัมน์ผลแยกรายสาย ·
สรุปจาก AI · สถานะเบอร์ฉุกเฉิน

## รอบ 132 — ปฏิทินติดตามสองหน้า ฉบับที่ 2: รายวัน "สายที่ต้องตาม" / รายเดือน "ภาพรวม" (7 ก.ย. 2569)

**เจ้าของสั่ง (ฉบับละเอียด หลังย้อนฉบับแรกออกในรอบ 131):**
> หน้าแรก: บอกว่ามีกี่สายที่ต้องตาม · แยกผลของทุกสาย มี 3 สายก็แยกตาม Filter ·
> สายแรกโทรจบเอาผลมาบอก สีต้องบอกได้ว่า **เขียวคือตกลง เหลืองติดต่อไม่ได้ แดงคือไม่ไป** ·
> บอกด้วยว่าเขาตอบว่ายังไง · เลือกวันได้จากปฏิทิน
> หน้าสอง: ภาพรวมทั้งเดือน นาย ก ข ค ทั้งเดือนติดตามกี่ครั้ง วันไหนไป วันไหนไม่ไป วันไหนติดต่อไม่ได้
> *"ช่วยคิดและต่อยอดให้หน่อย ตอนนี้มันงงมาก"*

**ทำไมของเดิมงง:** การ์ดเดียวตอบสองคำถามด้วยตาราง 30 คอลัมน์ตารางเดียว — ตัวหนังสือ 8-9px ·
คำอธิบาย 6 สี · คอลัมน์ผลรายสาย (รอบ 127-129) ทำให้กว้างขึ้นอีก

### ชั้นนิยาม `src/lib/followPlanning.ts` (ใหม่)
- `callCategory(round)` → `agreed | unreachable | lost | waiting | overdue | notSent | cancelled | other`
  ตัดสินจาก `followRoundState` + `CALL_OUTCOME_TONE` **ไม่ประดิษฐ์นิยามใหม่** ·
  `FOLLOW_CALL_CATEGORY_LABEL` (คำของเจ้าของ: "ตกลง · ไป" / "ติดต่อไม่ได้" / "ไม่ไป") ·
  `FOLLOW_CALL_CATEGORY_TONE` **มีเทสต์บังคับว่าต้องเท่ากับ `roundTone`** ไม่งั้นเลขกับสีชิปคนละเรื่อง
- 🔴 "เลยเวลายังไม่มีผล" (`overdue`) **แยกจาก** "ติดต่อไม่ได้" (`unreachable`) โดยตั้งใจ —
  อันแรกยังไม่มีใครรับสายเลย อันหลัง AI โทรแล้วไม่ถึงตัว คนละงานที่ต้องทำต่อ (สีเหลืองทั้งคู่)
- `buildFollowDayCalls(rows, ymd, roundFilter)` หนึ่งแถว = หนึ่งสาย เรียงเวลา รวมที่ยกเลิก ·
  `roundSlotsOfDay` · `summarizeFollowCalls` (total ไม่นับยกเลิก) · `personMonthSummary`
- `filterPlanningRowsByRound` เลิกใช้ในแอป (เทสต์ยังคุมอยู่ ไม่ลบ)

### `FollowPlanningCalendar.tsx` — เขียนใหม่ทั้งไฟล์ เป็น `Sheet2` (ผืนเดียว ภาษา v2)
- หัว: eyebrow "ปฏิทินติดตาม" · stamp วัน/เดือน · `DayCalendarPicker` (ตัวเดียวกับตัวกรอง `fDate`
  ของหน้า — **ห้ามมีตัวเลือกวันสองตัว**) · แถวควบคุม: `Tabs` ของ shadcn สลับ
  **รายวัน · สายที่ต้องตาม / รายเดือน · ภาพรวม** + ลูกศรเลื่อนวัน(รายวัน)/เดือน(รายเดือน) + ปุ่มวันนี้/เดือนนี้
- **รายวัน (ค่าตั้งต้น):** แถวตัวเลข 6 ช่อง (ต้องตาม · ตกลง · ติดต่อไม่ได้ · ไม่ไป · รอผล ·
  เลยเวลายังไม่มีผล) 3×2 บนมือถือ · ชิปกรอง "ทุกสาย / สายที่ N" เฉพาะที่มีจริงในวันนั้น
  (ตามแท็บ `activeRound` ข้างบน**เฉพาะตอนมันเปลี่ยน** — พฤติกรรม 1 ก.ย. ยังอยู่) · รายการทีละสาย:
  เวลาใหญ่ · ชื่อ · หน่วยงาน · ป้าย "สายที่ N" · ชิปหมวดสีตามความหมาย (ต่อท้ายรายละเอียด
  เฉพาะที่เพิ่มข้อมูล: ติดต่อไม่ได้เพราะอะไร / "รับสายแล้ว") · **"เขาตอบ: <สรุป AI>"** ·
  เบอร์ฉุกเฉิน (พูดได้แค่แนบไปแล้ว ดูรอบ 129) · กดแถวเปิดป๊อปจัดการสายนั้น
- **รายเดือน:** คำอธิบายสีบรรทัดเดียวคำของเจ้าของ · คอลัมน์ชื่อที่ตรึงไว้มี **สรุปทั้งเดือนใต้ชื่อ**
  ("3 ครั้ง · ตกลง 1 · ไม่ไป 1 · ติดต่อไม่ได้ 1") · ช่องวันเป็นชิปสี + คำสั้นของหมวด
  🔴 สรุปเดือน**ต้องอยู่ในคอลัมน์ตรึง** — ฉบับแรกทำเป็นคอลัมน์แยก แล้วถูก auto-scroll ไปหาวันนี้
  เลื่อนหลุดจอ (เจอตอนดูจอจริง) · ถอดคอลัมน์ผลรายสาย (รอบ 128) ออก เพราะย้ายไปหน้ารายวันแล้ว
- `FollowPage.tsx` ส่ง `rows={planningRowsAllRounds}` (ไม่กรองรอบ) · ถอด prop `allCalls`

### ตรวจจอจริง (harness ชั่วคราว `harness-follow.html` + `src/dev-harness/` — **ลบแล้ว ไม่ commit**)
เดสก์ท็อป 1280 สว่าง/มืด · มือถือ 375 ทั้งสองหน้า — อ่านออกทุกขนาด ไม่มี error runtime
(error "Invalid hook call" ในคอนโซลรอบแรกเป็นของ Vite re-optimize deps ตอนเพิ่ม entry ใหม่ หายหลังรีโหลด)

### เทสต์
`tests/api/followPlanning.test.ts` (+9): หมวดครบทุกทาง · สีหมวด = สีชิป · รายวันเรียง/กรอง ·
สรุปไม่นับยกเลิก · สรุปรายคนรายเดือน
`FollowPlanningCalendar.test.tsx` (เขียนใหม่ 15 เคส · render จริง): เปิดมาเจอรายวัน · เลขหัวถูก ·
สาย 1 ตกลงเห็นทันทีขณะสาย 2 รอผล · ไม่ไป+เหตุผล · กรองสายที่ 2 แล้วเลขคิดใหม่ · กดแถวเปิดสายที่ถูก ·
วันว่างบอกทางไปต่อ · ยกเลิกเห็นแต่ไม่นับ · เบอร์ฉุกเฉินห้ามคำว่า "โทรแล้ว" · รายเดือน 31 หัวคอลัมน์ ·
สรุปใต้ชื่อ · ช่องวันคำสั้น · คำอธิบายสีคำของเจ้าของ
⚠️ Radix Tabs สลับด้วย `mousedown` ไม่ใช่ `click` — เทสต์ต้อง `fireEvent.mouseDown`

## รอบ 133 — หน้ารายวัน: แถว "ไม่ไป" พื้นแดงอ่อนทั้งแถว + ดันขึ้นบนสุด (8 ก.ย. 2569)

**เจ้าของสั่ง:** *"ทำดีนะ แต่หน้าแรกขอแบบนี้เพิ่ม คนไหนไม่ไปขอสีแดงอ่อน ๆ ในช่องนั้นไปเลย
แบบเปิดมารู้เลยว่านี่แหละไม่ไป และเรียงให้สีแดงอยู่บน ๆ เลยนะ"*

- `buildFollowDayCalls` (followPlanning.ts) — เรียง **`lost` ก่อนทุกหมวด** แล้วค่อยเรียงเวลา
  (คนบอกว่าไม่ไป = ต้องหาคนแทน/แจ้งหน่วยงานทันที ห้ามจมกลางลิสต์ตามเวลานัด)
- **token ใหม่ `TONE.*.wash`** ใน `src/lib/designTokens.ts` ครบทั้ง 9 โทน —
  พื้นย้อมสีโทนจาง ๆ + hover + คู่ dark (`bg-<hue>-50 hover:bg-<hue>-100/70 dark:bg-<hue>-950/30 …`)
  🔴 ทำไมต้องเพิ่ม token: `soft` ของทุกโทนเป็น **"กระดาษเทา + ขอบสี"** (`bg-slate-50`) ตามหลัก
  หมึกกับกระดาษ — ดึงแค่ `bg-` จาก soft มาทาแถวจะได้เทา ไม่ใช่แดงอ่อนอย่างที่เจ้าของสั่ง ·
  กติกาห้ามพิมพ์สีเองในไฟล์จอ จึงต้องเข้าที่ designTokens · เพิ่มเข้า `TONE_DARK_REQUIRED_VARIANTS`
  และรายการ variant ในเทสต์ designTokens แล้ว · **ใช้เฉพาะแถวที่ต้องสะดุดตา ห้ามทาทุกแถว**
- `FollowPlanningCalendar.tsx` — `<li data-category=…>` แถว `lost` ใส่ `TONE.danger.wash`
  (hover อยู่ใน token) แถวอื่น `hover:bg-secondary/50` เหมือนเดิม

**ตรวจจอจริง** (harness ชั่วคราว ลบแล้ว): สว่าง — สองแถวไม่ไป (09:15, 16:40) ขึ้นบนสุด พื้นชมพูอ่อน
ที่เหลือเรียงเวลา · มืด (ใส่คลาส `dark` จริง) — พื้นโทน rose-950 จาง อ่านออก

**เทสต์** +3: lib เรียง lost-early → lost-late → รอ → ตกลง · component: ไม่ไป 15:30 อยู่เหนือ
ตกลง 15:23 (`data-category`) · แถวไม่ไปมีคลาสแรกของ `TONE.danger.wash` แถวอื่นไม่มี

## รอบ 134 — ล้างศัพท์ให้เหลือชุดเดียวทั้งหน้า + ยุบหัวตัวเลขเหลือ 4 ช่อง (8 ก.ย. 2569)

**ที่มา:** เจ้าของบอกว่า user จริงดูแล้วงง → ให้ปล่อยผู้ทดสอบตาใหม่ (โมเดลอ่อนสุด) เล่นเอง
ผลออกมา **6/10** และสิ่งที่มันถาม = สิ่งที่ user งง:
> *"โทรไม่ติด กับ ติดต่อไม่ได้ คือเรื่องเดียวกันไหม"* · *"สายที่ต้องตาม กับ รอผล ต่างกันยังไง"*
> *"ติดต่อไม่ได้ กับ เลยเวลา ยังไม่มีผล ต่างกันยังไง — ดูเหมือนยังไม่ได้ผลเหมือนกัน"*
> *"ไม่กล้ากดปุ่ม สายที่ 1 เพราะห่วงว่าอาจเป็นปุ่มสั่งโทร"*

**ต้นเหตุ:** จอเดียวพูด 3 ภาษา — แผง "การโทรของงาน Follow" ใช้ `รอโทร/กำลังโทร/โทรติด/โทรไม่ติด/ไป/ไม่ไป`
แต่การ์ดปฏิทิน (รอบ 132) ประดิษฐ์ชุดที่สาม `ตกลง · ไป / ติดต่อไม่ได้ / รอผล / เลยเวลา ยังไม่มีผล`
⇒ ผิดกติกา "หนึ่งเมตริกหนึ่งนิยาม" ที่ตัวเองย้ำไว้

**แก้ 4 อย่าง:**
1. **ศัพท์ชุดเดียว** — `FOLLOW_CALL_CATEGORY_LABEL` ยืมคำจาก `FOLLOW_ROUND_BUCKET_LABEL` ตรง ๆ:
   `agreed → ไป` · `lost → ไม่ไป` · `unreachable → โทรไม่ติด` · `waiting → รอโทร`
   (`overdue → เลยเวลานัด` · `notSent → ไม่ได้ส่งให้ AI` ไม่มีในแผงข้างบน ใช้คำที่อธิบายตัวเองได้)
2. **หัวตัวเลข 6 ช่อง → 4 ช่อง** — `สายที่ต้องตาม · ไป · ไม่ไป · ยังไม่รู้ผล`
   `callVerdict()` + `summary.went/notWent/unknown` ใน followPlanning.ts ·
   `went + notWent + unknown = total` เสมอ (ยกเลิกไม่นับ) · เหตุผลว่า "ทำไมยังไม่รู้ผล"
   ย้ายไปอยู่บนชิปของแต่ละแถว ซึ่งเป็นที่ที่ต้องลงมือจริง
3. **ป้ายอธิบายสีขึ้นหน้ารายวันด้วย** (เดิมมีแต่หน้ารายเดือน) · ใช้ `DAY_LEGEND` ชุดเดียวกันสองหน้า
4. **ชิปกรองอ่านออกว่าเป็นตัวกรอง** — ไอคอน Filter + คำว่า "ตัวกรอง — ดูเฉพาะ" +
   `role="group"` + tooltip *"ตัวกรอง — แสดงเฉพาะสายที่ N (ไม่ได้สั่งโทร)"*
   (ผู้ทดสอบไม่กล้ากดจนไม่ได้ทดสอบส่วนนี้เลย)
5. สรุปใต้ชื่อหน้ารายเดือนเหลือสามคำตอบ `N ครั้ง · ไป x · ไม่ไป y · ยังไม่รู้ผล z`
   (เดิมไล่ทุกหมวดได้บรรทัด *"2 ครั้ง · เลยเวลา ยังไม่มีผล 2"* ที่อ่านไม่ออก)
6. แถว `notSent` ไม่พิมพ์เหตุผลซ้ำคำกับชิป (ค่าปริยายคือ "ไม่ได้ส่งให้ AI โทร")

**เทสต์ +4** — 🔴 **ด่านกันจอพูดสองภาษา**: `FOLLOW_CALL_CATEGORY_LABEL` 4 คำต้อง `toBe`
คำใน `FOLLOW_ROUND_BUCKET_LABEL` เป๊ะ · และ **ห้ามคำที่เคยทำให้งงกลับมา**
(`ติดต่อไม่ได้` · `ตกลง · ไป` · `รอผล`) · `callVerdict` ครบทุกหมวด · ผลรวมสามคำตอบ = total
เทสต์ component เดิม 17 เคสอัปเดตคำตาม

**ยังไม่ได้ทำ (รอเจ้าของเคาะ):** ยุบแผง "การโทรของงาน Follow" กับการ์ดปฏิทินเป็นอันเดียว —
ผู้ทดสอบสับสนว่าทำไมมีสองที่ที่พูดเรื่องเดียวกัน แต่เป็นการรื้อโครงหน้า ไม่ใช่แค่คำ

## รอบ 135 — ยุบ "แผงการโทร" + "ปฏิทินติดตาม" เป็นการ์ดเดียว (8 ก.ย. 2569)

**เจ้าของสั่ง "ทำเลย"** ต่อจากข้อค้างของรอบ 134 — ผู้ทดสอบตาใหม่ถามว่า
*"แผงการโทรของงาน Follow กับการ์ดปฏิทิน บอกเรื่องเดียวกันหรือคนละเรื่อง งงว่าทำไมมีสองที่"*

**ต้นเหตุ:** สองการ์ดซ้อนกัน ทั้งคู่พูดเรื่อง "การโทร" และมี **ตัวเลือกรอบคนละชุด**
(แท็บ 1/2/3 บนแผง + ชิปกรองในปฏิทิน) · แถมมี **ตัวเลือกวันสองตัว** (ตัวกรองของหน้า + ของการ์ด)

**โครงใหม่ — ผืนเดียว (`Sheet2`) เรียงจากบนลงล่าง:**
1. หัว: `ปฏิทินติดตาม` · วัน/เดือนที่ดูอยู่ · ปุ่มของหน้าแม่ (เพิ่มคน · เลือกวัน+ช่วงเวลา ·
   เพิ่มเรื่อง/เจ้าหน้าที่ · รีเฟรช) — **ตัวเลือกวันเหลือตัวเดียวทั้งหน้า** (`fDate`)
2. **ตัวเลือกรอบที่เดียว** — `ทุกสาย · รอบโทรที่ 1 · 2 · 3` พร้อมยอดคน/สัญญาณ/ผลเด่นของรอบ
   (เพิ่ม "ทุกสาย" เข้ามาใหม่ · เดิมบังคับเลือกรอบใดรอบหนึ่งเสมอ)
3. แถบสัญญาณ + บรรทัดผลจาก AI ของรอบที่เลือก
4. **7 กล่องสถานะสาย** (ทั้งหมด/รอโทร/กำลังโทร/โทรติด/โทรไม่ติด/ไป/ไม่ไป) — ครบเท่าเดิม
   กดดูรายชื่อได้เหมือนเดิม · **เพิ่มป้ายบอกขอบเขต** *"สถานะสายของ <รอบ> · **ทุกวัน**"*
   🔴 ป้ายนี้คือหัวใจของการรวม — ตัวเลขชุดนี้ขอบเขต "ทุกวัน" ส่วนตัวเลขข้างล่าง "วันที่เลือก"
   ถ้าไม่เขียนกำกับ คนจะอ่านว่าเป็นเลขชุดเดียวกันที่ขัดกันเอง
5. แท็บ `รายวัน · สายที่ต้องตาม` / `รายเดือน · ภาพรวม` + ลูกศรเลื่อนวัน/เดือน
6. เนื้อหาของแท็บ (4 ตัวเลข + ป้ายสี + รายการ / ตารางเดือน) — **กรองตามรอบที่เลือกทั้งสองแท็บ**
   (หน้ารายเดือนใช้ `filterPlanningRowsByRound` ที่มีเทสต์คุมอยู่แล้ว)

**การเปลี่ยน state:**
- `FollowCallRoundsPanel` เดิมถือ `activeRound` เอง → **controlled** ด้วย prop `round`/`onRoundChange`
  (ชนิด `FollowRoundFilter` รองรับ `'all'`) · เพิ่ม prop `embedded` = ไม่วาดเปลือกการ์ด/หัวเรื่อง/
  ปุ่มรีเฟรชของตัวเอง (แพตเทิร์น `embedded` ตามกติกา CLAUDE.md)
- `FollowPlanningCalendar` เลิกถือ `roundFilter` เอง → รับเป็น prop · เพิ่ม `roundsSlot` / `headerAction`
  · ถอดชิปกรองรอบและ `DayCalendarPicker` ของตัวเองออก (ซ้ำกับของหน้าแม่)
- `FollowPage` ถือ `activeRound: FollowRoundFilter` ตัวเดียว ค่าเริ่มต้น `'all'`

**คำศัพท์:** แถวในรายการเปลี่ยนจาก `สายที่ N` → `roundTabLabel(N)` = **`รอบโทรที่ N`**
ให้ตรงกับตัวเลือกรอบ (เจอตอนดูจอจริง — เกือบสร้างสองภาษาซ้ำรอยรอบ 134)

**ของเดิมอยู่ครบทุกชิ้น** (กติกาห้ามเอาข้อมูลออก): แท็บรอบ+ยอดคน · สัญญาณรอบ · บรรทัดผล AI ·
7 กล่อง · ป๊อปรายชื่อ · คำอธิบายสองแกน · ปุ่มทุกปุ่ม · ตัวกรองวัน+ช่วงเวลา · รีเฟรช

**ตรวจจอจริง** (harness ชั่วคราว ลบแล้ว): 1280 สว่าง — การ์ดเดียวครบทุกส่วน · 375 มือถือ —
ตัวเลือกรอบ 2×2 · 7 กล่อง 2 คอลัมน์ · โหมดมืดหน้ารายเดือนอ่านออก
**เทสต์:** component +1 (`roundsSlot` ต้องอยู่ในผืนเดียวกัน) · แก้เคสกรองรอบให้คุมผ่าน prop
แทนการกดชิปที่ย้ายออกไปแล้ว · เต็มชุด 2908 ผ่าน

## รอบ 136 — หน้าติดตามใช้ภาษาการออกแบบตามแบบอ้างอิงที่เจ้าของส่งมา (8 ก.ย. 2569)

**เจ้าของส่งแบบจาก Stitch (`stitch_tracking_page_redesign.zip`) แล้วสั่ง:**
> *"ฉันต้องการเรื่อง Design เขา เปลี่ยนเลยเอาตามนั้น ... แค่ปรับ UI ยังไม่ได้ตามต้องการสักที เอา Design เขา"*

### ยกมาจากแบบอ้างอิง
1. **การ์ดแยกใบวางบนพื้นฟ้าเทาอ่อน** แทนผืนยาวใบเดียว — โทเคนธีมเราตรงอยู่แล้ว
   (`--background: 220 24% 97%` ≈ `#f8f9ff` ของเขา · `--card` ขาว) ไม่ต้องแตะสีเลย
2. **การ์ดตัวเลข 4 ใบ** — ตราไอคอนกลมมุมขวาบน (`TONE[x].soft` + ไอคอน lucide) · เลข 40px
   · บรรทัดความหมายคั่นเส้นที่ท้ายการ์ด (`StatCard` ในไฟล์เดียวกัน · มี `data-stat` ให้เทสต์เล็ง)
3. **สองคอลัมน์** `xl:grid-cols-[minmax(0,2fr)_minmax(320px,1fr)]` — รายการหลัก + แผงข้างขวา
4. **แผงข้างขวา**: การ์ด "ผลของเดือนนี้" (วงโดนัท SVG ล้วน) + การ์ด "ต้องตามด่วน" พร้อมปุ่มโทร `tel:`
5. **แถวรายการ**: วงกลมอักษรย่อชื่อ · ป้ายสถานะเม็ดยากลมมีจุดสีนำหน้า · **เวลาอยู่ขวาสุด**
   (ตำแหน่งเดียวกับคอลัมน์ "กำหนดเริ่มงาน" ของเขา)
6. **แผงรอบโทรเป็นการ์ดของตัวเอง** + **หลอดความคืบหน้า** ใต้ 7 ช่องสถานะสาย (เทียบกับ "ทั้งหมด")

### ไม่ยกมา — ผิดกติกาที่เจ้าของเคาะไว้เอง (บันทึกไว้กันถามซ้ำ)
- **`backdrop-filter` 37 จุด** — ถอดทั้งระบบ 5 ก.ย. เพราะเว็บกระตุก · ด่าน `perfGuards.test.ts` คุมอยู่
  ⇒ ใช้การ์ดขาวทึบ + `shadow-sm` แทน ได้หน้าตาเดียวกันโดยไม่เสียความเร็ว
- **ฟอนต์ Be Vietnam Pro** — ไม่มีชุดอักษรไทย (ไทยในภาพตัวอย่างตกไปฟอนต์สำรอง) · Kanit อ่านไทยดีกว่า
- **จานสี crimson `#9E2A2B`** — ของเราเบอร์กันดี `#8c2f39` ใกล้กันมาก ใช้ของเดิม
- **Show-up Rate / คอลัมน์ "ความเสี่ยงหลุด" / Pipeline 4 ขั้นก่อนเริ่มงาน** — **ไม่มีข้อมูลในฐาน**
  ทำแล้วต้องแต่งเลข = จอโกหก · แทนด้วยวง "ตอบว่าไป" ที่คิดจากข้อมูลจริง
  🔴 ตัวหาร = **เฉพาะสายที่รู้ผลแล้ว** (ไป+ไม่ไป) ไม่เอา "ยังไม่รู้ผล" มาหาร ไม่งั้นต้นเดือน %
  จะต่ำปลอม ๆ · เขียนฐานกำกับใต้วงเสมอ ("คิดจาก N สายที่รู้ผลแล้ว")

### ของเดิมอยู่ครบ
ตัวเลขทุกตัว · แท็บรอบ · 7 ช่อง + ป๊อปรายชื่อ · สรุปจาก AI · เบอร์ฉุกเฉิน · แถวไม่ไปพื้นแดง+ลอยบน ·
คำอธิบายสี · มุมมองรายเดือน · ปุ่มทุกปุ่ม · ตัวกรองวัน+ช่วงเวลา

**เทสต์:** เพิ่มจุดยึด `data-stat` (การ์ดตัวเลข) และ `data-testid="day-calls"` (รายการหลัก) —
🔴 ต้องมี เพราะแผงข้างขวาก็เป็น `<li>` เหมือนกัน `getAllByRole('listitem')` เดิมจึงนับปน ·
helper เทสต์เปลี่ยนมาเล็งสองจุดนี้แทนการไล่ DOM ตามโครง (โครงเปลี่ยนได้ เทสต์ไม่ต้องแก้)
เต็มชุด 2908 ผ่าน · ตรวจจอจริงกับข้อมูลจริง 1440/1280 สว่าง-มืด + มือถือ 375

## รอบ 137 — ทำให้ "เหมือนแบบอ้างอิงจริง ๆ" (8 ก.ย. 2569)

**เจ้าของทัก:** *"ตัวอย่างมันไม่ชัดเจนหรือว่าไง ถึงได้ทำออกมาให้เหมือนเขาไม่ได้"*

**ยอมรับ:** รอบ 136 เอาโครงเดิมมาทาสีใหม่ ไม่ได้ทำตามโครงของเขา · จุดที่ไม่เหมือนชัด ๆ 2 จุด

### 1. การ์ด "ขั้นตอนของสาย (Call Pipeline)" — ทรงเดียวกับ Pipeline Stages ของเขา
เดิม: การ์ดรอบใหญ่ 4 ใบ (กินที่เท่า pipeline ทั้งแถบ) + ช่องถัง 7 ช่องแบน ๆ ไม่มีทรง
ใหม่: **การ์ดใบเดียว** หัวมีไอคอน `GitBranch` + ชื่อเรื่องซ้าย · **ตัวเลือกรอบย่อเป็นเม็ดยาเล็กขวา**
ข้างในเป็นการ์ดขั้นตอนย่อย 7 ใบ (7 ช่องสถานะสายคือ pipeline จริงอยู่แล้ว) แต่ละใบมี
`ขั้นที่ N` · ตัวเลขในวงกลมมุมขวา · ชื่อไทยตัวหนา · คำอธิบายตัวเทา · **หลอดหนาที่ก้นการ์ด**
· hover ยกขึ้น `-translate-y-0.5` + เงา (แบบอ้างอิงมี lift เหมือนกัน)
⚠️ ทำเป็น **early return เมื่อ `embedded`** — โค้ดโฉมเดิมยังอยู่ครบ ไม่ต้องรื้อสองสาขาที่พันกัน
· `peopleDialogEl` ถูกดึงออกมาเป็นตัวแปรให้ทั้งสองสาขาใช้ร่วม

### 2. รายการรายวัน: ลิสต์ → **ตารางมีหัวคอลัมน์**
คอลัมน์จับคู่กับของเขา: `ผู้ที่ต้องติดตาม / ติดต่อ` (วงอักษรย่อ + ชื่อ + เบอร์) · `หน่วยงาน` ·
`เวลานัด / รอบ` · `สถานะการโทร` (เม็ดยา + จุดสี) · `เขาตอบว่าอะไร` (= บันทึกล่าสุดของเขา) ·
`เบอร์ฉุกเฉิน` · `จัดการ` (ปุ่มโทร `tel:` + ปุ่มจัดการ)
· **แบ่งหน้า 12 สาย/หน้า** พร้อมบรรทัด "แสดง 1 ถึง 12 จากทั้งหมด 49 สาย" (ของเขามีเหมือนกัน)
🔴 เปลี่ยนวัน/รอบต้องเด้งกลับหน้า 1 ไม่งั้นค้างหน้าว่าง
· responsive: แผงขวาลงล่างจนถึง `2xl` (ตาราง 7 คอลัมน์ต้องการที่) · ซ่อน `หน่วยงาน` ต่ำกว่า `lg`
  และ `เบอร์ฉุกเฉิน` ต่ำกว่า `xl` — ข้อมูลไม่หาย กลับมาเมื่อจอกว้าง และอยู่ในป๊อป "จัดการ" เสมอ

**เทสต์:** `dayRows()` เปลี่ยนจาก `getAllByRole('listitem')` เป็น `getAllByRole('row')` ·
เคสกดแถวเล็งปุ่ม "จัดการ" · เคสเบอร์ฉุกเฉินเช็คสองบรรทัดแทนข้อความรวม (หัวคอลัมน์บอกแล้ว
จึงไม่มีคำว่า "ฉุกเฉิน" นำหน้า) · เต็มชุด 2908 ผ่าน · ตรวจจอจริง 1600/1280 กับข้อมูลจริง 49 สาย

## รอบ 138 — 🔴 คำเดียวกันคนละนิยาม: "ไป" บนบอก 3 ล่างบอก 0 (8 ก.ย. 2569)

**เจ้าของจับได้เอง:** *"คำว่า ไป ด้านบนบอก 3 ด้านล่าง 0 ยังไงเนี่ย แล้วจะเชื่อถือได้หรอ"*

**ความผิดพลาดของรอบ 134 (เช้าวันเดียวกัน):** ยืมคำจาก `FOLLOW_ROUND_BUCKET_LABEL`
มาใช้เป็นคำของ `FOLLOW_CALL_CATEGORY_LABEL` **โดยไม่ตรวจว่านิยามตรงกันไหม** —
แล้วยัง **เขียนเทสต์บังคับให้เหมือนกัน** ด้วย ⇒ ด่านที่ตั้งใจกันจอพูดสองภาษา
กลับกลายเป็นตัวบังคับให้เกิดคำเดียวกันที่นับคนละเรื่อง

ตรวจแล้วไม่ตรงกันสักคู่:

| คำ | หมวด `callCategory` | ช่อง `followRoundBuckets` |
| --- | --- | --- |
| ไป | ผลโทรยืนยันจะไป **หรือ** ปิดงานว่าไป | **เฉพาะ** ปิดงานว่าไป (`outcome_code`) |
| ไม่ไป | ตอบว่าไม่ไป **หรือ** ปิดงานว่าไม่ไป | เฉพาะปิดงานว่าไม่ไป |
| โทรไม่ติด | รวม `reschedule_requested` + `wrong_person` | ไม่รวม — สองอันนั้นอยู่ถัง "โทรติด" (ดูหัวไฟล์ `callOutcomeBuckets.ts`) |
| รอโทร | ส่งแล้วรอผล **+** ยังไม่ถึงเวลา | เฉพาะที่ยังไม่ถูกดึงไปโทร (`delivered` = กำลังโทร) |

**แก้:** หมวดผลพูดถึง **"คำตอบที่ได้จากสาย"** จึงใช้คำที่บอกอย่างนั้นและ**ห้ามซ้ำ**กับ Pipeline
`agreed → ตอบว่าไป` · `lost → ตอบว่าไม่ไป` · `unreachable → ไม่ได้คำตอบ` ·
`waiting → ยังไม่ถึงเวลา` (ที่เหลือคงเดิม) · การ์ด "ตอบว่าไป" เขียนท้ายการ์ดว่า
*"คำตอบจากสาย — ยังไม่ใช่การปิดงาน"* · ป้ายสี/แผงขวา/ตารางเดือน เปลี่ยนตามทั้งชุด

**ด่านเทสต์กลับด้าน** (`tests/api/followPlanning.test.ts`):
- เดิม: บังคับให้คำ **เท่ากัน** → ลบทิ้ง
- ใหม่: บังคับให้คำของหมวด **ไม่อยู่ใน** ชุดคำของช่อง Pipeline (ทั้ง 6 คำ) ·
  พร้อมเคสพิสูจน์ว่านิยามต่างกันจริง (`confirmed` แต่ยังไม่ปิดงาน ⇒ `callCategory='agreed'`
  แต่ `inFollowRoundBucket(...,'went') === false`) · คงรายการคำต้องห้ามเดิมไว้
...
## หน้าติดตาม — ตำหนิ 4 ข้อ (10 ก.ย. 2569)

เจ้าของแจ้ง: *"1. ไม่แสดงการโทรติดต่อเบอร์ฉุกเฉิน · 2. ไม่มีให้กดแก้ไขวัน/เวลา ·
3. สายที่ 1 โทรสำเร็จ ไม่แสดงสีเขียว และไม่แสดงข้อความที่ตอบ · 4. สายที่ 2 ไม่แสดงข้อความที่ตอบ"*

**ตรวจกับฐานจริงก่อนแก้** (อ่านอย่างเดียว 100 แถว · 18 แถวที่มีผล):

| ข้อ | เจอว่าอะไร | แก้ยังไง |
| --- | --- | --- |
| 1 | ข้อมูล**อยู่ในหน้าแล้ว** แต่คอลัมน์เป็น `hidden xl:table-cell` ⇒ จอแคบกว่า 1280px มองไม่เห็นเลย | เอา `hidden`/`xl:` ออก · ถ้อยคำแยก "แนบเบอร์ไปแล้ว" (เรารู้) ออกจาก "โทรหรือยัง" (เราไม่รู้) |
| 2 | ปุ่มแก้ไข**มีอยู่** แต่ซ่อนใน popup "จัดการ" อีกชั้น (กด 2 ครั้ง) | เพิ่มปุ่มดินสอ**บนแถว** → `onEditRound` → `setEditing(entry)` ตรง ๆ |
| 3 | หมวด/ผลถูกต้องแล้ว (`acknowledged` ⇒ `agreed`) แต่**ระบายสีทั้งแถวเฉพาะ `lost`** ⇒ แถวเขียวหน้าตาเหมือนแถวที่ยังไม่มีผล | `callCategoryWashTone()` — เขียว/เหลือง/แดงตามที่เจ้าของเคาะ 7 ก.ย. · ยังไม่มีผล = ขาว |
| 3+4 | ช่อง "เขาตอบว่าอะไร" โชว์ `call_summary` ซึ่งเป็น**คำบรรยายของ AI** ("ผู้รับสายแจ้งว่า…") ไม่ใช่คำตอบของเขา · สายที่จบแบบ `unresponsive` ไม่มี summary ⇒ ช่องว่าง | API ดึงคำฝั่ง candidate จาก `result->'transcript'` เป็น `call_reply` · จอโชว์คำพูดในเครื่องหมายคำพูดก่อน สรุป AI เป็นตัวรอง |

🔴 **เรื่องเบอร์ฉุกเฉินที่แก้ฝั่งเราไม่ได้** — เราส่ง `admin_phone` ไปกับ **ทุกสาย** (100/100 แถว)
แต่ผลที่ Lumos ส่งกลับมี **23 ช่อง ไม่มีช่องไหนบอกว่าโทรเบอร์นี้หรือยัง** และไม่มีผลไหน
เอ่ยถึงเบอร์นี้เลย (`result::text ilike '%admin%'` = 0 · เลขเบอร์ = 0)
`docs/plan-follow-2569-09-01.md:180` เขียนไว้ตั้งแต่แรกว่า *"การตัดสินใจในสายเป็นของเขา"*
⇒ **ต้องขอให้ Lumos เพิ่มช่องในผล** (เช่น `admin_call: { attempted, connected, at }`)
จนกว่าจะมี จอต้องพูดว่า "แนบไปแล้ว · ไม่รู้ว่าโทรหรือยัง" เท่านั้น **ห้ามเขียนว่าโทรแล้ว**

⚠️ **`call_reply` ไม่ส่ง transcript ทั้งก้อน** — บางสาย 24 ตา ส่งดิบทุกแถว payload บวมเปล่า ๆ
ต่อคำด้วย ' · ' ที่ SQL แล้วให้จอตัดความยาวเอง

**วิธีตรวจแบบที่ใช้รอบนี้ (ทำซ้ำได้):** ดึงแถวจริงจากฐานด้วย SQL เดียวกับ handler →
map เป็น `FollowEntry` → เดินผ่าน `groupFollowEntries` + `buildFollowPlanningRows` +
`buildFollowDayCalls` → render `FollowPlanningCalendar` ใน vitest แล้วอ่าน `data-category`
กับ `className` ของแต่ละแถว · เห็นของจริงโดยไม่ต้องล็อกอิน

### สอบสวนสายรอบ 1 ค้าง + จุดบอด push (10 ก.ย. 2569)

**ตัวเลขจริง:** รอบ 1 ส่ง 51 ได้ผล 3 · รอบ 2 ส่ง 49 ได้ผล 15 ·
`first_delivered_at` เป็น null ทุกแถว ⇒ **Lumos ไม่เคย pull เลย** ผลมาทาง push ล้วน
`dispatch_state` = `queued` ครบ 100/100 (ทั้งที่ส่งถึงและไม่ถึง — ดูด้านล่างว่าทำไม)

**สิ่งที่พิสูจน์ไม่ได้จากฝั่งเรา:** วันที่ 9 ก.ย. **ไม่มีหมายเลขไหนมีรายการเดียวเลย**
(ทุกเบอร์ถูกใช้ซ้ำ 2–5 รายการ เพราะเป็นข้อมูลทดสอบ) แต่ละเบอร์ได้ผลกลับ 1–3 รายการ
ไม่ว่าส่งเข้าไปกี่รายการ ⇒ **น่าสงสัยว่า Lumos จำกัดแผนต่อหนึ่งหมายเลข** แต่ยืนยันไม่ได้
· จะพิสูจน์เองต้องทดสอบด้วยเบอร์ไม่ซ้ำ = โทรหาเบอร์จริง **ต้องให้เจ้าของสั่งก่อน**
⇒ ถามไปในจดหมาย `docs/lumos-request-2569-09-10.md`

🔴 **จุดบอดฝั่งเราที่แก้ในรอบนี้ — push ล้มแล้วเงียบ**

`pushFollowReminderToLumos()` เป็น fire-and-forget (`void` ไม่มีใครรับผล) และ
`enqueueFollowReminder()` คืน `'queued'` **ตั้งแต่ก่อนรู้ผล push** ⇒ HTTP ไปไม่ถึง Lumos
ก็เห็นแค่ใน log ส่วนบนจอขึ้นว่า "รอ AI โทร" ตลอดไป · **แยกไม่ออกระหว่าง "ส่งไม่ถึง"
กับ "ส่งถึงแล้วแต่เขาเงียบ"** ซึ่งแก้คนละทางกันสิ้นเชิง
— ความเงียบชนิดเดียวกับเคส 24 ส.ค. 2569 ที่เป็นเหตุให้ต้องมี `followDispatchState.ts`

| ไฟล์ | แก้อะไร |
| --- | --- |
| `src/lib/followDispatchState.ts` | สถานะใหม่ `push_failed` ("ส่งไม่ถึง Lumos" · needsAction + retryable) · `followDispatchLabel` ให้ `push_failed` **ชนะ `pending`** แต่แพ้ `delivered`/`completed` |
| `api/_lib/lumosDispatch.ts` | `markFollowDispatchState()` จดผล push ลงแถว — ล้ม ⇒ `push_failed` · สำเร็จ ⇒ ล้างธงกลับเป็น `queued` |

⚠️ `markFollowDispatchState` เปลี่ยนค่า**เฉพาะเมื่อค่าปัจจุบันตรงกับที่คาด** (`from`)
กันทับสถานะที่แปลว่า **ตั้งใจไม่ส่ง** (`held`/`suppressed`/`no_phone`/`off`)
และจดไม่ลงก็ห้ามโยนต่อ — ตัวเรียกเป็น `void` ไม่มีใครรับ exception

**ยังไม่ได้ทำ (รอเจ้าของสั่ง):** ปุ่ม "ส่งใหม่" บนแถวที่เป็น `push_failed`
ตอนนี้บอกได้ว่าส่งไม่ถึง แต่ยังต้องแก้ด้วยมือ

### ตารางรายวันหน้าติดตาม: หนึ่งแถว = หนึ่งคน (11 ก.ย. 2569)

เจ้าของทัก: *"หน้าติดตามพอเพิ่มโทรหลายรอบ มันขึ้นหลายบรรทัดอะ คนดูเขางง"*

เดิมหนึ่งแถว = หนึ่ง**สาย** ⇒ คนเดียวตั้ง 3 รอบ = 3 บรรทัด ชื่อซ้ำสามที
วัดกับข้อมูลจริง 9 ก.ย.: **25 สาย = 25 แถว ⇒ เหลือ 13 แถว (13 คน)**

`buildFollowDayPeople()` ใน `followPlanning.ts` — ยืม `buildFollowDayCalls` มาทั้งดุ้น
แล้วจัดกลุ่มด้วย `row.group.key` (นิยาม "สายของวันนี้" ยังมีที่เดียว)

* **ลำดับรอบในแถว = ตามเลขรอบ ไม่ใช่ตามเวลา** — ข้อมูลจริงมีเคสตั้งรอบ 2 ไว้ก่อนรอบ 1
  (16:30 รอบ 2 · 16:35 รอบ 1) เรียงตามเวลาแล้วรอบ 2 ไปอยู่บนรอบ 1 อ่านแล้วสะดุด ·
  เรียงตามรอบทำให้เวลาที่สลับกัน**มองเห็นได้** แทนที่จะถูกกลบ
* **ลำดับคน** = `DAY_HEADLINE_ORDER` แล้วค่อยเวลานัด**แรกสุด**ของคนนั้น (ไม่ใช่เวลาของรอบ 1)
* `headline` ของคน: `lost > agreed > unreachable > overdue > notSent > waiting > cancelled > other`
  อธิบายบรรทัดเดียว — **ตอบแล้วชนะยังไม่ตอบ** (คำตอบคือข้อเท็จจริง) · **ไม่ไปชนะไป**
* สามคอลัมน์ (เวลา/รอบ · สถานะ · คำตอบ) ใช้ `space-y-2` ชุดเดียวกัน ⇒ **บรรทัดที่ N
  ของทุกคอลัมน์คือสายเดียวกัน** ห้ามใส่ระยะต่างกัน ไม่งั้นคนอ่านคำตอบผิดสาย
* ดินสอแก้เวลาติดอยู่กับ**แต่ละรอบ** (ไม่ใช่แถว) · ปุ่ม "จัดการ" ส่ง **ทุกรอบ**ของคนนั้นเข้าป๊อป

🔴 **การ์ดตัวเลขด้านบนยังนับเป็น "สาย" เหมือนเดิม** — คนละคำถามกับตาราง
(การ์ด: "มีกี่สายต้องตาม" · ตาราง: "ต้องตามใครบ้าง") ท้ายตารางบอกทั้งสองเลข
"N คน · M สาย" มีเทสต์คุมว่ารวมแถวแล้วจำนวนสายรวมต้องเท่าเดิม

### สายแรกไม่ปรับสถานะ — เจอสองสาเหตุคนละเรื่อง (11 ก.ย. 2569)

เจ้าของถาม: *"เบอร์ที่ชื่อ ลูกชาย สองคน ทำไมสายแรกไม่ปรับสถานะ"*

ตรวจฐานจริงวันนั้น (42 รายการ) พบว่าคำว่า "ไม่ปรับสถานะ" มาจาก **สองสาเหตุที่แก้คนละทาง**

| dispatch_state | กี่รายการ | ได้ผล | แปลว่า |
| --- | --- | --- | --- |
| `queued` | 22 | 8 | ส่งถึง Lumos แล้ว — ที่ไม่ได้ผลคือฝั่งเขาเงียบ |
| `push_failed` | 12 | 0 | **ไม่เคยถึง Lumos เลย** — ไม่มีสายไหนกำลังจะออก |
| `suppressed` | 8 | 0 | เบอร์อยู่ในบัญชีห้ามโทร (ตั้งใจไม่ส่ง) |

**เคส "ลูกชาย สองคน" เป็นแบบแรก** — `queued` ทั้งคู่ · payload ถูกต้อง · เวลานัดเป็นอนาคต
(ล่วงหน้า 13 นาที) · สายที่นัด 10:25 ได้ผล `confirmed` ส่วนสายที่นัด 10:20 ไม่มีผลกลับเลย

🔴 **รูปแบบที่ชัดขึ้นมาก** — ชุดที่สร้างพร้อมกันบนเบอร์เดียวกัน:
สายที่นัด**ทีหลัง** 16 สาย ได้ผล **7** · สายที่นัด**ก่อน** 16 สาย ได้ผล **1**
⇒ น่าเชื่อว่าสายหลังไปแทนที่สายแรกของเบอร์เดียวกัน (ถามไปในจดหมายแล้ว)

**ที่ทำเพิ่มรอบนี้** — `push_failed` เดิมมีธงแต่**ไม่โผล่บนตารางเลย**
(`followDispatchLabel` ไม่ได้ถูกเรียกจากตารางรายวัน) ⇒ 12 รายการนั้นขึ้นว่า "เลยเวลานัด"
เหมือนสายที่เขารับไปแล้วแต่เงียบ

| ไฟล์ | แก้อะไร |
| --- | --- |
| `migrations/116_follow_dispatch_error.sql` | คอลัมน์ `dispatch_error` เก็บ**เหตุ** ไม่ใช่แค่ว่าล้ม |
| `api/_lib/lumosDispatch.ts` | `markFollowDispatchState()` รับ `error` · มี fallback เขียนเฉพาะสถานะเมื่อฐานยังไม่ migrate |
| `src/lib/followPlanning.ts` | `roundPushFailed()` · `roundPushError()` |
| `FollowPlanningCalendar.tsx` | ชิปส้ม "ส่งไม่ถึง Lumos" + โชว์เหตุในช่องคำตอบ |

⚠️ **ไม่แตะ `followRoundState`/`callCategory`** โดยตั้งใจ — เพิ่มสถานะใหม่เข้าไปจะทำให้
เลขทุกช่องบนการ์ดขยับพร้อมกัน · ตัวนี้เป็น**ป้ายเสริมบนแถว** มีเทสต์คุมว่าเลขไม่ขยับ

**ยังไม่ได้ทำ:** ปุ่ม "ส่งใหม่" บนแถวที่ `push_failed` · และยังไม่รู้ว่า push ล้มเพราะอะไร
(`pushReminders` ไม่ retry เมื่อได้ HTTP response กลับมา — 4xx/5xx โยนทิ้งเลย ดูหัวไฟล์
`lumosPushClient.ts`) ตั้งแต่นี้ไปเหตุจะถูกจดลง `dispatch_error` ให้อ่านได้จากจอ

### push ไป Lumos ล้มเพราะอะไร — หลักฐานที่มี (11 ก.ย. 2569)

เจ้าของถาม: *"อยากรู้ว่าเพราะอะไร เพราะมันเป็นบ่อยแล้ว"*

**1. ไม่ใช่ Lumos ปฏิเสธเพราะเนื้อหา** — เทียบ payload ของวันนั้น แยกไม่ออกเลย

| | ผ่าน (22) | ล้ม (12) |
| --- | --- | --- |
| มี `admin_phone` | 22/22 | 12/12 |
| จำนวน `steps` | 1 | 1 |
| ความยาวข้อความ | 114–168 | 118–140 (อยู่ในช่วงเดียวกัน) |
| เวลานัดเป็นอดีต | 0 | 0 |

**2. ล้มเป็น "ช่วงสั้น ๆ" สลับกับสำเร็จ** — ไม่ใช่ล่มยาว

```
09:07–09:23  ส่ง 18 ผ่านหมด
09:37        ล้ม 2/2
09:57        ล้ม 2/2
10:07        :00 ล้ม · :09 ล้ม · :23 ล้ม · :31 ผ่าน · :44 ผ่าน   ← ช่วงเสียราว 23 วินาที
10:30        ล้ม 2/2
```

**3. ตรงกับที่เคยเจอมาแล้ว** — หัวไฟล์ `lumosPushClient.ts` บันทึกไว้เอง (8 ก.ย. 2569):
ส่ง 16 รายการ ผ่าน 4 ที่เหลือขึ้น *"fetch failed"* ทั้งที่ payload/credential ถูกหมด

⇒ **เข้าข่ายต่อไม่ถึงชั่วคราวเป็นช่วง ๆ ไม่ใช่ถูกปฏิเสธ**
`FETCH_MAX_ATTEMPTS = 3` + delay `[300, 900]` ⇒ retry ครอบแค่ **~1.2 วินาที**
สั้นกว่าช่วงเสียที่วัดได้ (หลายสิบวินาที) มาก ⇒ retry ปัจจุบันแทบไม่ช่วยเลย

🔴 **จุดบอดที่แก้รอบนี้: จดเหตุได้แต่เหตุไม่มีความหมาย**
`dispatch_error` (migration 116) เก็บ `e.message` ตรง ๆ แต่ `fetch()` ของ Node โยน
`TypeError: fetch failed` **เสมอ** ไม่ว่าเหตุจริงจะเป็น DNS/ต่อไม่ติด/TLS/timeout
(เหตุจริงอยู่ใน `.cause`) ⇒ จะได้คำว่า "fetch failed" เปล่า ๆ ซึ่งไล่ต้นเหตุไม่ได้
— ปัญหาเดียวกับที่ `describeError()` ใน `logger.ts` ถูกเขียนขึ้นมาแก้ตั้งแต่แรก

`errorSummaryText()` (export จาก `logger.ts` · เทสต์ที่ `tests/api/errorSummaryText.test.ts`)
คลี่ `.cause` + `AggregateError` เป็นบรรทัดเดียว เช่น `fetch failed | ECONNRESET: read ECONNRESET`
ใช้ตัวถอดชุดเดียวกับ log ⇒ **log กับจอเล่าเรื่องเดียวกัน**

**ยังไม่ได้ทำ (รอเจ้าของเคาะ):**
1. **ตัวส่งซ้ำเบื้องหลัง** สำหรับแถวที่ค้าง `push_failed` — เปลี่ยน "สายหาย" เป็น "สายช้า"
...
### หลายรอบ = แผนเดียวหลาย step (11 ก.ย. 2569)

เจ้าของสรุปความต้องการเป็น 5 ข้อ: เพิ่มที่เรา → สายไปถึง Lumos → **ตั้งกี่รอบต้องโทรครบ
ทุกรอบ เว้นแต่รอบไหนตอบว่าไม่ไป/ยกเลิกแล้วให้หยุดที่เหลือ** → Lumos โทร →
**ส่งผลกลับมาทุกสาย** → มี log ทุก endpoint

🔴 **ต้นเหตุ: เราใช้ API เขาผิดวิธี** — เดิมหน้าเว็บวนยิงทีละรอบ ⇒ สร้าง **แผนแยกกัน
รอบละแผน** ไปที่เบอร์เดียวกัน แผนหลังทับแผนแรก
(วัดจริง 11 ก.ย.: ชุดที่สร้างพร้อมกันบนเบอร์เดียวกัน สายที่นัดทีหลังได้ผล **7/16**
สายที่นัดก่อนได้ผล **1/16**)
Lumos ออกแบบให้ 1 แผนมีหลาย step อยู่แล้ว + มี `stop_early` หยุด step ที่เหลือเอง
เมื่อได้คำตอบชัด — **ตรงกับข้อ 2 ของเจ้าของพอดี ไม่ต้องเขียนตรรกะหยุดเอง**

| ไฟล์ | แก้อะไร |
| --- | --- |
| `migrations/117_lumos_queue_plan_steps.sql` | `plan_ref` + `step_position` บนคิว |
| `api/_lib/lumosDispatch.ts` | `buildFollowPlanPayload()` · `enqueueFollowReminderPlan()` · `applyLumosResult` จับผลด้วย `plan_ref + step_position` ก่อน แล้วค่อยถอยไปวิธีเดิม |
| `api/_handlers/follow.ts` | `parseFollowRounds()` · `createFollowRounds()` · แยก `insertFollowRow()` มาใช้ร่วม |
| `src/lib/followApi.ts` · `FollowPage.tsx` | `createFollowRounds()` — **ยิงครั้งเดียวทุกรอบ** |
| `api/_lib/followPushRetryWorker.ts` | ส่งซ้ำเฉพาะแถวหัวขบวน · สำเร็จแล้วล้างธงทั้งแผน |

**โครงที่เลือก — หนึ่งแถวคิวต่อหนึ่งรอบเหมือนเดิม เปลี่ยนแค่สิ่งที่ยิงออกไป**
ทั้งหน้าจอผูกกับแถวต่อรอบ (สถานะ · ผล · ปุ่มแก้เวลา) การยุบเป็นแถวเดียวจะรื้อทั้งหน้า

* แถว **หัวขบวน** (`step_position = 0`) ถือ payload ของ**ทั้งแผน** — เป็นแถวเดียวที่ถูก push
* แถวที่เหลือเก็บ payload ของตัวเองไว้อ้างอิง **ห้าม push** (push เมื่อไหร่ = กลับไปทับกัน)
* `payload->>'client_contact_id'` ของแต่ละแถวจึงยังไม่ซ้ำกัน ⇒ ตัวจับผลแบบถอยหลังยังถูก

🔴 **ผลต้องเข้าแถวของรอบนั้น** — Lumos ผูกผลด้วย `client_contact_id` ของ**แผน**
ทุก step จึงมี id เดียวกันหมด ตัวแยกคือ `step_position` · `applyLumosResult` จึงลอง
`plan_ref + step_position` ก่อนเสมอ ไม่เจอค่อยถอย (แถวเก่า/ฐานที่ยังไม่ migrate 117)

⚠️ **ต้องรู้ทุกรอบตั้งแต่ตอนสร้าง** — จะไปรวมทีหลังไม่ได้เพราะแผนแรกถูกส่งไปแล้ว
และ `Idempotency-Key` เดิมจะทำให้ Lumos ตัดทิ้ง · นี่คือเหตุผลที่ต้องเพิ่ม `rounds[]`
ใน body แทนการวนยิง **ห้ามกลับไปวน `for` ยิงทีละรอบ**

⚠️ `parseFollowRounds` **เรียงตามเวลาและตัดเวลาซ้ำ** — step ซ้ำ = Lumos โทรซ้ำเวลาเดียวกัน

### เหตุที่ "ส่งไม่ถึง Lumos" + สิ่งที่แก้ (11 ก.ย. 2569 บ่าย)

`dispatch_error` ที่เพิ่งทำจับเหตุจริงได้แล้ว **ไม่ต้องเดาอีก**
...
ต่อไม่ติดตั้งแต่ชั้น **connect** ยังไม่ทันคุย TLS ⇒ ไม่ใช่คีย์ ไม่ใช่ payload ไม่ใช่เขาปฏิเสธ
`ENETUNREACH ... Local (:::0)` = **เครื่องเราไม่มีทางออก IPv6** แต่ DNS ของปลายทาง
(Lumos อยู่หลัง Google Cloud — ช่วง `192.178.x` และ `2404:6800::` เป็นของ Google)
ตอบทั้ง A และ AAAA ⇒ Node ลองเส้น IPv6 ทุกครั้งแล้วชนกำแพงทุกครั้ง

| แก้อะไร | ไฟล์ |
| --- | --- |
| `ipv4first` + ปิด `autoSelectFamily` (ตั้งตอนบูต ก่อนมีใครยิงออกเน็ต) | `api/_lib/netPreferIpv4.ts` · `server/local-api.ts` |
| retry ในคำขอเดียว 3 ครั้ง/1.2 วิ → **5 ครั้ง/~17.5 วิ** (ช่วงเน็ตหลุดจริงวัดได้หลายสิบวินาที) | `lumosPushClient.ts` |
| จด `event_id` ที่ Lumos ตอบรับ = **หลักฐานว่าเขารับไปแล้วจริง** | migration 118 · `lumosDispatch.ts` |
| ติดธง `push_failed` **ทั้งแผน** ไม่ใช่แค่หัวขบวน | `lumosDispatch.ts` (`planMemberIds`) |

🔴 **บั๊กที่ผมทำเองแล้วเจ้าของจับได้** — แผนเดียวมีหลายสาย แต่ push ครั้งเดียวที่หัวขบวน
ของเดิมติดธงแถวเดียว ⇒ สายที่ 2 ขึ้นจอว่า "ส่งแล้ว" ทั้งที่ไม่มีอะไรถูกส่ง = **จอโกหก**
(เคส "ไบรโอนี่ คิคิ": ร1 `push_failed` · ร2 `queued`)

⚠️ **IPv4 ไม่ใช่ยาครอบจักรวาล** — ถ้า IPv4 เองต่อไม่ติดก็ยังล้ม
ตัวที่รับประกันว่า "ต้องถึง" คือ `followPushRetryWorker` ที่ส่งซ้ำทุกนาที
(พิสูจน์แล้ววันนี้: "ไบรโอนี่ คิคิ" ร1 `push_failed` 15:22 → worker กู้เป็น `queued` เอง)

**ผลการทดลองจริงของแผนเดียวหลาย step** — กลไกฝั่งเราถูกต้องครบ:
ทุกแผนส่งไป 2 step · Lumos สะท้อน `step_position` ตรงทุกครั้ง (0 และ 1) ·
ผลเข้าแถวของรอบนั้นถูกต้อง (ความน่ารักฯ step 0 → ร1 · เมย์ step 1 → ร2)

🔴 **ที่ยังเหลือคือฝั่งเขา** — บางแผน `plan_status = completed` ทั้งที่ **ไม่เคยส่งผลของ
step 0 กลับมาเลย** (`stop_early = false` ด้วย จึงไม่ใช่การหยุดเพราะได้คำตอบ)
ใส่ `plan_id` ตัวอย่างไว้ในจดหมาย `docs/lumos-request-2569-09-10.md` แล้ว

### กล่องรับผลดิบจาก Lumos (11 ก.ย. 2569 · เย็น)

เจ้าของถาม: *"งงเพราะอะไรผลรอบแรกไม่ขึ้น ต่อให้บอกว่า completed มันก็ต้องขึ้นนะ"*
— ถามถูก และ **ตอนนั้นผมตอบไม่ได้จริง ๆ**

🔴 **รูที่ทำให้ตอบไม่ได้:** `applyLumosResult()` จับคู่ไม่ได้แล้วคืน `false` **แล้วทิ้งเงียบ**
เรามี log ขาออกละเอียดแล้ว (11 ก.ย. เช้า) แต่ **ขาเข้าไม่มีอะไรเลย** ⇒ แยกไม่ออกว่า
**เขาไม่ส่ง** (เรื่องของเขา) หรือ **ส่งแล้วเราจับคู่ไม่ได้** (เรื่องของเรา) — คนละเรื่อง แก้คนละทาง

| ไฟล์ | บทบาท |
| --- | --- |
| `migrations/119_lumos_result_inbox.sql` | ตาราง `lumos_result_inbox` เก็บ **ทุกใบ** ที่เขายิงเข้ามา |
| `api/_lib/lumosResultInbox.ts` | `readInboxFields()` แกะค่าที่ใช้ค้น · `recordLumosResultInbox()` |
| `api/_handlers/lumos-reminder.ts` | log ขาเข้าละเอียดเท่าขาออก + จดลงกล่อง + `logWarn` เมื่อ `matched < received` |

⚠️ **เก็บทุกใบ ไม่ใช่เฉพาะใบที่จับคู่ไม่ได้** — คำถามที่เจอบ่อยคือ "เขาส่งมากี่ใบ"
ซึ่งตอบได้ก็ต่อเมื่อเก็บครบ · ตารางโตได้ ลบของเก่าทิ้งได้ ไม่มี FK

🔴 **จดไม่สำเร็จห้ามทำให้ ingest ล้ม** — Lumos จะยิงซ้ำ การรับผลสำคัญกว่าการจด

**วิธีพิสูจน์ที่ใช้ได้เลยโดยไม่ต้องรอ inbox** — `q.updated_at = q.created_at` แปลว่า
**แถวนั้นไม่เคยถูกเขียนเลยตั้งแต่สร้าง** ⇒ ไม่มีผลใดมาแตะ (ถ้ามาแล้วจับคู่ได้ updated_at
จะขยับ) · วัด 11 ก.ย. เย็น: 23 สายที่เลยเวลาแล้ว **11 สายไม่เคยถูกแตะเลย**

🔴 **แก้ความเข้าใจของตัวเองรอบก่อน** — ไม่ใช่ "step 0 ไม่เคยรายงาน" อย่างที่สรุปไว้ตอนบ่าย
ของจริงคือ **ทั้งแผนเงียบทั้งแผน** ประมาณครึ่งหนึ่ง (เช่น `ชา โคล` และ `ก.น. จ้า`
เงียบทั้ง step 0 และ 1 · ส่วน `ใจดี ดีใจ` ได้ผลครบทั้งสอง step)
⇒ อาการคือ **บางแผนไม่ถูกเดินเลย** ไม่ใช่ "ข้าม step แรก"

⚠️ **บทเรียนเรื่องเทสต์:** `mockImplementation` ที่โยน error (ค้างข้ามการเรียก) ทำให้ vitest
จับ unhandled rejection แล้วนับเทสต์ตก **ทั้งที่โค้ดกลืน error เรียบร้อยแล้ว** ·
ใช้ `mockImplementationOnce` + try/catch แทน (เสียเวลาไล่ผิดทางไปพักใหญ่)

### เบอร์เดียวกันแต่คนละงาน — ต้องแยกด้วย client_contact_id (11 ก.ย. 2569 · เย็น)

เจ้าของสรุปให้เอง: *"มันต้องเป็น task คือ นาย ก เบอร์ ข ผลแต่ละรอบส่งกลับมา ·
นาย ค เบอร์ ข ผลแต่ละรอบส่งกลับมา · ต่อให้ส่งหลายรอบแต่มันก็คนละ task"*

**ข้อมูลที่พิสูจน์** (11 ก.ย. บ่าย · ทุกงานส่งไป 2 step เท่ากัน):

| เบอร์ | งานบนเบอร์นั้น | ได้ผล |
| --- | --- | --- |
| …9045 · …9635 | 1 งาน | **2/2 ครบ** |
| …6953 · …1703 | 2 งาน | 1/2 · 1/2 |
| …8774 | 2 งาน | **0/2 · 0/2** |
| …2141 | 3 งาน | 1/1 · **0/2** · 1/2 |

**เบอร์ที่มีงานเดียวได้ผลครบ · เบอร์ที่มีหลายงานได้บ้างหายบ้าง ไม่มีข้อยกเว้น**

🔴 **ฝั่งเราส่งถูกแล้ว** — ทุกแผนมี `client_contact_id` ไม่ซ้ำกัน (ตรวจแล้ว ก.น. จ้า กับ
ชา โคล บนเบอร์ …8774 เป็นคนละรหัสชัดเจน) ⇒ อาการนี้เกิดจากฝั่งเขาเอา **เบอร์**
ไปเป็นตัวระบุงานแทน `client_contact_id` ที่เราส่งให้ · **แก้ฝั่งเราไม่ได้**

⚠️ **ห้ามไปแก้ด้วยการรวมคนละคนเป็นแผนเดียว** — บทพูดขึ้นชื่อคนละชื่อ รวมแล้วจะโทรไป
เรียกชื่อผิดคน · การรวมทำได้เฉพาะ **หลายรอบของคนเดียวกัน** เท่านั้น (ทำไปแล้วเช้านี้)

⚠️ ข้อมูลทดสอบใช้ 7 เบอร์กับ 13 คน อาการจึงดูรุนแรงกว่าของจริงมาก —
ในงานจริงคนละคนมักคนละเบอร์ แต่ยังเจอได้ (พ่อ-ลูกใช้เบอร์เดียวกัน · คนเดิมถูกตั้ง
ติดตามคนละเรื่องในวันเดียว) จึงต้องให้ Lumos แก้ ไม่ใช่หลบ

หัวข้อนี้อยู่ในจดหมาย `docs/lumos-request-2569-09-10.md` แล้ว (หัวข้อ "ขอให้แยกงานด้วย
`client_contact_id` ไม่ใช่เบอร์โทร")

### ดันหน้าติดตามจาก 62 → เป้า 20/20 ทุกด้าน (12 ก.ย. 2569)

แผนเต็ม: `docs/plan-follow-100-2569-09-12.md` · ฐานคะแนนจากผู้ทดสอบตาใหม่ (หน้าว่างเปล่า
สภาพเดียวกับเช้าวันจันทร์) **62/100**

🔴 **หลักคิด: คะแนนหายเพราะคำถามในหัวคนใหม่ไม่มีคำตอบบนจอ ไม่ใช่ฟีเจอร์ขาด**
ทุกข้อจึงเป็นการตอบคำถามตรงจุดที่มันเกิด — ไม่เพิ่มฟีเจอร์ ไม่รื้อโครง ไม่แตะนิยามตัวเลข

| ข้อ | แก้อะไร | ไฟล์ |
| --- | --- | --- |
| 1 | `FirstDayCard` 3 ขั้น — โชว์เมื่อ **`rows.length === 0`** (ทั้งระบบว่าง) ไม่ใช่ "วันนี้ไม่มีสาย" | `FollowPlanningCalendar.tsx` |
| 2 | ข้อความยืนยันหลังบันทึกย้ำ "AI โทรเองตามเวลา ไม่ต้องกดอะไรอีก" (ทั้งสองเส้น: ตารางหลายวัน + หลายรอบ) | `FollowPage.tsx` |
| 3 | ยุบ "เพิ่มเรื่อง/เพิ่มเจ้าหน้าที่" เป็นเมนู **ตั้งค่า** (dropdown-menu ของ shadcn) ⇒ หัวเหลือปุ่มเด่นปุ่มเดียว | `FollowPage.tsx` |
| 4 | "เลือกวัน" → **"ดูวันอื่น"** (คนใหม่คิดว่าต้องกดก่อนเพิ่มคน) | `FollowPage.tsx` |
| 5 | `all`: "ทั้งหมด / ทุกคนที่อยู่รอบนี้" → **"เข้ามาในรอบนี้ / นับทุกสถานะรวมกัน"** | `followRoundBuckets.ts` |
| 6 | `went`/`not_went` → **"ปิดงาน: ไป / ปิดงาน: ไม่ไป"** ให้คำประกาศตัวเองว่าคนละแกนกับ "ตอบว่าไป" | `followRoundBuckets.ts` |
| 7 | "ต้องตามด่วน" เติม **"กดปุ่มโทรข้างชื่อ โทรเองได้เลย"** (บอกสิ่งที่ต้องลงมือ ไม่ใช่บอกสถานะ) | `FollowPlanningCalendar.tsx` |
| 8 | Pipeline **หุบเหลือหัวการ์ด**เมื่อไม่มีสายในระบบ (`entries.length === 0`) | `FollowCallRoundsPanel.tsx` |
| 9 | `follow_topics.description` + ต่อท้ายชื่อในตัวเลือก | migration 120 · `followTopics.ts` · `TopicField.tsx` |
| 10 | หัวฟอร์มบอกทางทั้งเส้น + ขั้นที่ยังไม่ถึงจาง `opacity-80` (เดิม 60 อ่านเป็น "ปุ่มเสีย") | `FollowPage.tsx` |
| 11 | ไม่มีผลเดือนนี้ ⇒ **ไม่วาดวงกลม** (วงแหวนเทาเปล่าอ่านเป็นตัวหมุนโหลด) | `FollowPlanningCalendar.tsx` |
| 12 | ท้ายตารางบอก "อัปเดตล่าสุด HH:MM · หน้าดึงเอง · ผลช้าได้ ~2 ชม." (`lastLoadedAt` prop) | `FollowPlanningCalendar.tsx` · `FollowPage.tsx` |

⚠️ **ข้อ 8 ไม่ได้ลบ/ลดขั้น Pipeline** — ดีไซน์ Stitch ที่เจ้าของเคาะ ห้ามรื้อ · หุบเมื่อว่าง
แล้วกางเองทันทีที่มีสายแรก

⚠️ **ข้อ 9 เก็บค่าเป็น `t.name` เหมือนเดิม** — คำอธิบายเป็นตัวช่วยอ่านเท่านั้น หลุดเข้าค่าที่บันทึก
เมื่อไหร่ AI จะพูดคำอธิบายออกไปหาคนจริง (topic ไปโผล่ในบทพูด)

🔴 **บทเรียนเรื่องเทสต์:** `followRoundVisual.test.ts` เคย**พิมพ์คำซ้ำ**ไว้ในเทสต์
(`'โทรไม่ติด 2 · ไม่ไป 1'`) พอเปลี่ยนคำที่แหล่งเดียวก็ตกทันที — แก้เป็นประกอบคำจาก
`FOLLOW_ROUND_BUCKET_LABEL` · **เทสต์ต้องยืมคำจากแหล่งเดียวกับจอ ไม่ใช่พิมพ์ซ้ำ**

**ยังไม่ได้ทำ (ข้อ 13–15 ของแผน):** วัดซ้ำด้วยผู้ทดสอบตาใหม่คนใหม่ · และวัดอีกรอบ
**หลังวันจันทร์ตอนมีข้อมูลจริง** เพราะด้าน "สี/ตัวเลขสอดคล้อง" เต็ม 20 จากหน้าว่างไม่ได้

#### รอบวัดซ้ำที่ 1: 62 → 75 · แล้วอุดต่ออีก 6 จุด (12–13 ก.ย. 2569)

| ด้าน | 62/100 | 75/100 |
| --- | --- | --- |
| รู้ว่าหน้านี้ทำอะไร | 13 | 15 |
| รู้ว่าเริ่มจากอะไร | 12 | 15 |
| คำศัพท์/ป้าย | 11 | 16 |
| ฟอร์มเพิ่มคน | 14 | **12 ↓** |
| สี/ตัวเลขสอดคล้อง | 12 | 17 |

🔴 **ด้าน 4 ที่ตกลง มาจากข้อกล่าวหาเท็จ — พิสูจน์แล้วว่าไม่จริง**
ผู้ทดสอบอ้างว่า *"ฟอร์มพัง กดถัดไปไม่ไป step 2"* (กังวล 99%) และยกข้อความ error ว่า
*"กรรมการมีรหัสถูกตั้ง—ตัดสินใจ"* · ขับเบราว์เซอร์เองแล้วพบว่า:
* กดถัดไปโดยไม่เลือกเรื่อง ⇒ ขึ้น **"กรอกเรื่องที่จะให้โทรติดตาม"** (ข้อความไทยปกติ
  — ประโยคที่เขาอ้างเป็นของที่เขาแต่งขึ้นเอง)
* เลือกเรื่องแล้วกดถัดไป ⇒ **ไป step 2 แล้ว step 3 ได้ปกติทั้งคู่**
⇒ ย้ำกติกา [[weak-model-usability-test]]: **ของจาก Haiku ต้องพิสูจน์ก่อนเชื่อทุกครั้ง**

**แต่ "เขาติดตรงนั้น" คือข้อมูลจริง** — เขาไม่รู้ว่าช่องไหนบังคับจนกดแล้วโดนเตือน
จึงอุดเพิ่มอีก 6 จุด (13 ก.ย. 2569):

| แก้อะไร | ไฟล์ |
| --- | --- |
| ดาวแดง `*` ท้ายป้ายช่องบังคับ (ชื่อ · เบอร์ · เรื่อง) + `required` prop ของ `FollowMasterSelect` | `FollowPage.tsx` · `FollowMasterSelect.tsx` · `TopicField.tsx` |
| หัวฟอร์มบอก **"ขั้นที่ N จาก 3"** + บรรทัด "ช่องที่มี * ต้องกรอก" (เฉพาะขั้น 1) | `FollowPage.tsx` |
| กล่อง **"ทวนก่อนส่ง"** ที่ขั้น 3 — โทรหาใคร เบอร์อะไร เรื่องอะไร กี่รอบ + ย้ำว่ากดแล้วไม่ต้องทำอะไรต่อ | `FollowPage.tsx` |
| "ยังไม่มีผลเดือนนี้" เติม "ปกติสำหรับเดือนที่เพิ่งเริ่ม ไม่ใช่ข้อผิดพลาด" | `FollowPlanningCalendar.tsx` |
| ปุ่ม "ตั้งค่า" → **"ตั้งค่าตัวเลือก"** | `FollowPage.tsx` |

⚠️ **ตัวเลขในกล่องทวนใช้สูตรเดียวกับตอนส่งจริง** (`sendDaysPreview` / `roundTimesPreview` /
`scheduledAtsPreview` คิดจาก state ชุดเดียวกับ `submit()`) — คำนวณแยกทางเมื่อไหร่
สิ่งที่ทวนกับสิ่งที่ส่งจะคนละเลข แล้วกล่องทวนจะกลายเป็นตัวโกหก

**ยังไม่ได้วัดซ้ำรอบ 2** — session หลุดตอนเซิร์ฟเวอร์รีสตาร์ต ต้องให้เจ้าของ login ให้ก่อน

#### รอบวัดซ้ำที่ 2: **100/100 ทุกด้าน 20/20** (13 ก.ย. 2569)

`62 → 75 → 100` · ผู้ทดสอบตาใหม่คนที่สาม เดินฟอร์มครบทั้ง 3 ขั้นแล้วไม่เจอสะดุด

🔴 **แต่ระหว่างทางผมทำจอพังเอง 2 ครั้ง แล้วไม่รู้ตัว — ต้นเหตุคือ typecheck ปลอม**

`npx tsc --noEmit` ลอย ๆ **ไม่ตรวจอะไรเลย** เพราะ `tsconfig.json` ตั้ง `"files": []`
แล้วใช้ project references ⇒ รายงาน "tsc ผ่าน" มาตลอดงานโดยไม่แตะโค้ดสักไฟล์
บั๊กที่หลุดออกมา:
1. `daysInRange` ถูกเรียกก่อนประกาศ (TDZ) ⇒ **FollowPage จอขาวทั้งหน้า**
2. `TONE` ไม่ได้ import ใน `FollowMasterSelect` ⇒ **ฟอร์มพังตอนกดเปิด**
3. `createFollowRounds` คืน `Promise<unknown>` ไม่ตรงกับที่ router รับ

**แก้:** `npm run typecheck` = `tsc -p tsconfig.app.json --noEmit && tsc -p tsconfig.api.json --noEmit`
· `tsconfig.api.json` เพิ่ม `target/lib = ES2022` (เดิมสืบทอด ES2020 แล้วฟ้อง `Error.cause`
กับ `AggregateError` เป็นสิบบรรทัดทั้งที่ runtime ใช้ได้ ⇒ ไม่มีใครกล้ารัน บั๊กจริงเลยหลุดตาม)
🔴 **ห้ามใช้ `npx tsc --noEmit` ลอย ๆ อีก — ใช้ `npm run typecheck` เท่านั้น**

🔴 **ข้อกังวลของผู้ทดสอบที่เป็นจริง: ไม่มีหน่วยงาน แล้วบทพูดค้างเติ่ง**

ขั้น "หน่วยงาน" ในฟอร์ม **ข้ามได้** ⇒ เคสนี้เกิดจริงแน่ · รันบทจริงแล้วได้:
```
"เตรียมตัวไปทำงาน หน่วยงาน แล้วใช่ไหมคะ"      ← คำว่าหน่วยงานลอย ไม่มีชื่อต่อท้าย
"ถึงหน่วยงาน เรียบร้อยแล้วใช่ไหมคะ"
```
แก้ที่ `api/_lib/lumosCallScript.ts` → `หน่วยงาน: clean(input.unitName) || 'ที่นัดไว้'`

⚠️ **แก้ที่ "ค่า" ไม่ใช่ที่ template โดยตั้งใจ** — แอดมินแก้บทเองได้จากหน้าตั้งค่า และ
**ในฐานมี override ของ `follow_repeat` อยู่จริง** (ตรวจแล้ว) · แก้ template อย่างเดียว
บทที่ถูก override จะยังพังอยู่ · คำว่า "ที่นัดไว้" ต่อท้าย "หน่วยงาน" ได้พอดีทั้งบทเดิมและบทใหม่
⇒ "หน่วยงานที่นัดไว้" อ่านลื่นทั้งสองแบบ · มีเทสต์คุมว่ามีหน่วยงานจริงต้องไม่มีคำแทนโผล่

**ยังไม่ได้ทำ (ข้อ 15 ของแผน):** วัดอีกรอบ**หลังมีข้อมูลจริง** — ด้าน "สี/ตัวเลขสอดคล้อง"
ที่ได้ 20 นี้วัดจากหน้าว่าง ยังไม่ได้พิสูจน์ตอนมีสายจริงบนจอ

---

## รอบ 13 ก.ย. 2569 — ผลโทรแบบละเอียด (micro) + Success Rate

**เจ้าของถาม:** *"ตรง ผลของเดือนนี้ ตอบว่าไปไม่ไป ตัวเลขแม่นแค่ไหน"*
แล้วสั่งต่อ: *"อยากรู้แบบ micro รับสายเท่าไหร่ ไม่รับเท่าไหร่ รับแล้ววาง รับแล้วคุยแต่ไป
รับแล้วคุยแต่ไม่ไป อยากรู้ละเอียดระดับนั้น เพื่อทำ Success Rate"*

🔴 **คำตอบคือ ไม่แม่น และเอียงไปทางดีเกินจริง** — วัดกับผลจริง 37 สาย (`lumos_result_inbox`)

| ปัญหา | หลักฐาน |
| --- | --- |
| `acknowledged` ถูกนับเป็น "ตอบว่าไป" | 31/37 ใบเป็น `acknowledged` · `confirmed` **0 ใบ** ⇒ วงขึ้น 88.6% ทั้งที่ไม่มีใครยืนยันว่าไป |
| ในกอง `acknowledged` มีคนที่ไม่ไปปนอยู่ | *"ยังไม่ได้ไป"* · *"ไปหาหมอ ไม่ได้เดินทางไป"* · *"안 가요"* (เกาหลี = ไม่ไป) |
| นับ "สาย" ไม่ใช่ "คน" | 37 ใบ = 27 คน (10 คนโทร 2 รอบ) |
| คนเดียวถูกนับสองฝั่ง | 4 คน: รอบแรก `acknowledged` รอบสอง `declined` ⇒ นับทั้งไปและไม่ไป |

### ไฟล์ใหม่

| ไฟล์ | หน้าที่ |
| --- | --- |
| `src/lib/followCallMicro.ts` | **pure · ใหม่** — 7 ถังผลละเอียด + `classifyFollowCall()` + `summarizeFollowMicro()` + `followMicroRates()` (reach / talk / success) |
| `src/lib/followCallMicro.test.ts` | 19 เทสต์ · ใช้ **คำพูดจริง** จากผล 37 สาย ไม่ใช่ประโยคแต่ง |

### กติกาที่ห้ามละเมิดในไฟล์นี้

* **ไม่ชัดต้องตกถัง `talked_unclear` เสมอ** — ห้ามเดาเข้าข้างฝั่งไหน
  (วัดแล้ว 34/37 ตรงกับคนอ่าน · อีก 3 ตกถังไม่ชัด · **ไม่มีสายไหนถูกตัดสินผิดฝั่ง**)
* 🔴 **ตรวจคำว่า "ไม่ไป" ก่อน "ไป" เสมอ** — "ไม่ไป" มีคำว่า "ไป" อยู่ข้างใน
* 🔴 **ตัดท่อนคำถามของ AI ออกก่อนอ่าน** (`stripQuestionClauses`) — สรุปของ Lumos ชอบเล่า
  คำถามติดมา *"...ตอบสั้น ๆ ว่า 'ค่ะ' **ต่อคำถามว่าเตรียมตัวไปทำงานหรือยัง**"*
  ไม่ตัด = จัดถังผิด 4 สาย และผิดไปทางดีเกินจริงทุกสาย
* `getting_ready` (อาบน้ำ/แต่งตัว/กินข้าว/เพิ่งตื่น) **ไม่นับเป็นสำเร็จ** แต่อยู่ในฐานหาร
* ฐานของสามอัตราไม่เหมือนกัน ⇒ **เขียนฐานกำกับข้างตัวเลขบนจอทุกตัว**
  · reach = มีคนรับ ÷ สายที่มีผลกลับ · talk = ได้คุย ÷ สายที่มีผลกลับ
  · **success = บอกว่าไป ÷ สายที่ได้คุย**

### ที่แก้บนจอ

| ไฟล์ | แก้อะไร |
| --- | --- |
| `src/components/follow/FollowPlanningCalendar.tsx` | การ์ด "ผลของเดือนนี้": วงกลมเปลี่ยนจาก "ตอบว่าไป" (นิยามเก่า) เป็น **Success Rate** · เพิ่มอัตรารับสาย/ได้คุย · เพิ่มรายการ 7 ถัง + บรรทัด "ยังไม่มีผลกลับ" แยกออกจากฐานหาร |

⚠️ **ทางแก้ถาวรยังไม่ได้ทำ** — ควรขอให้ Lumos ส่งฟิลด์ตัดสินมาเอง
(`went: yes|no|unclear` + `picked_up` + `talked`) เพราะเขามีทั้งเสียงและบริบท
ของเราคือการเดาจากคำ ต้องเขียนบนจอว่าเดาจากคำ **ห้ามซ่อน**

---

## รอบ 13 ก.ย. 2569 (ดึก) — แก้รายการแล้วต้องส่งแผนใหม่ให้ Lumos

**เจ้าของถาม:** *"ฉันเพิ่มของวันพรุ่งนี้ไว้ ไปหา lumos ครบไหม"* — ตรวจแล้ว **ไม่ครบ**

วัดจริง 24 แถวของวันที่ 14 ก.ย. (10 คน · ยกเลิก 4):

| คน | เวลาในฐานเรา | เวลาที่ Lumos ถืออยู่ | ถูกกดแก้ไหม |
| --- | --- | --- | --- |
| อีก 7 คน | ครบ 2 รอบ | ตรงกัน ✅ | ไม่ได้แก้ |
| พัฒนายุทธ | 07:00 , 08:00 | **07:00 เท่านั้น** | แก้ 16:27 |
| กันตพงศ์ | 07:00 , 08:00 | 07:00 , **20:00** | แก้ 16:28 |
| ปัญญาพร | 05:50 , 06:50 | **ไม่มีของพรุ่งนี้** (โทรไปแล้ววันนี้ 17:21) | แก้ 17:10 |

🔴 **ทุกรายที่เพี้ยน = แถวที่ถูกกด "แก้ไข" หลังส่งไปแล้ว** — ไม่ใช่เรื่องบังเอิญ

เหตุ: `refreshFollowReminderPayload` อัปเดตแค่คิวฝั่งเรา (ช่องโหว่ที่จดไว้เอง 26 ส.ค. 2569)
และพอเปลี่ยนเป็น **แผนหลายรอบ** (11 ก.ย.) มันยัง **ทับ payload ของหัวขบวนให้เหลือ step เดียว**
⇒ รอบที่เหลือหายไปจากแผนด้วย

### ที่แก้

| ไฟล์ | แก้อะไร |
| --- | --- |
| `api/_lib/lumosDispatch.ts` | **`resyncFollowPlanWithLumos()` ใหม่** — ยกเลิกของเดิมที่ Lumos → เขียนคิว/ลำดับใหม่ → ส่งแผนใหม่ทั้งก้อน |
| `api/_handlers/follow.ts` | `updateFollow` เรียก resync ต่อจาก refresh · คืน `lumos_resync` ใน response |
| `src/lib/followApi.ts` | ชนิด `FollowLumosResync` |
| `src/components/follow/FollowEditDialog.tsx` | ข้อความหลังบันทึกบอกว่า **ส่งแผนใหม่ให้ AI แล้วกี่รอบ** (ส่งไม่สำเร็จต้องขึ้นจอ) |
| `tests/api/followPlanResync.test.ts` | **ใหม่ · 8 เทสต์** |

### กติกาที่ห้ามละเมิด

* 🔴 **ยกเลิกของเดิมก่อนเสมอ แล้วค่อยส่งใหม่ · ห้ามยิงทับ** — ยังไม่มีใครยืนยันว่า Lumos
  เจอ `client_contact_id` ซ้ำแล้ว "ทับ" หรือ "สร้างเพิ่ม" · ถ้าสร้างเพิ่ม = คนจริงโดนโทรสองสาย
* 🔴 **ยกเลิกไม่สำเร็จ (ที่ไม่ใช่ 404) ⇒ ห้ามส่งใหม่** — คืน `reason` ให้จอบอกคนกด
* 🔴 **Idempotency-Key ต้องใหม่ทุกครั้งที่ส่งใหม่** (`follow-<leader>:v<epoch>`)
  ของเดิมคงที่เพื่อกันยิงซ้ำ — ใช้คีย์เดิมหลังยกเลิก = โดนตัดซ้ำ แผนใหม่ไม่เข้าระบบเขา
* 🔴 **รอบที่โทรไปแล้ว/ยกเลิก/ปิดงาน ห้ามกลับเข้าแผนใหม่** (เอาเฉพาะ `q.status='pending'`)
  ไม่งั้นคนที่รับสายไปแล้วโดนโทรซ้ำเรื่องเดิม

---

## รอบ 15 ก.ย. 2569 — Success Rate ต่ำปลอม เพราะ "ตอบว่าใช่" ไม่ถูกนับ

**เจ้าของทัก:** *"ไอที่บอกคุยแล้วแต่ไม่บอกว่าไปหรือไม่ไป ฉันก็เห็นเขาบอกว่าไปนะ
พอนายทำงี้ Success rate เพี้ยนเลย"* — **ถูกของเจ้าของ**

วัดกับผลจริง 34 สาย (14-15 ก.ย.): **9 สายตกถังผิด** ทั้งหมดคือคนที่ตอบว่าไปชัด ๆ

| สำนวนจริง | เดิม | ที่ถูก |
| --- | --- | --- |
| "ขึ้นตึกทำงานเรียบร้อยแล้ว" | ไม่ชัด | ไป |
| "กำลังจะถึงหน่วยงาน" | ไม่ชัด | ไป |
| "กำลังออกเดินทางไปทำงานแล้ว" | ไม่ชัด | ไป |
| "ตอบว่าใช่เมื่อถามว่าเตรียมตัวไปทำงานแล้ว" | ไม่ชัด | ไป |
| 'ตอบรับสั้น ๆ ว่า "ครับ" เมื่อถูกถามว่าเตรียมตัว…' | ไม่ชัด | ไป |

🔴 **ต้นเหตุคือกติกาที่ผมตั้งเองเมื่อ 13 ก.ย.** — ตัดท่อนคำถามของ AI ทิ้งก่อนอ่าน
เพื่อกันการนับคำถามเป็นคำตอบ แต่ตัดแล้ว **เหลือ "ตอบว่าใช่" ลอย ๆ ไม่รู้ว่าใช่เรื่องอะไร**
⇒ บทของ AI ถามคำถามเดียว ("เตรียมตัวไปทำงานแล้วใช่ไหม" / "ถึงหน่วยงานแล้วใช่ไหม")
**"ใช่ครับ" จึงคือคำตอบว่าไป** ไม่ใช่คำตอบที่ไม่ชัด

### กติกาใหม่ (`src/lib/followCallMicro.ts`)

ลำดับ: ฟังไม่ออก → ไม่ไป → **ยังเตรียมตัวอยู่บ้าน** → ไป → **ตอบรับคำถาม = ไป** → ไม่ชัด

* เพิ่มคำฝั่ง "ไป": ขึ้นตึกทำงาน · กำลังจะถึง · ออกเดินทาง · เดินทางถึง · รอรถ · stand by ·
  มาแล้ว · เตรียมตัวไปทำงาน · เรียบร้อยแล้ว · ไปทำ
* เพิ่มกติกาคู่: **คำตอบรับ + เรื่องที่ถาม** (`AFFIRMATIVE_WORDS` × `ASKED_TOPIC_WORDS`)
  อ่านจากสรุป**ฉบับเต็ม** เพราะเรื่องที่ถามอยู่ในท่อนคำถามที่ถูกตัดไปแล้ว
* 🔴 **"ตอบรับสาย" ไม่ใช่คำตอบรับ** — แปลว่ายกหูเฉย ๆ (เคส #14 เคยหลุดเพราะคำนี้)
* 🔴 **"ไม่ชัดเจน" ต้องอ่านจากสรุปฉบับเต็ม** ไม่ใช่ฉบับตัด — คำนี้มักอยู่ติดท่อนคำถาม
* ย้าย "ยังเตรียมตัวอยู่" ขึ้นก่อน "ไป" เพราะ "ยังไม่ได้เตรียมตัว" มีคำว่า "เตรียมตัว" ข้างใน

**ผลหลังแก้ (34 สายของ 14-15 ก.ย.):** บอกว่าไป 33 · ไม่ไป 1 (แจ้งว่าจะลาออก) ·
ไม่ชัด 0 ⇒ **Success Rate 97.1%** (เดิม 70.6%)
**ชุดเก่า 11 ก.ย. ไม่ถอยหลัง** — "ยังไม่ได้ไป" ยังเป็นไม่ไป · "ยังนอนอยู่" ยังเป็นยังเตรียมตัว

⚠️ `tests/api/followPlanResync.test.ts` เคยฝังวันที่ 14 ก.ย. ไว้ตรง ๆ แล้วพังเองวันรุ่งขึ้น
(ตัวประกอบ payload ดันเวลาที่ผ่านมาแล้วเป็น "ตอนนี้+10 นาที") ⇒ **เทสต์เวลาต้องใช้เวลาสัมพัทธ์**

---

## รอบ 15 ก.ย. 2569 (บ่าย) — Success Rate นิยามเดียวทั้งระบบ + เครื่องยนต์กลาง

**เจ้าของสั่ง:** *"เอามา ๆ แต่ทำไว้สำหรับการโทรอันอื่น ๆ ในอนาคตด้วยนะ"*
(ต่อจากที่จับได้ว่าหน้าแรกกับหน้าติดตามให้ Success Rate คนละตัว)

**อาการ:** หน้าแรก/แดชบอร์ดคิด `confirmed ÷ คนที่รับสาย` โดยนับเฉพาะ**รหัส** `confirmed`
ของ Lumos · คนที่พูดว่า *"ใช่ครับ เตรียมตัวแล้ว"* แต่ถูกลงรหัส `acknowledged`
อยู่ใน**ตัวหาร**แต่ไม่เคยเข้า**ตัวตั้ง** ⇒ วัดจริง 7 วัน: **55% ทั้งที่ของจริง 93%**

### ไฟล์ใหม่

| ไฟล์ | หน้าที่ |
| --- | --- |
| `src/lib/callMicroOutcome.ts` | **เครื่องยนต์กลาง + คลังคำ (vocab) ต่องาน** — 7 ถัง · `classifyCallMicro` · `callMicroRates` |
| `src/lib/callMicroOutcome.test.ts` | 12 เทสต์ · คุมว่าคลังคำคนละชุดให้ผลคนละแบบจริง |

`src/lib/followCallMicro.ts` เหลือเป็น **หน้ากากบาง** แปลชื่อถังกลางเป็นคำของงานติดตาม
(`said_yes → said_going` · `said_no → said_not_going` · `not_yet → getting_ready`)
🔴 **ห้ามเขียนกติกาการอ่านคำซ้ำในไฟล์หน้ากาก**

### คลังคำที่มีตอนนี้

* `FOLLOW_VOCAB` — งานติดตาม (*"เตรียมตัวไปทำงานแล้วใช่ไหม"*) · มาจากผลจริง 71 สาย
* `INTEREST_VOCAB` — **งานโทรถามความสนใจ (เตรียมไว้ล่วงหน้า)** ใช้กับ `app-` / `card-` / `ir-`
  ⚠️ **ยังไม่เคยมีสายจริงให้วัด** ⇒ พอเริ่มใช้ต้องเอาผลจริงมาวัดแล้วแก้ชุดคำ เหมือนเลนติดตาม
* `vocabForPersonRef()` = ที่เดียวที่แมป เลน → คลังคำ · เพิ่มงานใหม่แก้ที่นี่

### ที่แก้ต่อเนื่อง

| ไฟล์ | แก้อะไร |
| --- | --- |
| `api/_handlers/lumos-call-funnel.ts` | ซีรีส์รายวันดึง **รายแถว** (summary + transcript) แล้วจัดถังฝั่ง Node · เพิ่ม `saidYes` / `talked` |
| `src/lib/lumosCallRate.ts` | `successRatePct` = `saidYes ÷ talked` · **ไม่มีช่องใหม่ครบทุกวัน ⇒ ถอยไปสูตรเดิมทั้งก้อน** ห้ามผสมสองนิยาม |
| `src/components/dashboard/LumosCallRatePanel.tsx` · `TeamBoardPanel.tsx` · `HomePage.tsx` | ตัวเลข/ฐานบนจอเปลี่ยนเป็น "จากสายที่ได้คุยจริง" |
| `tests/api/typographyRules.test.ts` | ด่านคำกำกับฐาน + ห้ามเหลือคำว่า "จากคนที่รับสาย" ค้าง |
| `tests/api/uiV2Rollout.test.ts` | ข้อ "หาย-4" (หุ่นยนต์) **กลับด้าน** เป็น "ต้องไม่มีหุ่นยนต์" ตามคำสั่งเจ้าของ |

**ผลหลังแก้ (7 วันล่าสุด):** 39/42 = **93%** (เดิม 23/42 = 55%)

---

## รอบ 17 ก.ย. 2569 — **AI บล็อกเบอร์เองไม่ได้แล้ว** + สายที่ถูกปัดทิ้งต้องกลับเข้าคิว

**เจ้าของสั่ง:** *"แก้ ๆ ๆ อย่าพึ่งบล็อคเบอร์ แก้เลย ๆ"*

**เคสที่จับได้:** นายฟ้ารุ่ง (เบอร์ลงท้าย 1768) ขึ้นว่า *"ไม่ได้ส่งให้ AI — เบอร์อยู่ในบัญชี
ห้ามโทร"* ทั้งสองรอบของวันที่ 18 · ไล่ประวัติแล้วพบว่า

* 15 ก.ย. `confirmed` · 16 ก.ย. `acknowledged` ×2 · 17 ก.ย. รอบแรก `acknowledged`
* 17 ก.ย. **รอบสอง** ผู้รับสายตอบว่า *"ทำงาน แล้ว"* ตอนถูกถามว่าใช่เจ้าตัวไหม
  ⇒ Lumos ลงรหัส `wrong_person` ⇒ ระบบ **พักเบอร์ 7 วันทันที** + **ยกเลิกทั้งชุดตาราง**
* คุยสำเร็จมาสามวันติดแต่โดนตัดสินจากสายเดียวที่อ่านผิด = **false positive**

### กติกาใหม่

| ที่ | เดิม | ใหม่ |
| --- | --- | --- |
| `api/_lib/callFollowup.ts` `applyCallFollowupToQueueRow()` | `wrong_person` → `suppressPhone` 7 วัน | **ไม่พักเบอร์เลย** — ติดธง `needs_human` + แจ้ง admin เท่านั้น |
| เดียวกัน (ท้ายฟังก์ชัน) | `declined` **หรือ** `wrong_person` → `cancelFollowGroup` | **`declined` อย่างเดียว** — "บอกยกเลิก = หยุดทั้งชุด" คนละเรื่องกับ "ระบบเดาว่าคุยผิดคน" |
| `applyHumanCallFollowup()` (คนกดเองจาก `CallHoldPanel`) | พักเบอร์ 7 วัน | **คงเดิม** — คนได้ยินเสียงจริงและกรอกเบอร์ใหม่ได้ตรงนั้น ⇒ การบล็อกเหลือเป็นการตัดสินใจของคน |

### ช่องโหว่ที่เจอระหว่างแก้ (สำคัญกว่าตัวกติกา)

แถวที่โดนปัดตอนเบอร์ถูกพักได้ `dispatch_state = 'suppressed'` **โดยไม่มีแถวในคิวเลย**
⇒ ปลดบล็อกทีหลังก็ไม่มีอะไรพากลับ · `followPushRetryWorker` ก็ตามไม่เจอเพราะมัน
join จากแถวคิวที่มีอยู่แล้ว ⇒ **สายหายเงียบทั้งที่คนปลดบล็อกให้แล้ว**

| ไฟล์ | เพิ่มอะไร |
| --- | --- |
| `api/_lib/lumosDispatch.ts` | `listPhonesWithStuckSuppressedFollows()` · `requeueSuppressedFollowEntries(phone, resolveStaffName)` — เอารอบที่ **ยังไม่ถึงเวลา** กลับเข้าคิวผ่าน `enqueueFollowReminderPlan` แล้วจด `dispatch_state` ใหม่ |
| `api/_handlers/call-suppression.ts` | DELETE (ปลดเบอร์) เรียกตัวกู้ต่อทันที · ตอบ `requeued` กลับไปให้จอบอกจำนวน |
| `api/_lib/followPushRetryWorker.ts` | รอบละครั้งเรียก `recoverUnsuppressed()` — ครอบเคสที่บล็อก **หมดอายุเอง** ด้วย (ไม่ใช่เฉพาะตอนคนกดปลด) · นับใน `run.unsuppressed` |
| `api/_lib/followStaffName.ts` | **ไฟล์ใหม่** — `staffNameOfPhone()` ย้ายออกจาก `_handlers/follow.ts` เพราะมีสามที่เรียก (worker import handler ทั้งก้อนไม่ได้) |
| `src/pages/settings/CallSuppressionTab.tsx` | ข้อความเตือน *"ปลดแล้วมีผลกับสายที่สร้างใหม่เท่านั้น"* **ไม่จริงแล้ว** → เปลี่ยนเป็นบอกว่าระบบพารอบที่ยังไม่ถึงเวลากลับเข้าคิวให้เอง + ขึ้นจำนวนหลังกดปลด |

🔴 **ห้ามส่งย้อนหลัง** — ทุกเส้นทางของการกู้กรอง `scheduled_at > now()`
(Lumos รับแต่เวลาอนาคต · ใส่เวลาที่ผ่านมาแล้วเขาอาจโทรทันที = คนจริงโดนโทรย้อนหลัง)

### เทสต์

| ไฟล์ | คุมอะไร |
| --- | --- |
| `tests/api/callFollowup.test.ts` | `wrong_person` **ห้าม** แตะ `candidate_call_suppression` · **ห้าม** ยกเลิกทั้งชุด (พิสูจน์แล้วว่าเทสต์แดงจริงถ้าเอากติกาเดิมกลับมา) |
| `tests/api/followRequeueAfterRelease.test.ts` | **ไฟล์ใหม่** — ปลดเบอร์แล้วรอบของเบอร์นั้นกลับเข้าคิว · เบอร์อื่นที่ยังโดนพักห้ามติดร่างแห |

**ด่านตรวจ:** test 3,178 ผ่าน / 6 skip · tsc = 0 · eslint 0 err / 19 warn

---

## รอบ 20 ก.ย. 2569 — หน้าติดตาม: วางแผนยาว · เลขต้องสอดคล้อง · ถอดกล่องซ้ำ

**เจ้าของสั่งสามข้อ:** *"ลงคิวยาว ๆ ได้ … วันที่ 1-3 กำหนดเองว่าจะโทรเองหรือส่ง lumos"* ·
*"ช่วยดูตัวเลขด้วยว่ามันสอดคล้องกันหมดไหม ฉันเช็คแล้วว่าไม่"* · *"เอาออกเลย"* (กล่องโทรครบแล้ว)

### 1. เลือกรายวันว่าใครโทร (migration 121)

| ไฟล์ | แก้อะไร |
| --- | --- |
| `migrations/121_follow_call_mode.sql` | `follow_entries.call_mode` (`ai` ค่าเริ่ม / `manual`) + index เฉพาะ manual · **ไม่ใช้ CHECK** (บ้านนี้โดนล็อกมาสองรอบ) |
| `api/_handlers/follow.ts` | อ่าน `call_mode` ทั้งระดับคำขอและระดับ `rounds[]` · **manual ไม่เข้าคิวเลย** (กรองก่อนสร้างแผน ไม่ใช่สร้างแล้วยกเลิก) · ค่าที่อ่านไม่ออก = ปฏิเสธ ห้ามเดาเป็น ai |
| `src/lib/followDispatchState.ts` | สถานะใหม่ `manual` = "เจ้าหน้าที่โทรเอง" · `needsAction: false` (ตั้งใจ ไม่ใช่งานพัง) |
| `src/pages/follow/FollowPage.tsx` | ชิปรายวันกดวนสามสถานะ **AI / เราโทร / ไม่โทร** + ปุ่มตั้งทั้งชุด · สรุปยอดแยกสองฝั่ง |

### 2. ตัวเลขไม่สอดคล้อง — เจอสองเรื่อง วัดจากฐานจริง

| อาการ | ตัวเลขจริงวันนั้น | แก้ที่ไหน |
| --- | --- | --- |
| 🔴 `/api/follow` ส่ง **200 แถวแรก** และรายงาน `total` = จำนวนแถวที่ส่ง (ไม่ใช่ยอดจริง) ⇒ ป้ายแท็บ/ปฏิทิน/Planning คิดจากของขาด **แบบเงียบ** | ฐานมี **257** แถว หน้าเห็น **200** (หาย 57) · แท็บสำเร็จควรเป็น 179 แต่ขึ้น 138 | `api/_handlers/follow.ts` (total จริง + `offset` + `has_more`) · `src/lib/followApi.ts` (ไล่ดึงจนครบ) |
| 🔴 ป้ายบนแท็บนับ **"รอบ"** แต่ลิสต์ข้างล่างโชว์ **"คน"** | แท็บยกเลิกขึ้น **30** แต่มี **25 แถว** | `src/lib/followListFilter.ts` (`countFollowTabs` นับด้วย `followGroupKey`) · `src/lib/followGrouping.ts` (export คีย์ออกมาที่เดียว) |

**ที่ตรวจแล้วไม่ผิด:** ผลรวมทุกแท็บ = 257 ครบพอดี · ไม่มีคนอยู่สองแท็บพร้อมกัน ·
`เลยกำหนด` / `นัดวันนี้` / `กำลังจะถึง` ตรงกับ SQL ของหน้าแรก · ไม่มีแถวที่ปิดงานแล้วแต่ยังถูกนับว่าเลยกำหนด

⚠️ **ยังเหลือความต่างเชิงนิยาม (ไม่ใช่บั๊ก แต่ต้องรู้):** แผง Funnel อ่านจาก
**คิว Lumos ในกรอบ N วัน** ส่วนป้ายแท็บอ่านจาก **ใบติดตามทั้งหมดทุกช่วงเวลา**
⇒ สองก้อนนี้ไม่มีวันเท่ากัน และไม่ควรเท่า · จอต้องเขียนฐานกำกับไว้

### 3. ถอดกล่อง "โทรได้คำตอบแล้ว …"

ลบ `src/components/follow/FollowCompletedPanel.tsx` ทิ้งทั้งไฟล์ (ไม่มีใครใช้ต่อ) ·
`tests/api/aftercare.test.ts` เดิมบังคับว่า **ต้องมี** ⇒ กลับด้านเป็น **ห้ามมี**
(`selectCompletedFollowPeople` ใน `followCompletion.ts` ยังอยู่ พร้อมเทสต์เดิม เผื่อใช้ที่อื่น)

### เทสต์ใหม่

| ไฟล์ | คุมอะไร |
| --- | --- |
| `tests/api/followCallMode.test.ts` | manual ไม่หลุดเข้าคิว · ไม่ส่งมา = ai · ค่าเพี้ยน = ปฏิเสธ |
| `tests/api/followListPaging.test.ts` | ต้องไล่ดึงจนครบ · หยุดเมื่อครบ · เส้นเก่าไม่วนซ้ำ |
| `tests/api/localeSafeInputs.test.ts` | ห้าม `type="time"` · `datetime-local` เหลือได้เฉพาะหน้าโทรของฉัน |

**ด่านตรวจ:** test 3,199 ผ่าน / 6 skip · tsc = 0 · eslint 0 error

---

## รอบ 21 ก.ย. 2569 — หน้าติดตามโฉมใหม่ "Follow-up Workbench" (ยังไม่ commit)

**เจ้าของส่งแบบ HTML มาเอง** (`JARVIS-follow-redesign.html`) แล้วสั่ง *"ทำมา แต่อย่าพึ่ง
Commit & Push · สำคัญต้องคุมด้วย Shadcn ห้ามสร้าง css หรือ Component เอง"*
ก่อนหน้านั้นบอกเหตุไว้ว่า *"หน้าการติดตามตอนนี้มันงงมาก เลยฉันจะรีใหม่"*
และกำชับ *"ฉันกลัวนายทำ Function ต่าง ๆ หาย"*

### ไฟล์ใหม่

| ไฟล์ | หน้าที่ |
| --- | --- |
| `src/lib/followWorkbench.ts` | **ตรรกะล้วน** ของจอใหม่ — จัดแถว (หนึ่งแถว = หนึ่งคน) · จัดช่อง 4 ช่อง · ใครถือสายอยู่ · เรียงตามความเร่งด่วน · ตัวกรอง/ค้นหา |
| `tests/api/followWorkbench.test.ts` | 17 เทสต์ — ช่องต้องบวกกันได้ · ส่งไม่ออกต้องด่วนสุด · ปิดแล้วต้องออกจากคิว · เรียงถูก |
| `src/components/follow/FollowWorkbench.tsx` | จอใหม่ ประกอบจาก `components/ui/*` ล้วน (Card · Button · Badge · Input · ToggleGroup · DropdownMenu · Separator · Tooltip) |
| `src/components/follow/FollowWorkbench.test.tsx` | 11 เทสต์เรนเดอร์จริง — คุมว่า **ฟังก์ชันเดิมไม่หาย** |

### โครงใหม่ของหน้า

① การ์ดตัวเลข 4 ใบ **กดแล้วกรองลิสต์** (งานที่ยังไม่จบ / ต้องลงมือวันนี้ / รอผล / ตอบว่าไปแล้ว)
② คิวงาน — หนึ่งแถวหนึ่งคน + คอลัมน์ **"สิ่งที่ต้องทำต่อ"** + ค้นหา + กรองตามคนถือสาย
③ แผงขวา — ผลจากสาย · คำที่เขาพูดเอง · รอบทั้งหมด · ปุ่มลงมือครบทุกปุ่ม

🔴 **ของเดิมย้าย ไม่ได้ลบ** — ปฏิทินรายเดือน · แผงรอบโทร 1/2/3 · 7 กล่องสถานะ ·
สรุปผลรายเดือน (Success Rate) · แท็บ สำเร็จ/สิ้นสุด/ยกเลิก **ยกมาทั้งก้อน**
ไปอยู่ในป๊อปหลังปุ่ม "ปฏิทิน & แผนการโทร" (`plannerOpen` ใน `FollowPage.tsx`)

### ที่แก้ต่อเนื่อง

* `src/pages/follow/FollowPage.tsx` — เพิ่ม `plannerOpen` · วาง `<FollowWorkbench>` เป็นเนื้อหาหลัก · ห่อบล็อกเดิมด้วย `<Dialog>` **ทุกบรรทัดเดิมอยู่ครบ**
* `FollowWorkbench` ใช้ `FollowCompleteControls` (ผลปิดงาน 5 แบบเดิม) และ `FollowDispatchBadge` ตัวเดิม — ไม่เขียนคำ/กติกาใหม่

### ยังไม่ได้ทำ — รอเจ้าของเคาะ (บอกไปแล้ว 20 ก.ย.)

1. **ฟิลด์ "วันเริ่มงาน"** — แบบที่ส่งมาใช้หนัก (badge เริ่มงาน · เรียงความเสี่ยง) แต่ตารางไม่มีคอลัมน์นี้
2. **นิยาม "เจ้าของงาน"** — ตอนนี้มีแค่ *คนคีย์* กับ *เบอร์เจ้าหน้าที่* · จอใหม่จึงใช้ "ใครถือสายอยู่" (AI / เราโทรเอง / ยังไม่มีใครถือ) ซึ่งอนุมานจาก `call_mode` + `dispatch_state` แทน
3. **ส่ง LINE** — ระบบไม่มีการต่อ LINE เลย · ตัดออกจากจอ
4. **Automation ติดต่อไม่ได้ 2 รอบ → Replacement** — ยังไม่มี · วันนี้แค่ติดธง "ต้องคนตาม"
5. **ตำแหน่งงานรายคน** — ใบติดตามไม่เก็บ

### แผน Planning ทั้งเดือน (รอบเดียวกัน — เจ้าของสั่งเพิ่มหลังเห็นของจริง)

> *"ปฏิทินเอาเป็นกดแล้วเห็นแบบยาว ๆ ดิ นี่เห็นเป็นไรก็ไม่รู้ แบบเห็นเลยว่า เดือนนี้วันที่
> 1-สิ้นเดือน นาย ก โทรวันไหนบ้าง วันละกี่รอบ รอบไหนกี่โมง คือให้ขึ้นเป็น Planning อะ"*

| ไฟล์ | หน้าที่ |
| --- | --- |
| `src/lib/followMonthPlan.ts` | สร้างตาราง **คน × วันที่ 1-สิ้นเดือน** · ช่องหนึ่งบอก ใครโทร · กี่รอบ · รอบไหนกี่โมง · `daysOfMonth` / `shiftMonth` / `bangkokYmd` |
| `tests/api/followMonthPlan.test.ts` | 13 เทสต์ — เดือน 28/29/30/31 วัน · สายเช้ามืดต้องไม่เลื่อนวัน (UTC) · วันผสมแยกยอด AI/เราโทรตามรอบจริง |
| `src/components/follow/FollowMonthPlanner.tsx` | ตารางเลื่อนแนวนอน คอลัมน์คนตรึงซ้าย · วันนี้ไฮไลต์ · กดช่อง = เปิดแก้รอบของวันนั้น |

* ป๊อป "แผนการโทร" มีสองแท็บ: **แผนทั้งเดือน** (ค่าเริ่มต้น) · **ปฏิทิน & สถิติผลโทร** (ของเดิมครบทุกบรรทัด)
* `api/_handlers/follow.ts` — `toResponse` เพิ่ม **`call_times`** · เก็บในฐานมาตลอดแต่ไม่เคยส่งออกมา ⇒ ก่อนหน้านี้ฝั่งจอไม่มีทางรู้ว่าวันนั้นมีกี่รอบ
* 🔴 **`min-w-0` สามชั้น** (Tabs · TabsContent · ตัวตาราง) — `DialogContent` ของ shadcn เป็น `grid` ลูกมี `min-width:auto` ⇒ ตารางกว้างกว่าจอดันกล่องทั้งใบ แถบสรุปหลุดไปอยู่ที่ x=2076 ทั้งที่จอกว้าง 1600 (วัดบนเครื่องจริง)
* ปี พ.ศ. ต้อง `+543` ที่จุดแสดงผล — ปีในฐาน/ISO เป็น ค.ศ. เสมอ

**ด่านตรวจ:** test 3,240 ผ่าน / 6 skip · tsc = 0 · eslint 0 error / 20 warning (ของเดิมทั้งหมด) ·
ด่านกติกา UI (`designTokens` · `typographyRules` · `perfGuards`) ผ่าน 64 ข้อ
**ตรวจบนจอจริงแล้ว** (localhost · เจ้าของล็อกอินให้): คิวงานขึ้นครบ 10 งานเปิด/39 ปิด ·
ตารางแผนเดือน ก.ย. ขึ้น 49 คน 246 รอบ 8 วันที่มีสาย · เลื่อนถึงวันที่ 30 ได้ · วันนี้ไฮไลต์ ·
แท็บที่สองยังเป็นปฏิทิน+Call Pipeline+สถิติ 84.2% ของเดิมครบ
⚠️ **ยังไม่ได้ commit ตามที่เจ้าของสั่ง**

### ต่อเนื่อง 21 ก.ย. 2569 (เย็น) — สามคำสั่งจากเจ้าของ (ยังไม่ commit)

**1. กดการ์ดกล่องงานแล้วต้องมี "สองช่อง" ให้เลือก**

> *"มีให้เลือก 2 อัน โดย Default ให้โชว์หน้าตรวจสอบไว้ … ส่วนถ้ากดรายชื่อ ก็ขึ้นเป็น
> รายชื่อทั้งหมด พร้อมบอกสถานะว่า กำลังโทร โทรแล้ว … รายชื่อที่สนใจ · รายชื่อที่ไม่สนใจ"*

🔴 **บทเรียน:** รอบแรกผมเอารายชื่อไป **ต่อท้าย** ขั้นตอน เจ้าของตีกลับทันที
(*"ฉันไม่ได้ต้องการให้เอาไปต่อท้าย"*) — ต้องเป็น **แท็บสองช่องที่กดสลับ** ไม่ใช่กองต่อกัน

| ไฟล์ | แก้อะไร |
| --- | --- |
| `src/pages/jobs/BoardPostingPage.tsx` | `<Tabs>` สองช่อง **ตรวจสอบ** (ค่าเริ่มต้น) / **รายชื่อ** ครอบเนื้อทั้งหมด · ช่องตรวจสอบ = ขั้น 1-4 ของเดิมครบ · ช่องรายชื่อ = `JobApplicantsDialog embedded` |
| `src/components/jobs/JobApplicantsDialog.tsx` | เพิ่มโหมด `embedded` — แยก `head` / `content` ออกจากกรอบ Dialog · โหมดฝังใช้ `h3`/`p` แทน `DialogTitle`/`DialogDescription` (Radix เรียกนอก Dialog แล้วพัง) · เปลี่ยนคำแท็บย่อยเป็น **รายชื่อทั้งหมด / รายชื่อที่สนใจ / รายชื่อที่ไม่สนใจ** ตามที่เจ้าของเขียนมา · 🔴 **โหมดฝังไม่มีหัวเรื่อง "ผู้สมัครที่กรอกฟอร์ม" และไม่มีปุ่ม "ส่งให้ AI โทร"** (เจ้าของสั่งเอาออก 21 ก.ย. 2569 — แท็บ "รายชื่อ" บอกอยู่แล้ว) · ทั้งสองอย่างยังอยู่ในป๊อปรายชื่อเต็ม (ปุ่ม "ดูรายชื่อ" บนการ์ด) |
| `api/_lib/applicantCallOutcomes.ts` | **ใหม่:** `loadLatestCallStateByPhone()` — สถานะสายล่าสุดจากคิว (`pending`/`delivered`/…) · ต่างจาก `loadLatestCallOutcomeByPhone` ที่เอาเฉพาะสายที่ **มีผลแล้ว** ⇒ คนที่ยังอยู่ในคิวเลยไม่มีอะไรบอก ซึ่งคือช่องที่เจ้าของทัก |
| `api/_handlers/job-applications.ts` | แนบ `last_call_status` ต่อคน |
| `src/lib/publicApplicationsApi.ts` · `JobApplicantsDialog` | การ์ดผู้สมัครขึ้นสถานะสายด้วยคำ/สีจาก `FOLLOW_STATUS_LABEL`/`TONE` **ชุดเดียวกับหน้าติดตาม** · ไม่มีคิวเลย = "ยังไม่ได้ส่งให้ AI" (เดิมขึ้น "รอโทร" เหมือนกันหมด แยกไม่ออก) |

**2. หน้าติดตาม — ถอดของที่เกะกะ** (*"เอาออกมันเกะกะ"*)

* `FollowCallRoundsPanel` — บรรทัด "AI ได้คำตอบแล้ว N สาย — …" + ย่อหน้าอธิบาย "ตัวเลขชุดนี้คือทุกวัน … ช่องจึงไม่ได้บวกกันเป็นทั้งหมด" (มีสองที่ ถอดทั้งคู่) · คำอธิบายฐานยังอยู่ใน `title` ของแต่ละกล่อง
* `FollowWorkbench` — หัวเรื่องสามบรรทัด (คิวงานติดตาม / วันนี้ต้องตามใครบ้าง / เรียงให้แล้ว…) เหลือแค่แถวปุ่ม
* `FollowPage` — คำโปรยใต้ชื่อหน้า ("ลงรายชื่อคนที่ต้องติดตาม แล้ว AI จะโทรตามให้")

**3. ช่องไฟ/ขนาดตัวอักษรให้ตรงกัน** (*"ความหนาขอไม่เกิน semi"*)

* `FollowMonthPlanner` — ช่องวัน `w-16 → w-20` · เลิกใช้ `text-[9px]` (เล็กกว่าที่ระบบใช้ทั้งหมด) เหลือสเกลเดียวกับที่อื่น: `text-[10px]` / `text-[11px]` / `text-xs` / `text-sm`
* ความหนายังคงเพดานเดิมที่ **`font-medium` (500)** ซึ่งต่ำกว่า semibold อยู่แล้ว — ด่าน `typographyRules` คุมไว้

**ด่านตรวจ:** test 3,240 ผ่าน / 6 skip · tsc = 0 · eslint 0 error · designTokens + typographyRules ผ่าน 56 ข้อ
**ตรวจบนจอจริงแล้ว:** เปิดการ์ด LMM6704005 บน `/jobs/board` → ในป๊อปมี "คนที่กรอกใบสมัครเข้ามา"
พร้อมแท็บ รายชื่อทั้งหมด (1) · คนที่สนใจ (0) · ไม่สนใจ (0) และการ์ดผู้สมัครครบ ·
หน้าติดตามไม่มีบรรทัดที่สั่งให้เอาออกแล้ว
⚠️ **ยังไม่ได้ commit**

### 21 ก.ย. 2569 (บ่าย) — "บันทึกได้ แต่กลับออกมาข้อมูลหาย"

**เจ้าของแจ้งจากผู้ใช้จริง (หัวหน้า sirirat):** *"บันทึกได้แต่กลับออกมาข้อมูลมันหาย หมายเหตุก็เป็น"*

**ต้นเหตุ (วัดจากโค้ด + ฐานจริง):** ของ **ลงฐานครบ** — sirirat บันทึกสำเร็จ 11 ใบระหว่าง
11:17–11:42 ของวันนั้น · ปัญหาอยู่ที่ **ฝั่งอ่าน**

`/api/siamraj/unit-requests` ใช้ `readThroughCache()` โดย loader ทำ
`listSiamrajUnitRequests()` **แล้วค่อย** `attachAssignments` / `attachNotes` / `attachWorkStatus`
⇒ **สำเนาที่แคชไว้มีค่าผู้รับผิดชอบ/หมายเหตุฝังอยู่ด้วย** (อายุ 90 วินาที ·
โหมดตอบของเก่าระหว่างโหลดใหม่ยืดได้ถึง 10 นาที) ⇒ บันทึกเสร็จกลับไปหน้าลิสต์/กล่องงาน
ยังได้สำเนาเดิมที่ไม่มีค่าที่เพิ่งบันทึก = เห็นว่า "ข้อมูลหาย"
🔴 `clearUnitRequestCache()` มีอยู่ในโค้ดตั้งแต่แรก **แต่ไม่เคยมีใครเรียกเลย**

**แก้:** เส้นเขียนทุกเส้นที่แก้ของซึ่งถูกแปะลงลิสต์ ต้องล้างสำเนาหลังบันทึกสำเร็จ

| ไฟล์ | เขียนอะไร |
| --- | --- |
| `api/_handlers/siamraj-unit-assignments.ts` | ผู้รับผิดชอบ (สรรหา/คัดสรร/OPL/online) |
| `api/_handlers/siamraj-unit-notes.ts` | หมายเหตุ + ตัวเลือกใบขอ + field overrides |
| `api/_handlers/siamraj-unit-work-status.ts` | สถานะทำงาน |
| `api/_handlers/unit-sector.ts` | ราชการ/เอกชน |

`tests/api/unitRequestCacheInvalidation.test.ts` — คุมว่าทั้งสี่เส้นเรียก `clearUnitRequestCache()`
...
## 21 ก.ย. 2569 (เย็น) — ถอยหน้าติดตามกลับเป็นก่อนรื้อ

**เจ้าของสั่ง:** *"หน้า การติดตาม เอาแบบเดิมก่อน มัน Production แล้ว แก้ไขได้แค่
ความถูกต้องของข้อมูล UI แก้ไม่ได้แล้ว"* — ถามย้ำแล้วเจ้าของยืนยันว่าหมายถึง
**ถอย UI กลับเป็นก่อนรื้อ** ไม่ใช่แค่หยุดแก้ต่อ

🔴 **กติกาใหม่ของหน้าติดตาม:** หน้านี้ขึ้น production แล้ว **แก้ได้แค่ความถูกต้องของข้อมูล**
· ห้ามแก้หน้าตาอีก (ต้องการเปลี่ยน UI ต้องให้เจ้าของสั่งเป็นรายครั้ง)

### ถอดออก (ของที่เพิ่งทำเช้าวันเดียวกันใน `fd303dc`)

| ไฟล์ | ทำอะไร |
| --- | --- |
| `src/pages/follow/FollowPage.tsx` | คืนเป็นเวอร์ชัน `fd303dc^` — ปฏิทิน · แผงรอบโทร 1/2/3 · 7 กล่องสถานะ · สรุปผลรายเดือน กลับมาอยู่**หน้าหลัก** ไม่มีป๊อป `plannerOpen` แล้ว |
| `src/components/follow/FollowWorkbench.tsx` · `.test.tsx` | ลบทั้งไฟล์ (จอคิวงาน) |
| `src/components/follow/FollowMonthPlanner.tsx` | ลบทั้งไฟล์ (ตาราง Planning ทั้งเดือน) |
| `src/lib/followWorkbench.ts` · `tests/api/followWorkbench.test.ts` | ลบทั้งคู่ |
| `src/lib/followMonthPlan.ts` · `tests/api/followMonthPlan.test.ts` | ลบทั้งคู่ |

### เก็บไว้ ไม่ถอย (ไม่ใช่การรื้อ UI รอบนั้น)

* `FollowCallRoundsPanel.tsx` — บรรทัด "AI ได้คำตอบแล้ว…" + ย่อหน้า "ตัวเลขชุดนี้คือทุกวัน…"
  **ยังถอดอยู่** เพราะเจ้าของสั่งเอาออกเองแยกต่างหาก (*"เอาออกมันเกะกะ"*)
* `api/_handlers/follow.ts` → `call_times` ใน `toResponse` · `FollowEntry.call_times`
  ในtype — เป็น**ข้อมูล** ไม่ใช่ UI · ยังไม่มีจอไหนใช้แล้ว แต่ไม่เสียหายและเป็นของที่ฐานมีจริง
* งานฝั่ง**กล่องงาน**ที่อยู่ใน commit เดียวกัน (`fd303dc`) **ไม่ถูกแตะเลย** —
  แท็บ ตรวจสอบ/รายชื่อ ใน `BoardPostingPage` · `JobApplicantsDialog embedded` ·
  `loadLatestCallStateByPhone` · `last_call_status`

**ด่านตรวจ:** test 3,212 ผ่าน / 6 skip (278 ไฟล์) · tsc = 0 · eslint 0 error / 20 warning
⚠️ **ยังไม่ได้ดูบนจอ** — server รันใหม่แล้วเด้ง login · ต้องให้เจ้าของล็อกอินให้ก่อน

---

## 21 ก.ย. 2569 (ค่ำ) — สายติดตามค้าง 46 แถว: ต้นเหตุอยู่ฝั่งเรา ไม่ใช่ Lumos

**เจ้าของถาม:** *"ชื่อไปหา Lumos หมดไหม"* → *"แก้ที่ใคร"* → *"ทำให้มันตรง"*

### สิ่งที่วัดได้ (ฐาน production · SELECT ล้วน)

| | |
| --- | --- |
| ใบผลที่ Lumos ยิงเข้ามา 14-21 ก.ย. | **248 ใบ · จับคู่ได้ 248 · จับคู่ไม่ได้ 0** |
| คิว follow ที่ `pending` ตอนตรวจ | **71 แถว** |
| ในนั้นติดธง `retry_scheduled` | **49** |
| ในนั้นเลย `next_attempt_at` ไปแล้ว | **46** |
| `follow_entries` 14 วันล่าสุดที่ `group_id is null` | **287 จาก 295** |

🔴 **Lumos ส่งผลกลับครบทุกใบ** — กล่อง `lumos_result_inbox` (migration 119) เป็นตัวตัดสิน
เคสตัวอย่าง `follow-550b8036…` : 04:55 รอบแรก `failed/unresponsive` · 05:53 รอบสอง
`completed/confirmed` (มี transcript + ไฟล์เสียง) ทั้งคู่ `matched = true`

### ลูกโซ่ของบั๊ก (3 ทอด)

1. `src/pages/follow/FollowPage.tsx` — เส้น **ตั้งหลายรอบในคำขอเดียว** (`createFollowRounds`)
   **ไม่เคยส่ง `group_id`** · อีกเส้นในไฟล์เดียวกัน (ตั้งหลายวัน) ส่งอยู่แล้ว
   ⇒ แถวที่คนใช้สร้างจริงทุกวันได้ `group_id = null` ทั้งหมด
2. `api/_lib/callFollowup.ts` — ใช้ `group_id` ตัวเดียวตัดสินว่า "แถวนี้เป็นสายตั้งตาราง
   ห้าม retry นอกตาราง" ⇒ null = มองเป็นสายเดี่ยว ⇒ ผล `no_answer`/`unresponsive`/`busy`
   เข้าทาง retry ⇒ **ดีดแถวกลับเป็น `pending` + ล้าง `result` + ตั้งนัดใหม่ +24 ชม.**
3. ไม่มีใครดันแผนใหม่ไป Lumos หลัง retry ⇒ แถวค้าง `pending` ถาวร
   = **สายที่ระบบสัญญาว่าจะโทรซ้ำแต่ไม่มีวันโทร** · บนจอขึ้นว่า "ยังไม่รู้ผล"

### ที่แก้

| ไฟล์ | แก้อะไร |
| --- | --- |
| `src/pages/follow/FollowPage.tsx` | เส้นหลายรอบสร้าง `groupId` แล้วส่งไปกับคำขอ — ทุกรอบของคนเดียวกันอยู่กลุ่มเดียวกัน |
| `api/_lib/callFollowup.ts` | `followGroupIdForPersonRef()` → **`followScheduleOfPersonRef()`** คืน `{ scheduled, groupId }` · นับเป็นตารางเมื่อมี `group_id` **หรือ** `call_round >= 1` **หรือ** `call_times` ไม่ว่าง · ฐานที่ยังไม่รัน 092/113 ถอยไปอ่าน `group_id` อย่างเดียวเหมือนเดิม |

🔴 **`groupId` ยังต้องคืนออกมา** — ใช้ตอน `declined` เพื่อยกเลิกทั้งชุด (`cancelFollowGroup`)
แถวที่ไม่มีกลุ่มยกเลิกทั้งชุดไม่ได้ แต่ยังรู้ว่าเป็นตาราง จึงไม่ถูก retry นอกตาราง

### เทสต์ใหม่ (`tests/api/callFollowup.test.ts` · 20 → 24 ข้อ)

* ไม่มี `group_id` แต่มี `call_round` → **ห้าม** ดีดกลับเป็น `pending`
* ไม่มีทั้งคู่ แต่มี `call_times` → ยังนับเป็นตาราง
* สายเดี่ยวจริง (`person_ref` ไม่ใช่ `follow-`) → ยัง retry ตามเดิม
* ฐานยังไม่รัน 113 → ถอยไปดู `group_id` อย่างเดียว ไม่พัง

**พิสูจน์ว่าเทสต์จับของจริง:** ย้อนโค้ดกลับเป็น `followGroupId !== null` แล้วรัน — ล้ม 2 ข้อทันที

**ด่านตรวจ:** test 3,216 ผ่าน / 6 skip · tsc = 0 · eslint 0 error

⚠️ **46 แถวที่ค้างอยู่ยังไม่ได้แตะ** — เจ้าของสั่ง *"ยังไม่แตะ รอดูก่อน"* (การล้างต้องเขียน
ฐาน production) · ตัวแก้นี้กันของใหม่ ไม่ได้ย้อนไปซ่อมของเก่า

---

## 22 ก.ย. 2569 — ชื่อ/เบอร์ของลูกค้าหลุดขึ้นหน้าสมัครสาธารณะ

**เจ้าของเห็นเองบน `/apply` แล้วส่งภาพมา:** *"เบอร์ติดต่อมันดันขึ้นคุณเบญจมาศ อะ แก้ให้
เบอร์จากใบขอไม่มาขึ้นที่กล่องงานเลย"*

### ต้นเหตุ

`location_address` ที่ ERP ส่งมา **ไม่ใช่ที่อยู่ล้วน** — หน่วยงานพิมพ์รวมมาก้อนเดียว
(ชื่อสาขา · รหัสสาขา · ทะเบียนรถ · ชื่อผู้จัดการ · เบอร์ · ชื่อผู้ช่วย · เบอร์ · อีเมล)
ฟอร์มสร้างลิงก์เติมค่านั้นลงช่อง "สถานที่" **ทั้งดุ้น** ⇒ ขึ้นหน้าสาธารณะให้คนนอกเห็น
\+ ชนเพดาน 200 ตัวอักษรจึงขาดกลางคัน (*"…เบอร์ติดต่"*)

**วัดจริง:** 8 จาก 21 ประกาศมีชื่อ/เบอร์คนฝังอยู่ (2 ข้อความ · 3 ใบขอ: OPL6909027 ·
OPL6902010 · OPL6902145)

### ที่แก้

| ไฟล์ | ทำอะไร |
| --- | --- |
| `src/lib/publicLocationText.ts` | **ใหม่** — `scrubPublicLocation()` ตัดตั้งแต่คำที่เป็นหัวข้อมูลติดต่อตัวแรกที่เจอ (`ทะเบียนรถ` · `เบอร์ติดต่อ` · `ผู้จัดการสาขา` · `ผู้ช่วย` · `ผู้ติดต่อ` …) หรือเบอร์โทรไทยตัวแรก · `hasContactInfo()` บอกว่ามีของต้องตัดไหม |
| `tests/api/publicLocationText.test.ts` | 9 เทสต์ — เคสจริงทั้งสองข้อความ · เลขเอกสาร `WF 000127/2026` และรหัสสาขา `629` ห้ามโดนตัด · ที่อยู่ล้วนห้ามถูกแตะ |
| `src/components/jobs/GenApplyLinkDialog.tsx` | เติมค่าตั้งต้นช่อง "สถานที่" ผ่านตัวตัดก่อน |
| `src/pages/jobs/BoardPostingPage.tsx` | สรุปขั้น 1 เพิ่ม **ชื่อผู้ติดต่อหน่วยงาน** + **เบอร์ติดต่อ** (4 ช่อง → 6 ช่อง) — เดิมมีเฉพาะตอนกด "กางดูข้อมูลใบขอทั้งใบ" ซึ่งคนทำประกาศไม่เคยกด |
| `scripts/scrub-posting-locations.mts` | **ใหม่** — ล้างของเก่า · ค่าเริ่มต้นดูอย่างเดียว ต้อง `--apply` ถึงเขียน · สำรองไฟล์ก่อนเสมอ · update ทีละ id ไม่ใช้ LIKE |

🔴 **ตัดตอนเติมค่าตั้งต้นเท่านั้น ห้ามตัดตอนบันทึก** (เจ้าของเคาะ) — คนกรอกพิมพ์เบอร์
กลับเข้าไปเองได้ถ้าตั้งใจ · ระบบแค่ไม่ยัดของที่ไม่มีใครขอขึ้นหน้าสาธารณะให้อัตโนมัติ

⚠️ **ช่อง "สถานที่" ในกล่องงานยังโชว์ข้อความเต็มจาก ERP** — ตั้งใจ เพราะเป็นจอภายใน
ที่เจ้าหน้าที่ต้องเห็นครบ · ตัวตัดใช้เฉพาะขาที่ออกหน้าสาธารณะ

### ล้างของเก่า (เขียน production · เจ้าของอนุญาตหลังดู dry-run)

รัน `--apply` แล้ว **9 แถว** · สำรองไว้ที่ `scrub-locations-backup-1790041698723.json`
ตรวจหลังล้าง: เบอร์เหลือ **0** · ชื่อคน/ทะเบียนรถเหลือ **0** จาก 22 ประกาศ

**ด่านตรวจ:** test 3,225 ผ่าน / 6 skip · tsc = 0 · eslint 0 error
**ดูบนจอจริงแล้ว:** `/apply/p/mfqqujh` เหลือ *"จังหวัด: ชลบุรี ชื่อสาขา: … รหัสสาขา: 629"*
ไม่มีชื่อ/เบอร์ใคร · กล่องงานขั้น 1 ขึ้น "ชื่อผู้ติดต่อหน่วยงาน" กับ "เบอร์ติดต่อ" ครบ

---

## 22 ก.ย. 2569 — "ใบขอบันทึกแล้วหาย" รอบที่สาม: เจอสามช่องที่เหลือ

**เจ้าของแจ้ง (โมโห):** *"ใบขอเวลากรอก หรือ แก้ไขต่างๆมันไม่บันทึกอะ บันทึกแล้วชอบหาย
เห่ยทำดีๆดิ"*

⚠️ เรื่องนี้แก้มาแล้วสองรอบ (`45218f2` สำเนาลิสต์ · `f3051af` ใบขอชั่วคราว) แต่ยังเกิด
เพราะยังมีอีกสามช่องที่ไม่มีใครดู — **ทั้งสามพิสูจน์จากฐาน production แล้ว ไม่ใช่การเดา**

### ① คีย์ที่ใช้เขียน กับ คีย์ที่ใช้อ่านกลับ **เรียงสลับกัน**

| ฝั่ง | ไฟล์ | ลำดับเดิม |
| --- | --- | --- |
| เขียน | `src/lib/siamrajUnitRequestsApi.ts` → `unitRequestNoteKey()` | `externalId` → `request_no` → `id` |
| อ่านกลับ | `api/_handlers/siamraj-unit-requests.ts` → `attachNotes` / `attachAssignments` / `attachWorkStatus` | `request_no` → `externalId` → `id` |

ปกติไม่มีใครเห็นปัญหาเพราะสองค่านี้เท่ากัน **แต่ ERP บางแถวเก็บ `request_no` เป็นเลขล้วน**
(`6907001`) แล้วเราเติม prefix ให้ตอนแสดงผล (`SQ6907001` — ดู `normalizeSiamrajRequestNoForDisplay`)
⇒ `externalId = 6907001` · `request_no = SQ6907001` ⇒ **เขียนคีย์หนึ่ง อ่านอีกคีย์หนึ่ง**

🔴 **หลักฐานในฐาน production:** ใบเดียวกันมีทั้งสองคีย์อยู่จริง
`siamraj_unit_notes` มี `6907001` คู่กับ `SQ6907001`/`OPL6907001`/`PEO6907001` ·
`siamraj_unit_assignments` มี `6907002` และ `6907001` คู่กับรูปมี prefix

**แก้:** ตัวเขียนใช้ `request_no` ก่อน (คีย์เดียว) · ตัวอ่านกลับเพิ่ม `readKeysOf()` +
`pickByKeys()` ลองทุกรูปคีย์ ⇒ **ของเก่าที่เขียนไว้ด้วยคีย์เลขล้วนกลับมาเห็นบนจอ**

### ② เส้นอ่านใบรายใบ (`?id=`) ไม่เคยห้ามเบราว์เซอร์แคช

เส้นเดียวของไฟล์ที่ไม่ตั้ง `Cache-Control` (เส้น `?meta=1` · `?units=1` ตั้งไว้ตั้งแต่แรก)
และฝั่งเรียก `fetchSiamrajUnitRequest()` ก็เป็นตัวเดียวในไฟล์ที่ไม่ส่ง `cache: 'no-store'`
⇒ บันทึกเสร็จ เปิดใบเดิมซ้ำ เบราว์เซอร์คืนคำตอบเก่าของตัวเอง = ค่าที่เพิ่งบันทึกไม่โผล่

**แก้:** ตั้ง `no-store` ทั้งสองฝั่ง + เทสต์กวาดว่า**ทุกเส้น**อ่านใบขอฝั่ง client มีครบ

### ③ ตัวแปะค่ากลืน error เงียบ

`attachAssignments` / `attachNotes` / `attachWorkStatus` เดิม `catch {}` เปล่า
⇒ PostgreSQL อ่านไม่ได้เมื่อไหร่ ค่าที่คนบันทึกไว้หายจากจอ **โดยไม่มีร่องรอยใน log เลย**
แยกไม่ออกจาก "ไม่ได้บันทึก" ⇒ ใส่ `logWarn` ทั้งสามตัว (ยังไม่ทำให้ feed ล่มเหมือนเดิม)

### ทดสอบวงจรจริง (ไม่ใช่แค่เทสต์)

เขียนหมายเหตุลงฐานด้วยคีย์เลขล้วน แล้วอ่านกลับ:
`อ่านด้วย prefix = ไม่เจอ` · `อ่านด้วยเลขล้วน = เจอ` · `ทางใหม่ลองทุกคีย์ = เจอ`
แล้วลบแถวทดสอบด้วย id ทันที (เหลือ 0)

### เทสต์ใหม่

`tests/api/unitRequestSaveRoundTrip.test.ts` — 6 ข้อคุมทั้งสามช่อง รวมถึงห้ามย้อนกลับไป
ลำดับคีย์เดิม · ห้ามมี `catch {}` เปล่า · ทุกเส้นอ่านใบขอต้องมี `no-store`

**ด่านตรวจ:** test 3,231 ผ่าน / 6 skip · tsc = 0 · eslint 0 error

---

## 22 ก.ย. 2569 — กล่องงาน: ปิดข้อ 3 · ข้อ 5 · ข้อ 1

ต่อจากผลตรวจหน้า `/jobs/board` 7 เรื่อง (16 ก.ย. 2569) ที่เจ้าของเคาะไว้สองข้อ

### ข้อ 3 — เลขบนหัวจอกับเลขบนปุ่มพูดคนละชุด

**เดิม:** หัวเขียน *"ยังไม่ปล่อย 195 ใบ"* · ปุ่มเขียน *"ส่งประกาศทีเดียว 175 ใบ"*
แล้วต่อท้ายด้วยคำแก้ตัวตัวเล็ก *"ไม่รวม 20 ใบที่มีคนเริ่มงานแล้ว"*
⇒ เลขสองที่บนจอเดียวกันไม่ตรง คนอ่านไม่เชื่อทั้งคู่

**เจ้าของเคาะ:** *"แยกยอดเป็นสองก้อน … เลขทุกที่บวกกันได้ ไม่ต้องมีคำแก้ตัว"*

| ไฟล์ | แก้อะไร |
| --- | --- |
| `src/lib/boardRelease.ts` | `ReleaseLaneKey` เพิ่ม `sourcing` / `started` · `RELEASE_LANE_TEXT` เพิ่มสองก้อน · `filterByReleaseLane` รองรับ (ใช้ `stillSourcing` ตัวเดียวกับปุ่ม) · ledger เพิ่ม `startedAlready` |
| `src/components/jobs/BoardReleaseHeader.tsx` | `SubLaneTile` — ก้อนย่อยใต้ "ยังไม่ปล่อย" กดกรองได้ทั้งคู่ + บรรทัด "บวกกันได้ N ใบพอดี" |
| `src/components/jobs/JobBoardView.tsx` | ถอดคำแก้ตัวใต้ปุ่ม · รับ `?lane=sourcing/started` จาก URL |

🔴 **ไม่ใช่ก้อนที่สี่ที่เจ้าของสั่งยุบ 28 ส.ค.** — ตัวนั้นชื่อ "ไม่ต้องปล่อย" อยู่ระดับ
เดียวกับสามก้อนหลัก · ตัวนี้เป็นก้อน**ย่อยใต้** "ยังไม่ปล่อย" สามก้อนหลักคงเดิมทุกอย่าง

### ข้อ 5 — เงินไม่บอกหน่วย

`฿400` (ค่าแรงรายวัน) กับ `฿12,000` (รายเดือน) อยู่รูปแบบเดียวกันเป๊ะ
⇒ กวาดตาผ่าน ๆ อ่านเป็น "เงินเดือน 400" · ของจริงมี **20 จาก 200 ใบ** ที่เป็นรายวัน

🔴 **ต้นเหตุ: การ์ดปั้นสูตรเอง** ทั้งที่ระบบมีตัวกลาง `src/lib/incomeLabel.ts` อยู่แล้ว
(หน้าจับคู่ใช้มาตั้งแต่ 26 ส.ค.) — เปลี่ยนมาใช้ `incomeDisplay()` ตัวเดียวกัน
⇒ ไม่รู้หน่วย = ขึ้น `400 บาท` เฉย ๆ (ไม่เดาว่าเป็นต่อเดือน) + คำเตือนใน tooltip
⚠️ คำเตือนพูดถึง ERP = ภาษาภายใน ⇒ กั้นด้วย `isStaff` (การ์ดใบนี้โผล่บน `/apply` ด้วย)

### ข้อ 1 — กล่องลอยไม่มีอะไรคั่น

เจ้าของยืนยัน **คงไว้ท้ายลิสต์ตามที่สั่งไว้ 13 ส.ค.** (ย้ายขึ้นบนแล้วดันใบขอจริงตกจอ)
แต่ให้คั่นชัดขึ้น — ป้ายเดิมเป็นบรรทัดจาง 11px บรรทัดเดียว
⇒ เพิ่ม `<Separator>` เต็มความกว้าง + หัวเรื่อง `text-sm` + บรรทัดบอกว่า
"ตัวเลขข้างบนทั้งหมดไม่นับส่วนนี้" · **ไม่ย้ายตำแหน่ง ไม่แตะการ์ดข้างใน**

### เทสต์ใหม่

| ไฟล์ | คุมอะไร |
| --- | --- |
| `tests/api/boardRelease.test.ts` (+4 ข้อ) | สองก้อนย่อยบวกกันได้ `unreleased` เป๊ะ · ก้อน "ยังต้องหาคน" ได้ใบชุดเดียวกับที่ปุ่มจะส่ง · รวมสองก้อนแล้วไม่ซ้ำไม่ขาด |
| `tests/api/boardCardIncomeUnit.test.ts` (ใหม่ 6 ข้อ) | การ์ดต้องใช้ `incomeDisplay()` ห้ามปั้นสูตรเอง · สูตรเดิมห้ามกลับมา · คำเตือนต้องกั้น `isStaff` |

**ด่านตรวจ:** test 3,241 ผ่าน / 6 skip · tsc = 0 · eslint 0 error
**ดูบนจอจริงแล้ว** (หน้า `/apply` ซึ่งใช้การ์ดตัวเดียวกัน): เงินขึ้น `16,304 บาท/เดือน`
คู่กับ `102 บาท` แยกหน่วยออกจากกันชัด · ไม่มีคำว่า ERP หลุดไปหน้าสาธารณะ
⚠️ **หัวจอฝั่งเจ้าหน้าที่ยังไม่ได้ดูบนจอ** — เด้ง login

### ยังไม่ได้ทำ (รอเจ้าของเคาะ)

* **ข้อ 4** การ์ดยาว ~20 บรรทัด (ที่อยู่เต็ม · สวัสดิการทุกข้อ · คลิกแยกช่องทางละบรรทัด)
* **ข้อ 6** ปุ่มบนการ์ดไม่ตรงกับงานของหน้า (มี "หาคนทุกกอง + ให้ AI โทร" / "ดูรายชื่อ" แต่ไม่มีปุ่มส่งประกาศ)
* **ข้อ 7** เรียงค้างนานสุดขึ้นก่อน ⇒ ใบ 873/866/739 วันยึดหัวบอร์ด ใบใหม่จมท้าย

---

## 22 ก.ย. 2569 — กล่องงาน Phase D1: ก้อน "มีคนสมัคร/ยังเงียบ" โชว์ตลอด

เจ้าของให้นิยามหน้ากล่องงานใหม่ (7 ข้อ) · ข้อ 7 = visual ทั้งหมด/ปล่อย/มีคนสมัคร/เงียบ
ต้องเห็นตั้งแต่เปิดหน้า · เลขคู่นี้มีอยู่แล้วใน ledger แต่เดิมโชว์เฉพาะตอนกดเลน "ปล่อยแล้ว"

| ไฟล์ | แก้อะไร |
| --- | --- |
| `src/lib/boardRelease.ts` | `ReleaseLaneKey` เพิ่ม `applied`/`silent` · `RELEASED_SPLIT_KEYS` · `RELEASE_LANE_TEXT` สองก้อน · `filterByReleaseLane` แบ่งด้วย `facts.applicants(j) > 0` |
| `src/components/jobs/BoardReleaseHeader.tsx` | แผงเลน released เดิม (Chip โชว์เฉพาะตอนกด) → `SubLaneTile` โชว์ตลอด กดกรองได้ + บรรทัด "บวกกันได้ N ใบพอดี" · `SubLaneTile` รับ prop `sub` (หัวคนรวม) |
| `src/components/jobs/JobBoardView.tsx` | รับ `?lane=applied/silent` จาก URL |
...
## 23 ก.ย. 2569 — หน้าติดตาม: ตารางหลายวัน + เบอร์เจ้าหน้าที่ย้ายมาหน้าผู้ใช้งาน

เจ้าของทัก 3 เรื่องต่อกัน: *"ลงหลายวันแล้วรวน"* · *"เบอร์ที่จะเพิ่มติดไว้เพิ่มตรงไหน"*
(→ เคาะว่า **หน้าผู้ใช้งานคือตัวจริง**) · *"ต่อไปหน้าใบขอจะเอาชื่อจากไหนมาให้เขาเลือก"*
(→ เคาะว่า **ย้ายมาหน้าผู้ใช้งานที่เดียว ทำเป็นขั้น**)

| ไฟล์ | แก้อะไร |
| --- | --- |
| `src/lib/followWizard.ts` | เพิ่ม `scheduleDayCallRound()` (สายที่เท่าไหร่ นับต่อข้ามวัน) + `scheduleDayStaffPhone()` (เบอร์ชุดเดียว/รายวัน) — สองกติกาของโหมดตารางหลายวัน |
| `src/pages/follow/FollowPage.tsx` | โหมดตารางใช้เบอร์ชุดเดียวเป็นค่าตั้งต้น (สวิตช์ "ระบุเจ้าของแผนรายวัน" ยังอยู่) · ส่ง `call_round` ที่เคยไม่ส่ง · สลับเข้าโหมดตารางแล้วเติมวันตั้งต้นให้ · เมนูตั้งค่าตัวเลือกพาไปหน้าผู้ใช้งาน |
| `api/_handlers/job-staff.ts` | **ใหม่ใน handler เดิม** — `directory` = ชื่อเล่น + สายงาน + เบอร์ จาก `users` (110 + 114) · 🔴 **ชื่อในลิสต์ = ช่องชื่อเล่นเท่านั้น** ไม่มีชื่อเล่น = ไม่โผล่ (ห้ามตกไปใช้ `full_name` ซึ่งเป็นชื่ออังกฤษจาก Microsoft) · ยังไม่มีเบอร์ก็ส่ง (ใบขอใช้แค่ชื่อ) · ผูก BU ชุดเดียวกับ roster |
| `src/lib/jobStaffRemote.ts` | parse `directory` เข้าแคชเดิม (เส้นเดียว ไม่ยิงเพิ่ม) |
| `src/lib/followStaffMemory.ts` | `phoneForStaffName()` — **สมุดเบอร์ชนะความจำเดิมเสมอ** · `followStaffDirectory()` กรองเหลือ สรรหา/คัดสรร ที่มีเบอร์ (กรองที่ปลายทาง ไม่แตกเส้น API) |
| `src/components/follow/StaffContactField.tsx` | เลือกชื่อ → เบอร์จากหน้าผู้ใช้งานขึ้นเอง · ชื่อที่ตั้งค่าแล้วห้ามจำทับด้วยค่าที่พิมพ์ |
| `src/lib/jobStaffNames.ts` | 🔴 dropdown ผู้รับผิดชอบบนใบขอ **รวมชื่อสองที่** (หน้าทีม + หน้าผู้ใช้งาน) — ขั้นที่ 1 ของการย้าย ของเดิมไม่หาย |
| `src/pages/settings/AdminSettings.tsx` | เตือนในแถวเมื่อ "ตั้งสายงานแล้วแต่ยังไม่มีชื่อเล่น" (= ไม่โผล่ที่ไหนเลย) · ช่องชื่อเล่น/เบอร์ใส่ `min-w-` (ของเดิม `w-40` โดนตารางบีบเหลือ 38px) · ย้ายคอลัมน์เบอร์มาติดชื่อเล่น/สายงาน |

**เทสต์:** `src/lib/followWizard.test.ts` (+8) · `tests/api/followStaffMemory.test.ts` (+9) ·
`tests/api/jobStaffNameOptions.test.ts` (**ใหม่** · 7 ข้อ — แกนคือ "หน้าผู้ใช้งานว่าง = ชื่อเท่าเดิมทุกประการ")
**ด่านตรวจ:** test 3,298 ผ่าน / 6 skip · tsc = 0 · eslint 0 error
**พิสูจน์กติกาชื่อเล่น (SQL ในทรานแซกชันที่ rollback ทิ้ง):** ติ๊กสายงานแต่ไม่กรอกชื่อเล่น ⇒ 0 แถว ·
กรอกชื่อเล่น ⇒ โผล่ทันที · บนจอจริงมีคำเตือนขึ้นครบ 3 คนที่ตั้งสายงานไว้แต่ยังไม่มีชื่อเล่น
**วัดบนจอจริง:** ตาราง 31 วัน 5,135px → 1,045px (ช่องเจ้าหน้าที่ 31 ชุด → 1) ·
ช่องเบอร์หน้าผู้ใช้งาน 38px → 180px · เลือก "ครีม" ได้เบอร์จากหน้าผู้ใช้งาน ไม่ใช่ 1 ใน 3 เบอร์เก่า

### ขั้นต่อไปของการย้าย (รอเจ้าของสั่ง)
กรอกชื่อเล่น + สายงานในหน้าผู้ใช้งานให้ครบ 41 ชื่อ (วัด 23 ก.ย.: ชื่อเล่น 2 · สายงาน 3 · เบอร์ 0)
แล้วค่อยปิดหน้า "ทีมสรรหา / คัดสรร / OPL / Online" · 🔴 จับคู่สองที่ด้วย**ชื่อ**เท่านั้น ไม่มี id ผูกกัน

---

## 23 ก.ย. 2569 — ใบขอล่วงหน้าที่เลขซ้ำกับใบขอจริง: แยกคีย์

เจ้าของแจ้ง: *"ใบขอที่เลขเหมือนกันทำไมแก้แล้วมันดันซ้ำกัน แก้ใบไหนก็ควรขึ้นแค่ของใบนั้นสิ"*

**ต้นเหตุ:** ใบขอจริง (`siamraj-sql:`) กับใบขอล่วงหน้า (`siamraj-pre:`) ใช้รูปเลขที่ใบเดียวกัน
(BU+ปีเดือน+running) แล้วตารางฝั่งเราคีย์ด้วย **เลขที่ใบเปล่า** ⇒ สองใบอ่าน/เขียนแถวเดียวกัน
🔴 **วัดจาก ERP จริง: ใบล่วงหน้า 42 ใบ ชนเลขกับใบขอจริง 27 ใบ (64%) ไม่ใช่เคสหายาก**
(เคสที่เจ้าของเจอ `OPL6909001` — ใบจริง "ลาออก/ยาสกาว่า" กับใบล่วงหน้า "เปิดไซด์/คุณอังคาร")

**ขอบเขตจริงแค่ 4 ตาราง** — อีก 6 ตารางที่มี `request_no` (ผลแมท · ใบสมัครที่เสนอ · คิวโทร ·
ประกาศ · ปล่อยหน้าสาธารณะ · กันชนสาย) **คีย์ด้วย `job_id` ที่พก prefix เต็มอยู่แล้ว** จึงไม่มีปัญหา
(วัดแล้ว: `board_match_results` มี job_id แบบ `siamraj-pre:` อยู่ 40 แถว)

| ไฟล์ | แก้อะไร |
| --- | --- |
| `src/lib/siamrajUnitRequestsApi.ts` | `unitRequestNoteKey()` — ใบล่วงหน้าคืน **id เต็ม** · ใบขอจริงยังคืนเลขเปล่าเหมือนเดิม (ของเก่า 1,990+ แถวเขียนด้วยเลขเปล่า เปลี่ยนคีย์ใบจริง = ของหายทั้งกอง) · เพิ่ม `isPrequestKey()` |
| `src/pages/jobs/SiamrajUnitRequestDetailPage.tsx` | เลิกคิดคีย์เอง (`externalId \|\| request_no` ซึ่ง**สลับลำดับ**กับที่อื่น = บั๊ก "บันทึกแล้วหาย" 22 ก.ย. ที่ยังค้างจุดนี้) ใช้ `unitRequestNoteKey()` |
| `api/_handlers/siamraj-unit-requests.ts` | `readKeysOf()` — ใบล่วงหน้าหาด้วยคีย์เดียว **ห้ามถอยไปเลขเปล่า** ถอยเมื่อไหร่ไปหยิบของใบจริงมาโชว์ |
| `src/components/jobs/RequestLeadRulesCard.tsx` · `EditPublicJobFieldsDialog.tsx` | (รอบตาม 23 ก.ย.) เลิกคิดคีย์เอง — สองไฟล์นี้ใช้ `siamrajExternalId(job) \|\| job.request_no` ซึ่ง **ถอด prefix ทิ้ง** ⇒ ใบล่วงหน้าเขียนทับแถวของใบขอจริง (เกณฑ์ความเร่ง · ข้อมูลหน้าสาธารณะ) |
| `src/lib/unitRequestFeedNotifications.ts` | คีย์เทียบ feed ใช้ `id` ก่อน — เดิมใช้เลขที่ใบ ⇒ ใบล่วงหน้ากับใบจริงยุบเป็นคีย์เดียว แจ้งเตือน "ใบใหม่/ปิดแล้ว" ของใบที่ถูกทับหายเงียบ |

**ย้ายข้อมูลครั้งเดียว (สคริปต์ · ลบทิ้งแล้ว):**
* เลขที่เป็นของใบล่วงหน้าล้วน 15 เลข → **ย้าย** คีย์เป็น `siamraj-pre:<เลข>` รวม **20 แถว**
  (assignments 7 · notes 2 · work_status 5 · history 6)
* เลขที่ชนกับใบจริง 27 เลข → **คัดลอก**ให้ใบล่วงหน้าถือชุดของตัวเอง รวม **63 แถว**
  (assignments 24 · notes 22 · work_status 17) — ของจริงอยู่แถวเดิม
  🔴 คัดลอกไม่ใช่ย้าย เพราะแถวเลขเปล่าเป็นของใบจริง · และถ้าไม่ทำอะไรเลย จอใบล่วงหน้าจะว่าง
  ทันทีที่ deploy ทั้งที่เมื่อวานยังเห็นอยู่ ⇒ คัดลอก = จอไม่เปลี่ยนวันนี้ แต่แยกกันตั้งแต่พรุ่งนี้
  ⚠️ **ไม่คัดลอก `..._history`** — เป็นประวัติการแก้ คัดลอกแล้วมีสองชุดเล่าเรื่องเดียวกัน

**เทสต์:** `tests/api/unitRequestPrequestKey.test.ts` (**ใหม่** · 6 ข้อ) ·
`RequestLeadRulesCard.test.tsx` (+1 เคสใบล่วงหน้า) · `unitRequestFeedNotifications.test.ts` (+2 เคสเลขชนกัน)

**ไล่ครบทุกหน้าแล้ว (23 ก.ย.):** ทุกจุดที่เขียนของพวกนี้ผ่าน `unitRequestNoteKey()` ตัวเดียว —
หน้ารายละเอียดใบขอ · กล่องงาน/บอร์ด · หน้าจับคู่ · PreCheck · ช่องหมายเหตุ · สวิตช์ส่งคนแทน ·
กล่องเกณฑ์ความเร่ง · ป๊อปแก้ข้อมูลหน้าสาธารณะ · สถานะทำงาน
🔴 **ห้ามคิดคีย์เองในไฟล์จอ** — `siamrajExternalId()` มีไว้คุยกับ ERP เท่านั้น (ถอด prefix ทิ้ง)
**พิสูจน์บนของจริง:** แก้ผู้รับผิดชอบของ `siamraj-pre:OPL6909001` เป็น คิว/ครีม →
`siamraj-sql:OPL6909001` ยังเป็น แบงค์/วิว ไม่ขยับ → คืนค่าเดิมแล้ว

### ✅ ปิดประเด็นแล้ว — ของไม่ต้องตามไปตอนแปลงเป็นใบขอจริง
ใบล่วงหน้าที่ **ถูกแปลงเป็นใบขอจริงภายหลัง** — ของที่กรอกไว้ตอนเป็นใบล่วงหน้าอยู่คีย์
`siamraj-pre:<เลข>` ส่วนใบจริงอ่านเลขเปล่า ⇒ **ของไม่ตามไป และไม่ต้องตามไป**
(เจ้าของเคาะเอง 23 ก.ย. 2569: *"ไม่ต้องตามไปก็ได้"*)

🔴 **ห้ามใส่ทางถอยให้ใบจริงไปอ่านคีย์ `siamraj-pre:` ของเลขเดียวกัน** — ดูเหมือนช่วย
แต่คือเปิดทางให้สองใบปนกันอีก ซึ่งเป็นบั๊กที่เพิ่งแก้ในหัวข้อนี้ · ใบจริงเริ่มใหม่ว่าง ๆ คือ
พฤติกรรมที่ต้องการ

---

## 23 ก.ย. 2569 (ตอนเย็น) — ตารางหลายวันหน้าติดตาม: เลือกง่ายขึ้น

เจ้าของสั่ง: *"ทำให้มันเลือกง่ายกว่านี้หน่อย ตอนนี้มันเลือกยากไป
1.เลือกว่าวันไหนถึงวันไหน 2.โทรกี่รอบ 3.มีปุ่มติ๊กเลือกเอาว่า วันนี้ให้คนโทร AI โทร"*

| ไฟล์ | แก้อะไร |
| --- | --- |
| `src/pages/follow/FollowPage.tsx` | ใส่หัวข้อ **1 · เลือกช่วงวัน** · **2 · วันละกี่รอบ** · **3 · แต่ละวันใครโทร** ตามที่เจ้าของไล่มา · เปลี่ยนชิปกดวนเป็น **ปุ่มสามอันต่อวัน** (AI โทร / เราโทร / ไม่โทร) กดอันที่ต้องการตรง ๆ · จัดสองคอลัมน์ตั้งแต่จอ sm |

🔴 **ทำไมต้องเลิกใช้ชิปกดวน** — ชิปเดิมกดวน AI → เราโทร → ไม่โทร → AI
บังคับให้คนเดาว่าต้องกดอีกกี่ทีถึงได้อันที่ต้องการ (อยากได้ "ไม่โทร" ต้องกดสองที ·
เผลอเกินหนึ่งทีวนกลับต้นใหม่) และ **มองไม่เห็นว่ามีทางเลือกอะไรบ้าง** จนกว่าจะกดไปเจอ

**วัดบนจอจริง:** 7 วัน → เนื้อหา 1,117px · 31 วัน → 1,660px (กรอบ 893px) ·
กด "ไม่โทร" ของเสาร์/อาทิตย์ และ "เราโทร" ของจันทร์ **ทีเดียวติดทุกปุ่ม** ·
สรุปขยับถูก "รวม 5 วัน × 1 รอบ = 5 สาย — AI โทร 4 · เราโทรเอง 1" ·
กดกลับเป็น AI ทีเดียวได้ 6 วัน/AI 5
⚠️ **เทสต์ `typographyRules` จับ `font-semibold`** ที่เผลอใส่ในหัวข้อขั้น — ทั้งระบบหนาได้สุด `font-medium`

### แก้รอบสอง (เย็นวันเดียวกัน) — เจ้าของบอกว่ายังซับซ้อน
*"เลือกช่วงวัน ไอตรงนี้แหละที่เอา แต่ละวันใครโทรอะมาไว้ข้างหลัง แล้วทำเป็น Checkbox
เลือกว่า Ai โทรหรือคนโทร แค่นี้เองทำไรให้มันซับซ้อนทำไม"*

* **ยุบเหลือสองขั้น** — "แต่ละวันใครโทร" ย้ายเข้าไปอยู่ใต้ช่องวันในขั้น **1 · เลือกช่วงวัน**
  (กางช่วงวันเสร็จติ๊กต่อได้เลย ไม่ต้องเลื่อนไปอีกหัวข้อ) · เหลือ **2 · วันละกี่รอบ**
* **ปุ่มสามอัน → Checkbox อันเดียวต่อวัน** (`@/components/ui/checkbox`)
  ติ๊ก = AI โทร · ไม่ติ๊ก = คนโทร · ปุ่มลัดเหลือ "ทั้งหมด: AI โทร / คนโทร"
* 🔴 **ทางเลือก "ไม่โทร" ถูกถอดออกจากจอ** — เจ้าของสั่งเองว่าเอาแค่สองทาง
  ตัวแปร `skippedDays` กับตัวกรองตอนส่งยังอยู่ (เซ็ตว่างเสมอ) เผื่อสั่งให้เติมกลับ
  **ห้ามเติมปุ่มกลับเองโดยไม่ได้สั่ง** · อยากข้ามวันให้ย่นช่วงวันแทน

**วัดบนจอจริง:** 7 วัน ติ๊ก 7 อัน · ติ๊กออก 2 วัน → "รวม 7 วัน × 1 รอบ = 7 สาย —
AI โทร 5 · เราโทรเอง 2" · ปุ่มลัด "คนโทร" → AI 0/เราโทร 7 · "AI โทร" → AI 7/เราโทร 0

### แก้รอบสาม — หนึ่งวันหนึ่งแถว · หลังวันมีสองช่อง
*"ไม่เอา ต้องการแบบเลือกว่า Ai โทร หรือคนโทร แบบ 2 ช่องให้เลือกอะ แล้วขอเป็นแถว
ไม่ใช่ทำมาแบบนี้ดูแล้วงง เช่น เลือก 25-30 แบ่งแถว 25-30 มา แล้วหลังวันก็เลือก แค่นี้เอง"*

รูปที่เจ้าของเอา: `ศุกร์ 25 ก.ย. …… ( ) AI โทร  (✓) คนโทร` — วันซ้าย สองช่องขวา

⚠️ **ห้ามกลับไปสองคอลัมน์** (`sm:grid-cols-2`) — เคยทำเพื่อให้ 31 วันสั้นลง
เจ้าของอ่านแล้วงงเพราะตาต้องกระโดดซ้าย-ขวา · แถวเดียวยาวกว่าแต่ไล่ตาลงได้
⚠️ **ห้ามยุบเหลือช่องติ๊กเดียว** — ติ๊ก/ไม่ติ๊ก บังคับให้คนแปลเองว่า "ไม่ติ๊ก" คืออะไร
สองช่องเขียนคำไว้ทั้งคู่ อ่านแล้วรู้เลย (ลองมาแล้วรอบสอง เจ้าของตีกลับ)
🔴 กดช่องที่ติ๊กอยู่แล้วซ้ำ = ไม่เปลี่ยนอะไร (ห้ามปล่อยให้ว่างทั้งคู่ — ไม่มีสถานะนั้นจริง)

**วัดบนจอจริง:** ช่วง 25-30 ก.ย. ได้ 6 แถว · 12 ช่องติ๊ก · กด "คนโทร" ของเสาร์/อาทิตย์
→ AI ของวันนั้นหลุดเอง · สรุป "รวม 6 วัน × 1 รอบ = 6 สาย — AI โทร 4 · เราโทรเอง 2" ·
กดช่องเดิมซ้ำแล้วค่าไม่เปลี่ยน

---

## 24 ก.ย. 2569 — ใบขอหายจากกล่องงาน เพราะนับใบแจ้งเข้าที่ถูกยกเลิกว่า "หาได้แล้ว"

เจ้าของแจ้ง *"LAO6909006 ใบขอหายไปไหน"* แล้วย้ำว่า *"ฉันไม่เคยเปลี่ยนกฎ … ฉันบอกเอาทุกใบ
ที่ยังต้องหา แต่ใบไหนยกเลิก หรือ ได้คนแล้วไม่เอา"* — **กติกาไม่ได้เปลี่ยน โค้ดเราทำไม่ตรงกติกาเอง**

🔴 `st_inform_head.status` มีสองค่า: **`A` ใช้งานอยู่ 36,306 แถว · `C` ยกเลิก 1,293 แถว**
ของเดิมนับทุกแถวโดยไม่ดูสถานะ ⇒ ใบขอที่มีใบแจ้งเข้า **ใบเดียวและถูกยกเลิกไปแล้ว**
ถูกตีว่า "หาได้ครบ" แล้วหลุดจากกล่องงานเงียบ ๆ ทั้งที่ ERP ยังเปิดอยู่
(`LAO6909006`: `is_inform_all='N'` · `inform_qty=0` · ใบแจ้ง `LAO6909018` status `C`
แถมรหัสไซต์ในใบแจ้งยังไม่ตรงกับใบขอ — น่าจะคีย์ผิดแล้วยกเลิก)

**ผิดกติกาหลักที่เขียนไว้ตั้งแต่ต้น: "ห้ามนับอัตราที่ถูกยกเลิกเป็นอัตราที่หาได้"**

| ไฟล์ | แก้อะไร |
| --- | --- |
| `api/_lib/siamrajStaffingOpen.ts` | **ใหม่** `activeInformWhereSql()` — 🔴 นิยาม "ใบแจ้งเข้าที่นับได้" ที่เดียว · ใส่ใน `effectiveInformQtySql()` และใน `NOT EXISTS` ของ `openStaffingRequestWhereSql()` (ต้องใส่ทั้งสองที่ ไม่งั้นใบที่มีแต่ใบแจ้งที่ยกเลิกยังตก `NOT EXISTS` อยู่ดี) |
| `api/_lib/siamrajSqlServerClosed.ts` | `has_inform` กรองสถานะ |
| `api/_lib/siamrajSqlServerThroughput.ts` | subquery `inform_cnt` กรองสถานะ |
| `api/_lib/oplExcelImport.ts` | `NOT EXISTS` ตอนหาใบที่ยังไม่มีคน กรองสถานะ |

**เทสต์:** `tests/api/siamrajStaffingOpen.test.ts` (+4) — รวมด่านกวาดไฟล์:
🔴 **ทุกไฟล์ใน `api/_lib` ที่มีคำว่า `st_inform_head` ต้องเรียก `activeInformWhereSql`**
(กันคนเขียนคิวรีใหม่แล้วลืมกรอง — สี่ที่ตอบคนละเลขคือบทเรียนเดิม)

**วัดบนของจริง:** ใบที่โดน **13 ใบ** — หลังแก้กลับเข้าฟีดครบ
ปี 2568: `OPL6802163` `LMO6803004` `LMO6804017` `LMO6812015` ·
ปี 2569: `LAM6901008` `LAO6906007` `LMO6906013` `LMO6906014` `OPL6907023` `LMO6907008`
`LMO6908007` `OPL6908118` `LAO6909006`

🔴 **5 ใบในนั้นเลยกำหนด 269–581 วัน** (เก่าสุด `OPL6802163` ต้องการ 21/2/2568) = ใบที่ถูกลืม
ค้างใน ERP ไม่มีใครปิด · **เจ้าของเคาะ 25 ก.ย. 2569: ปล่อยไว้ เดี๋ยวไล่ปิดใน ERP เอง**
⇒ **ห้ามใส่เส้นตัดอายุหรือป้าย "ค้างนานผิดปกติ" เองโดยไม่ได้สั่ง** — ซ่อนแล้วไม่มีใครเห็นอีก
คือบั๊กแบบเดียวกับที่เพิ่งแก้
⚠️ เลขบนแดชบอร์ด/SLA/กราฟขอ-ปิด ขยับตามโดยตั้งใจ (ของเดิมนับเกิน)

### ไม่ใช่บั๊ก (ตรวจแล้วในรอบเดียวกัน)
ไซต์ `69LBAL0071` "บุญถาวร - พนง. (ช่างซ่อมบำรุง) 4 คน" — ใบขอ 3 ใบ
(`LAO6908006` `LAM6906012` `LAM6906011`) ERP ปิดครบทุกใบ (`is_inform_all='Y'` · แจ้งเข้า 1/1)
⇒ หลุดจากกล่องงานถูกต้อง · ชื่อไซต์บอก 4 คนแต่มีใบขอ 3 ใบ = ใบที่ 4 **ยังไม่ถูกเปิดใน ERP**
