...
## ใบขอแยกสองหน้า — รายละเอียด / คนที่จับคู่ได้ (16 ส.ค. 2569)

เจ้าของเคาะจากภาพเสนอ: *"กดเข้าใบงานไปก็เจอของใครของมัน แต่แยกว่าอันนี้หน้ารายละเอียด
ใบงาน อีกหน้าเป็นหน้าที่ผ่านการ matching มาแล้ว"*

| ไฟล์ | หน้าที่ |
|---|---|
| `src/components/jobs/UnitRequestTabs.tsx` | แท็บสองหน้า — navigate จริง ไม่ใช่ซ่อน/โชว์ |
| `src/pages/jobs/UnitRequestMatchingPage.tsx` | หน้า "คนที่จับคู่ได้" — route `/jobs/siamraj/:id/matching` |
| `src/lib/unitMatchingView.ts` | **pure ทั้งไฟล์** — แบ่งกลุ่ม/นับ/สถานะท้ายแถว/บรรทัดข้อมูล |
| `src/pages/jobs/SiamrajUnitRequestDetailPage.tsx` | เพิ่มแท็บด้านบน (ของเดิมไม่ถูกแตะ) |

เทสต์ `src/lib/unitMatchingView.test.ts` (16 เคส) · mutation 2/2

⚠️ **navigate จริงไม่ใช่แท็บซ่อน/โชว์** — คนต้องส่งลิงก์หน้าที่ตัวเองดูอยู่ให้กันได้
⚠️ **กลุ่มที่ไม่มีคนต้องไม่ถูกสร้าง** (ต่างจากแถบตัวเลขที่โชว์ 0 เสมอ) — หัวข้อว่างเปล่า
กลางหน้ารายชื่อทำให้คนคิดว่าโหลดไม่ครบ
⚠️ **ใบที่ยังบอกที่มาไม่ได้ ไปกลุ่ม "ยังบอกที่มาไม่ได้" ท้ายสุด** ห้ามยัดรวมกับ "สมัครเข้ามาเอง"
⚠️ **"เบอร์ใช้โทรไม่ได้" ต้องเด่นกว่า "รอโทร"** — ไม่งั้นค้างในถังรอโทรตลอดกาลโดยไม่มีใครรู้
⚠️ ปุ่ม "ชวนคนที่เคยปฏิเสธ" **ส่งจริงตั้งแต่กด** (นิยามของเส้นนี้) จึงต้องสรุปผลให้เห็นทุกครั้ง
⚠️ หน้า Matching เดิม **ไม่ถูกปิด** — เปลี่ยนบทบาทเป็นมุมมองรวมทุกใบ
