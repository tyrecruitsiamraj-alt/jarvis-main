...
### รอบเก้าสิบ — ชื่อหน่วยงาน: จุดทำงานนำ คู่สัญญาตาม (เจ้าของจับได้เอง 3 ก.ย. 2569)
...
| `src/lib/unitDisplay.ts` (ใหม่) | `unitLabel` (จุดทำงาน) · `unitSubLabel` (คู่สัญญา) · `unitOneLine` (ที่แคบ/CSV) · `unitTitleText` |
| `unitRequestDisplay.jobBoardCardTitle()` | หัวการ์ดใช้ `unitLabel` — **จุดคอขวดของทั้งบอร์ด + ตัวเลือกหน่วยงานงานติดตาม** |
| `JobListPage` · `JobBoardView` · `JobDetailPage` · `JobDashboard` · `PreCheckPage` · `MatchingPage` · `EditPublicJobFieldsDialog` · `UnitRequestInfoFields` | จอขึ้นจุดทำงานก่อน + บรรทัด "คู่สัญญา …" ตาม · หน้ารายละเอียดแยกสองแถว "จุดทำงาน" / "คู่สัญญา" · ช่องค้นหาใบขอค้นชื่อจุดทำงานได้ด้วย |
...
**ด่านตรวจ:** เทสต์ใหม่ `tests/api/siamrajUnitName.test.ts` 14 ข้อ (มีเคสจริง 69LBDL0044) ·
รวม 2,598 ผ่าน · tsc 4 = 0 · eslint 0 err · build ผ่าน · ตรวจบนจอ: รายการใบขอ · การ์ดกล่องงาน · หน้ารายละเอียด
